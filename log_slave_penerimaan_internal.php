	...
	switch($method)
	{
		...
		$no=0;	 
                ...
                $sql2="select * from ".$dbname.".log_pengiriman_ht 
                      where lokasipenerima ='".$_SESSION['empl']['lokasitugas']."' order by nomor desc";
                //echo $sql2;
		...
		$str="select * from ".$dbname.".log_pengiriman_ht 
                      where lokasipenerima ='".$_SESSION['empl']['lokasitugas']."'  order by nomor desc limit ".$offset.",".$limit." ";
		$res=mysql_query($str);
		...
		<td>".tanggalnormal($bar['tanggalkirim'])."</td>
		 ...
		 echo"<td><input type=text class=myinputtext id=tglTrima onmousemove=setCalendar(this.id) onkeypress='return false';  size=10 maxlength=10 style=\"width:100px;\" /></td>";
                 echo"<td><button class=mybutton onclick=saveFranco('".$bar['nomor']."')>".$_SESSION['lang']['save']."</button></td>";
                }
                else
                {
		...
		case'update':
                    ...
                    $sCek="select distinct tanggalkirim from ".$dbname.".log_pengiriman_ht where nomor='".$idNomor."'";
...
//                    }
		    $sIns="update ".$dbname.".log_pengiriman_ht set tanggalterima='".$tglKrm."', penerima='".$_SESSION['standard']['userid']."' where nomor='".$idNomor."'";
                    if(!mysql_query($sIns))
                    {
                            echo"Gagal".mysql_error($conn);
                    }
