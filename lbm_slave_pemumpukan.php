...
//    where substr(a.kodeorg,1,4)='".$kdUnit."' and statusblok='TM' and substr(notransaksi,15,2)='TM' 
//    and (substr(tanggal,1,7) between '".$thn[0]."-01' and '".$periode."') and substr(kodebarang,1,3)='311' 
//    group by tahuntanam,kodebarang order by a.tahuntanam asc";
...
if($proses=='excel')
{
    ...
    $tab.="
    <table>
    <tr><td colspan=5 align=left><b>13. PEMUPUKAN</b></td><td colspan=7 align=right><b>".$_SESSION['lang']['bulan']." : ".substr(tanggalnormal($periode),1,7)."</b></td></tr>
    <tr><td colspan=5 align=left>".$_SESSION['lang']['unit']." : ".$optNmOrg[$kdUnit]." </td></tr>
    <tr><td colspan=5 align=left>&nbsp;</td></tr>
    </table>";
}
        
	$tab.="TANAMAN MENGHASILKAN ( TM )<br/><table cellspacing=1 border=".$brdr." class=sortable>
...
//        echo "</pre>";
        ...
        {
            $tab.="<td>Anggaran 1 Thn</td><td>Realisasi SBI</td><td>Sisa</td>";
        }
        $tab.="</tr></thead>
	<tbody>";
        ...
        else
        {
            ...
            $tab.="</tbody></table><br />";
            $tab.="TANAMAN BELUM MENGHASILKAN ( TBM )<br/><table cellspacing=1 border=".$brdr." class=sortable>
            <thead class=rowheader>
            <tr>
            <td rowspan=2>".$_SESSION['lang']['tahuntanam']."</td>
            <td rowspan=2>".$_SESSION['lang']['luas']." HA</td>
...
//                    $tab.= '<td>'.$lstKdBrg.'</td>';
        ...
        $tab.="</tbody></table><br />";
                ...
                $tab.="TANAMAN BARU ( TB )<br/><table cellspacing=1 border=".$brdr." class=sortable>
                <thead class=rowheader>
                <tr>
                <td rowspan=2>".$_SESSION['lang']['tahuntanam']."</td>
                <td rowspan=2>".$_SESSION['lang']['luas']." HA</td>
        ...
        case'pdf':
           ...
           function Header() {
            ...
            global $optNmBrg;
                ...
                $this->Cell($width,$height,strtoupper("13. PEMUPUKAN "),0,1,'L');
...
//                $this->SetFont('Arial','B',5);
            ...
            $pdf->Cell(245,$height,"TANAMAN MENGHASILKAN ( TM )",0,1,'L',1);
            $pdf->SetFillColor(220,220,220);
            $pdf->SetFont('Arial','B',5);
                $pdf->Cell(45,$height,"Tahun",TLR,0,'C',1);
                $pdf->Cell(45,$height,"Luas",TLR,0,'C',1);
                if(!empty($dtBarang))foreach($dtBarang as $lstKdBrg)
                {
                    $ard+=1;
                    if($ard!=$cols)
                ...
                $pdf->Cell(45,$height,"Tanam",BLR,0,'C',1);
                ...
                if(!empty($dtBarang))foreach($dtBarang as $lstKdBrg)
                {
                    ...
                    if($ard5!=$cols)
                    {
                     $pdf->Cell(45,$height,"Anggaran 1 Thn",TBLR,0,'C',1);
                     $pdf->Cell(45,$height,"Realisasi SBI",TBLR,0,'C',1);
            ...
            $pdf->SetFillColor(255,255,255);
            $pdf->Cell(245,$height,"TANAMAN BELUM MENGHASILKAN ( TBM )",0,1,'L',1);
            $pdf->SetFillColor(220,220,220);
            $pdf->SetFont('Arial','B',5);
                $pdf->Cell(45,$height,"Tahun",TLR,0,'C',1);
                $pdf->Cell(45,$height,"Luas",TLR,0,'C',1);
                ...
                $pdf->SetY($tinggi+10);
                 $pdf->SetFillColor(255,255,255);
            $pdf->Cell(245,$height,"TANAMAN BELUM MENGHASILKAN ( TB )",0,1,'L',1);
            $pdf->SetFillColor(220,220,220);
            $pdf->SetFont('Arial','B',5);
                $pdf->Cell(45,$height,"Tahun",TLR,0,'C',1);
                $pdf->Cell(45,$height,"Luas",TLR,0,'C',1);
