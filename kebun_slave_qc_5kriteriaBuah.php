...
switch($proses)
{
	...
	$sql="select distinct kode,nama from ".$dbname.".kebun_qc_5stbuah order by nama asc";
	...
	if($nm_ancak==''||$KbnId=='')
	{
        ...
        $str="select * from ".$dbname.".kebun_qc_5stbuah where kode='".$KbnId."'";
        //exit("Error".$str) ;
        $res=mysql_query($str);
	...
	$sqlCek="select kode from ".$dbname.".kebun_qc_5stbuah where kode='".$KbnId."'"; 
	$queryCek=mysql_query($sqlCek) or die(mysql_error());
	$rowCek=mysql_fetch_row($queryCek);
	if($rowCek<1)
	{
		$sql="insert into ".$dbname.".kebun_qc_5stbuah 
		(`kode`,`nama`) 
	...
	$sql="update ".$dbname.".kebun_qc_5stbuah set nama='".$nm_ancak."' where kode='".$KbnId."'";
	//echo "warning:".$sql;
	if(mysql_query($sql))
	echo"";
	else
	...
	break;
	case'delHead':
            $sql="delete from  ".$dbname.".kebun_qc_5stbuah  where kode='".$KbnId."'";
            //echo "warning:".$sql;
            if(mysql_query($sql))
            echo"";
            else
	...
	$ql2="select count(*) as jmlhrow from ".$dbname.".kebun_qc_5stbuah order by kode asc";// echo $ql2;
	...
	//$sql="select * from ".$dbname.".kebun_qc_5stancak order by nama desc limit ".$offset.",".$limit."";
        $sql="select * from ".$dbname.".kebun_qc_5stbuah order by kode asc";
	$query=mysql_query($sql) or die(mysql_error());
	while($res=mysql_fetch_assoc($query))
	{
		$no+=1;
...
//				</tr>";
	...
	$sqlCek="select kode from ".$dbname.".kebun_qc_5nilaibuah where kode='".$idAncak."'"; 
	$queryCek=mysql_query($sqlCek) or die(mysql_error());
	$rowCek=mysql_fetch_row($queryCek);
	if($rowCek<1)
	{
		$sql="insert into ".$dbname.".kebun_qc_5nilaibuah 
        ...
        if($idAncak==''||$nilDari=='0'||$nilSmp=='0'||$nilai=='0')
	{
        ...
        $sql="update ".$dbname.".kebun_qc_5nilaibuah set `dari`='".$nilDari."',`sampai`='".$nilSmp."',`nilai`='".$nilai."' 
              where `kode`='".$idAncak."'";
        if(mysql_query($sql))
        echo"";
        else
        ...
        $sql="delete from ".$dbname.".kebun_qc_5nilaibuah  where `kode`='".$idAncak."'";
        if(mysql_query($sql))
        echo"";
        else
        echo "DB Error : ".mysql_error($conn);
	...
	$ql2="select count(*) as jmlhrow from ".$dbname.".kebun_qc_5nilaibuah order by kode asc";// echo $ql2;
	$query2=mysql_query($ql2) or die(mysql_error());
	while($jsl=mysql_fetch_object($query2)){
	$jlhbrs= $jsl->jmlhrow;
	}
	...
	//$sql="select * from ".$dbname.".kebun_qc_5nilaiancak order by kebun_qc_5nilaiancak desc limit ".$offset.",".$limit."";
        $sql="select * from ".$dbname.".kebun_qc_5nilaibuah order by kode asc";
	$query=mysql_query($sql) or die(mysql_error());
	while($res=mysql_fetch_assoc($query))
	{
		$no+=1;
