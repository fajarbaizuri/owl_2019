...
if($proses=='excel')
{
    ...
    $tab.="
    <table>
    <tr><td colspan=8 align=left><b> 04.2 PEKERJA DAN TANGGUNGAN  </b></td><td colspan=3 align=right><b>".$_SESSION['lang']['bulan']." : ".substr(tanggalnormal($periode),1,7)."</b></td></tr>
	...
	<tr>
        ...
        <td ".$bgcoloraja." rowspan=2 colspan=2>JUMLAH KARYAWAN</td>
        <td ".$bgcoloraja." rowspan=2 colspan=2>PENAMBAHAN</td>
        <td ".$bgcoloraja." rowspan=2 colspan=2>PENGURANGAN</td>
        <td ".$bgcoloraja." rowspan=3>POSISI s/d AKHIR BI</td>
        <td ".$bgcoloraja." rowspan=3>TURN OVER KARYAWAN  (%)</td>";
        $tab.="<tr>";
        $tab.="<tr class=rowcontent><td  ".$bgcoloraja.">AWAL TAHUN</td><td  ".$bgcoloraja.">BULAN LALU</td>";
        $tab.="<td  ".$bgcoloraja.">BI</td><td  ".$bgcoloraja.">s/d BI</td><td  ".$bgcoloraja.">BI</td><td  ".$bgcoloraja.">s/d BI</td></tr>";
        $tab.="</thead>
	<tbody>";
        //staff
        ...
        case'pdf':
           ...
           function Header() {
            ...
            global $afdId;
                ...
                $this->Cell($width,$height,strtoupper("04.3 MUTASI KARYAWAN "),0,1,'L');
                ...
                $this->Cell(180,30,"JUMLAH KARYAWAN",1,0,'C',1);
                $this->Cell(90,30,"PENAMBAHAN",1,0,'C',1);
                $this->Cell(90,30,"PENGURANGAN",1,0,'C',1);
                $this->SetFont('Arial','B',6);
                $this->Cell(90,$height,"POSISI s/d AKHIR BI",1,0,'C',1);
                $this->Cell(90,$height,"TURN OVER KARYAWAN (%)",1,0,'C',1);
                $tinggiAkr=$this->GetY();
                $ksamping=$this->GetX();
                $this->SetY($tinggiAkr+30);
                $this->SetX($ksamping-540);
                $this->Cell(90,30,"AWAL TAHUN",1,0,'C',1);
                $this->Cell(90,30,"BULAN LALU",1,0,'C',1);
                $this->Cell(45,30,"BI",1,0,'C',1);
                $this->Cell(45,30,"s/d BI",1,0,'C',1);
                $this->Cell(45,30,"BI",1,0,'C',1);
                $this->Cell(45,30,"s/d BI",1,1,'C',1);   
          }
              function Footer()
            {
                $this->SetY(-15);
