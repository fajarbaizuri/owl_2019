...
//print_r($thnProd);
	...
	switch($proses)
	{
		...
		case'loadData':
                ...
                $offset=$page*$limit;
                 $sql2="select count(b.kodeorg) as jmlhrow from ".$dbname.".setup_blok a left join ".$dbname.".kebun_5bjr b on a.kodeorg=b.kodeorg
                      where b.kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by b.kodeorg asc";
                    //exit("Error".$sql2);
		...
		$str="select a.tahuntanam,a.jenisbibit,b.* from ".$dbname.".setup_blok a left join ".$dbname.".kebun_5bjr b on a.kodeorg=b.kodeorg
                      where b.kodeorg like '".$_SESSION['empl']['lokasitugas']."%' and tahunproduksi='".$thnProd."' order by b.kodeorg asc  limit ".$offset.",".$limit."";
		$res=mysql_query($str);
