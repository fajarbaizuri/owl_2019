...
$aresta="SELECT noakun,sum(jumlah) as biaya FROM ".$dbname.".keu_jurnaldt_vw
    WHERE kodeorg like '".$unit."%' and tanggal like '".$periode."%' and (noakun like '7%' )
    GROUP BY noakun";
//echo $aresta;
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
...
$aresta="SELECT noakun,sum(rp".$bulan.") as budget FROM ".$dbname.".bgt_budget_detail
    WHERE kodeorg like '".$unit."%' and tahunbudget = '".$tahun."' and (noakun like '7%' )
    GROUP BY noakun";
//echo $aresta;
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
...
// biaya sd bulan ini
$aresta="SELECT noakun,sum(jumlah) as biaya FROM ".$dbname.".keu_jurnaldt_vw
    WHERE kodeorg like '".$unit."%' and tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15') and (noakun like '7%' )
    GROUP BY noakun";
//echo $aresta;
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
...
$aresta="SELECT noakun, namaakun FROM ".$dbname.".keu_5akun
    WHERE length(noakun)=7 and (noakun like '7%' )
    ORDER BY noakun";
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
