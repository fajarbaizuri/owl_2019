...
$kgPtngan=$_POST['kgPtngan'];
	switch($proses)
	{
		...
		{
			echo"warning:Data Timbangan Kosong";
			exit();
		}
		break;
		case'getData':
		...
		";
		$sFraksi="select kode,keterangan,type from ".$dbname.".pabrik_5fraksi order by kode asc";
		...
		echo"<td>JJG Dikirim</td><td>KG Potongan</td>
		<td>Action</td>
		</tr>
		</thead>
		<tbody>";
		...
		while($rNotiket=mysql_fetch_assoc($qNotiket))
		{
			...
			echo"<td align=right>".number_format($rJjg['jjgsortasi'],0)."</td>";
                      //  echo"<td align=right>".number_format($rJjg['persenBrondolan'],0)."</td>";
                        echo"<td align=right>".number_format($rJjg['kgpotsortasi'],0)."</td>";
			echo"<td>
...
//                    echo"</pre><br />";
		...
		break;
                ...
                #ambil potongan sortasi
                    ...
                    echo"</tr></thead>
                         <tbody><tr class=rowcontent><td>Standar Potongan*100(%)</td><td id=nettox></td><td id=bjrx></td>";
		...
		$table.="<table id='ppDetailTable'>";
                ...
                $table .= "<thead>";
                $table .= "<tr>";
                $table .= "<td>".$_SESSION['lang']['noTiket']."</td><td>JJG Dikirim</td>";
                $qHead="select distinct kode,keterangan from ".$dbname.".pabrik_5fraksi  order by kode asc";
               ...
               // $table .= "<td>% Brondolan</td><td>KG Potongan</td><td>Action</td></tr>";
               ...
               // $table.="<td><input type=text class=myinputtextnumber style='width:65px;' id=persenBrnd  onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"4\" value=0  onblur=hitungPotongan(this.value,'BRD',".mysql_num_rows($zd).")></td>";
		...
		echo"<td>JJG Dikirim</td><td>KG Potongan</td><td>Action</td></tr></thead><tbody>";
