...
//GET LAIN dan Total Per Kepala Akun
$sSdm="select tahunbudget, afdeling, tipebudget, kodebudget, kegiatan,sum(rupiah) as rupiah, namakegiatan,substr(kegiatan,1,3) as kepalaAkn from ".$dbname.".bgt_budget_kegiatan_vw
    where afdeling like '".$kodeOrg."%' and tahunbudget='".$thnBudget."' and kodebudget like 'OTHER%' and kodebudget!='UMUM' group by kegiatan order by kegiatan asc";
$qSdm=mysql_query($sSdm) or die(mysql_error($sSdm));
while($rSdm=mysql_fetch_assoc($qSdm))
{
    $dataKeg[$rSdm['tahunbudget']][$rSdm['kegiatan']][$rSdm['kepalaAkn']][lain]+=$rSdm['rupiah'];
    $totalKplaSDM[$rSdm['tahunbudget']][$rSdm['kepalaAkn']][lain]+=$rSdm['rupiah'];
}
$sSdm2="select tahunbudget, sum(rupiah) as rupiah,substr(kegiatan,1,3) as kepalaAkn from ".$dbname.".bgt_budget_kegiatan_vw
    where afdeling like '".$kodeOrg."%' and tahunbudget='".$thnBudget."' and kodebudget like 'OTHER%'  and kodebudget!='UMUM' group by kepalaAkn order by kepalaAkn asc";
$qSdm2=mysql_query($sSdm2) or die(mysql_error($sSdm2));
while($rSdm2=mysql_fetch_assoc($qSdm2))
{
    $totalKplaSDM[$rSdm2['tahunbudget']][$rSdm2['kepalaAkn']][lain]=$rSdm2['rupiah'];
}
//exit("error".$sSdm2);
...
if($kodeOrg==''||$thnBudget=='')
{
    exit("Error:Field Tidak Boleh Kosong");
            ...
            $arrLang=array("0"=>$_SESSION['lang']['sdm'],"1"=>$_SESSION['lang']['material'],"2"=>$_SESSION['lang']['peralatan'],"3"=>$_SESSION['lang']['kndran'],"4"=>$_SESSION['lang']['kontrak'],"5"=>"Lain-Lain");
            ...
            foreach($listNoakun as $barisNoakun)
            {
                ...
                if($ktKrgng!='' and $ktKrgng!=$new)
                {
                    ...
                    if(substr($ktKrgng,0,1)!=6)
                    {
						...
						<td align=right colspan=2>".$_SESSION['lang']['total']." TBM</td>";
						...
						$tab.="<td align=right>".number_format($totalKplaSDM[$thnBudget][$ktKrgng][lain],2)."</td>";
                        $tab.="<td align=right>".number_format($totKepla[$thnBudget][$ktKrgng][lain],2)."</td>";
                        $tab.="</tr></thead>";  
                        }
                        else if($ktKrgng=='127')
                        {
                    ...
                    else 
                    {
                      ...
                      if($awal==$rTotal)
                      {
						...
						@$totKepalas[$thnBudget][$ktKrgng][lain]=$totalKplaSDM[$thnBudget][$ktKrgng][lain]/$ttlLuastbm;
						...
						$tab.="<td align=right>".number_format($totalKplaSDM[$thnBudget][$ktKrgng][lain],2)."</td>";
                        $tab.="<td align=right>".number_format($totKepalas[$thnBudget][$ktKrgng][lain],2)."</td>";
                        $tab.="</tr></thead>";  
                        $awal=0;
                      }
...
//                    
                 ...
                 if(substr($new,0,1)!=6)
                    {
                        if($new=='123' || $new=='122')
                        {
                            ...
                            if($new=='122')$pembagi[$new]=$ttlLuasLc; 
					...
					$tab.="<td align=right>".number_format($dataKeg[$thnBudget][$barisNoakun][$new][lain],2)."</td>";
                    $tab.="<td align=right>".number_format($kegKntrak[$thnBudget][$barisNoakun][$new],2)."</td>";
                    $tab.="</tr>";
				...
				$grnTotKegLain+=$dataKeg[$thnBudget][$barisNoakun][$new][lain];
                $grnTotKeghaLain+=$kegKntrak[$thnBudget][$barisNoakun][$new];
           ...
           if($ktKrgng=='122')
			...
			@$bagiTotalLain=$totalKplaSDM[$thnBudget][$ktKrgng][lain]/$ttlLuastbm;
            $tab.="<td align=right>".number_format($totRupiah[$thnBudget][$ktKrgng],2)."</td>";
            $tab.="<td align=right>".number_format($totBagi,2)."</td>";
            $tab.="<td align=right>".number_format($totalKplaSDM[$thnBudget][$ktKrgng][sdm],2)."</td>";
			...
			$tab.="<td align=right>".number_format($totalKplaSDM[$thnBudget][$ktKrgng][lain],2)."</td>";
            $tab.="<td align=right>".number_format($bagiTotalLain,2)."</td>";
            $tab.="</tr>";  
            $tab.="<tr class='rowheader'>";
			...
			$tab.="<td align=right>".number_format($grnTotKegLain,2)."</td>";
            $tab.="<td align=right>".number_format($grnTotKeghaLain,2)."</td>";
            $tab.="</tr></thead>";
            $tab.="</tbody></table>";
