...
if($nopp=='')
{
    exit("Error:No PP Tidak Boleh Kosong");
}
...
switch($proses)
{
 ...
 //       }
	...
	<tbody>";
        ...
        $tab.="</tr>";
        $tab.="<tr class='rowcontent'><td rowspan=5 colspan=3 valign=top align=left>".$_SESSION['lang']['rekomendasi']."</td>";
        ...
        if($formPil!='0')
        {
            $tab.="<tr class=rowcontent><td colspan=5>&nbsp;</td>";
            
            foreach($dtNomor as $brsNomor2 =>$listNomor2)
            {
                $noUrut+=1;
                $tab.="<td align=center colspan=3><button class=mybutton id=save_".$noUrut." onclick=simpanSemua(".$noUrut.",".$totRow.")>".$_SESSION['lang']['save']."</button></td>";
            }
            $tab.="</tr>";
        }
	$tab.="</tbody></table>";
...
//                                    $nilDiskon=str_replace(',', '', $nilDiskon);     
                                    $sUpdate="update ".$dbname.".log_perintaanhargaht set id_franco='".$idFranco."', stock='".$stockId."', catatan='".$ketUraian."',sisbayar='".$termPay."', ppn='".$nilPPn."', subtotal='".$subTotal."', diskonpersen='".$diskonPersen."', nilaidiskon='".$nilDiskon."', nilaipermintaan='".$nilaiPermintaan."'
