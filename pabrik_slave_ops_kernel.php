...
switch($proses) {
    ...
    # Form Edit Header
    case 'showEdit':
		$query = selectQuery($dbname,'pabrik_ops_kernelht',"*",
    ...
    case 'add':
		...
		$query = insertQuery($dbname,'pabrik_ops_kernelht',$data,$cols);
    ...
    case 'edit':
		...
		$query = updateQuery($dbname,'pabrik_ops_kernelht',$data,$where);
		if(!mysql_query($query)) {
			echo "DB Error : ".mysql_error();
		}
		break;
    case 'delete':
		...
		$query = "delete from `".$dbname."`.`pabrik_ops_kernelht` where ".$where;
		if(!mysql_query($query)) {
			echo "DB Error : ".mysql_error();
			exit;
		}
