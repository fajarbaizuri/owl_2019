...
if($proses=='excel')
{
    ...
    $tab.="
    <table>
    <tr><td colspan=5 align=left><b>17.2  PENGIRIMAN BIBIT</b></td><td colspan=7 align=right><b>".$_SESSION['lang']['bulan']." : ".substr(tanggalnormal($periode),1,7)."</b></td></tr>
    <tr><td colspan=5 align=left>".$_SESSION['lang']['unit']." : ".$optNmOrg[$kdUnit]." </td></tr>
    <tr><td colspan=5 align=left>&nbsp;</td></tr>
    </table>";
}

        $arrData=array("TB"=>"PENANAMAN BARU (TB)","SISIP"=>"SISIPAN");
	$tab.="<table cellspacing=1 border=".$brdr." class=sortable>
	<thead class=rowheader>";
        $tab.="<tr><td rowspan=2>No.</td><td rowspan=2>T U J U A N</td><td rowspan=2 colspan=2>BIBIT</td>";
	$tab.="<td colspan='".$colsPertama."'>AFDELING KEBUN SENDIRI</td><td colspan='".$colsKedua."'>KEBUN LAIN</td></tr>";
        $tab.="<tr>";
        if($colsPertama!=0)
        {
            foreach($kbnSndri as $lstKbn)
            {
                $tab.="<td>".$lstKbn."</td>";
            }
        }
        ...
        $tab.="</tr></thead><tbody>";
        $i=0;
        foreach($arrData as $dtini=>$isData)
        {
          ...
          if(!empty($lstBibit)){
            ...
            {   
               ...
               if($afdC==false) {
                    ...
                    if($blankC==false) {
                        $tab .= "<td rowspan='".(4-1)."'></td><td rowspan='".(4-1)."'></td>";
                        $blankC = true;
                    }
                }
               $tab.="<td>".$dtBibit."</td>";
            ...
            $tab.="<tr class=rowcontent><td>Sub Total</td><td>Bi</td>";
            if($colsPertama!=0)
            {
                 foreach($kbnSndri as $lstKbn)
                 {
            ...
            else
            {
                ...
                $tab.="</tr>";
                  $tab.="<tr class=rowcontent><td >&nbsp;</td><td>s/d Bi</td>";
            if($colsPertama!=0)
            {    
                 foreach($kbnSndri as $lstKbn)
                 {
        ...
        $tab.="<tr><td>&nbsp;</td><td colspan=2>Grand Total</td><td>Bi</td>";
         if($colsPertama!=0)
         {  
            foreach($kbnSndri as $lstKbn)
             {
         ...
         else
         {
            ...
            $tab.="</tr>";
        $tab.="<tr><td>&nbsp;</td><td colspan=2>&nbsp;</td><td>s/d Bi</td>";
        if($colsPertama!=0)
        {  
            foreach($kbnSndri as $lstKbn)
             {
        ...
        case'pdf':
           ...
           function Header() {
            ...
            global $colsKedua;
                ...
                $this->Cell($width,$height,strtoupper("17.2  PENGIRIMAN BIBIT"),0,1,'L');
                ...
                $this->Cell(130,$height,"TUJUAN",TLR,0,'C',1);
                $this->Cell(100,$height,"BIBIT",TLR,0,'C',1);
                $this->Cell($cold,$height,"AFDELING KEBUN SENDIRI",TLR,0,'C',1);
                $this->Cell($cold2,$height,"KEBUN LAIN",TLR,1,'C',1);
          ...
          if(!empty($lstBibit)){
            ...
            {   
               if($afdC==false) {
                ...
                $pdf->Cell(20,$height,"Bi",TLR,0,'L',1);
                ...
                $pdf->Cell(20,$height,"s/d Bi",TBLR,0,'L',1);
            ...
            $pdf->Cell(20,$height,"Bi",TBLR,0,'L',1);
           if($colsPertama!=0)
           {
                 foreach($kbnSndri as $lstKbn)
                 {
