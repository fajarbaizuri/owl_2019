...
switch($proses)
{
	...
	$sql="select sum(kuantitas) as cpo,sum(kernelquantity) as kernel,kodetangki from ".$dbname.".pabrik_masukkeluartangki where kodeorg = '".$kodeorg1."' and tanggal = '".$tanggal1x."' group by kodetangki";
	$res1 = fetchData($sql);$z=$m=0;
	foreach ($res1 as $r) {
		$z+=$r['cpo'];
		$m+=$r['kernel'];
	}
...
/*	$a1="select sum (kuantitas) as cpo, sum(kernelquantity) as kernel where kodeorg = '".$kodeorg1."' and tanggal = '".$tanggal1x."'";
	$a2=mysql_query($a1);
	$a3=mysql_fetch_assoc($a2);
		$stokawalCPO=$a3['cpo'];
		$stokAwalKernel=$a3['kernel'];*/
      ...
      //$e=$m-$n;
        ...
        echo"<td align=right style=cursor:pointer onclick=viewDetail('".$kodeorg1."','".$tanggal1x."','40000001',event) title='Detail Pengiriman'>".number_format($c,0)."</td>";
        ...
        echo"<td align=right style=cursor:pointer onclick=viewDetail('".$kodeorg1."','".$tanggal1x."','40000002',event) title='Detail Pengiriman'>".number_format($g,0)."</td>";
        echo"<td align=right>".number_format($m,0)."</td>";
        echo"</tr>";
		echo"</tbody></table><br>";
		
		echo"<table class=sortable cellspacing=1 border=0>
			<thead><tr class=rowheader><td></td>";
		foreach($res1 as $r) {
			echo "<td>".$r['kodetangki']."</td>";
		}
		echo "</tr></thead><tbody>";
		echo"<tr class=rowcontent>";
        echo"<td>CPO</td>";
		foreach($res1 as $r) {
			echo "<td align=right>".number_format($r['cpo'],0)."</td>";
		}
        echo"</tr>";
        echo"<tr class=rowcontent>";
        echo"<td>Kernel</td>";
        foreach($res1 as $r) {
			echo "<td align=right>".number_format($r['kernel'],0)."</td>";
		}
        echo"</tr>";
		echo"</tbody></table>";
	break;
