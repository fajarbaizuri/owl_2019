	...
	//exit("Error:MASUK");
		...
		$ql2="select count(*) as jmlhrow from ".$dbname.".bgt_budget_detail a left join ".$dbname.".setup_kegiatan b ".
			"on a.kegiatan=b.kodekegiatan where b.kelompok in ('TB','LC','PNN','TBM','TM') and a.kodeorg like '".
				$_SESSION['empl']['lokasitugas']."%' and not kodebudget in ('UMUM','OTHER') and tipebudget='ESTATE' and tutup=0 and tahunbudget=".$thnbudgetHeader;
		}else{
			$ql2="select count(*) as jmlhrow from ".$dbname.".bgt_budget_detail a left join ".$dbname.".setup_kegiatan b ".
    "on a.kegiatan=b.kodekegiatan where b.kelompok in ('TB','LC','PNN','TBM','TM') and a.kodeorg like '".
        $_SESSION['empl']['lokasitugas']."%' and kodebudget='OTHER' and tipebudget='ESTATE' and tutup=0 and tahunbudget=".$thnbudgetHeader;
		}		
				//echo $ql2;
...
,a.rp02,a.rp03,a.rp04,a.rp05,a.rp06,a.rp07,a.rp08,a.rp09,a.rp10,a.rp11,a.rp12 from ".$dbname.".bgt_budget_detail a left join ".$dbname.".setup_kegiatan b ".
			"on a.kegiatan=b.kodekegiatan where b.kelompok in ('TB','LC','PNN','TBM','TM') and a.kodeorg like '".
				$_SESSION['empl']['lokasitugas']."%' and not kodebudget in ('UMUM','OTHER') and tipebudget='ESTATE' and tutup=0 and tahunbudget=".$thnbudgetHeader." limit ".$offset.",".$limit."";
		}else{
			$sLoad="select a.tahunbudget,a.kodeorg,a.kodebudget,a.kegiatan,a.rupiah,a.kodevhc,a.kodebarang,a.kunci,a.jumlah,a.rp01
,a.rp02,a.rp03,a.rp04,a.rp05,a.rp06,a.rp07,a.rp08,a.rp09,a.rp10,a.rp11,a.rp12 from ".$dbname.".bgt_budget_detail a left join ".$dbname.".setup_kegiatan b ".
    "on a.kegiatan=b.kodekegiatan where b.kelompok in ('TB','LC','PNN','TBM','TM') and a.kodeorg like '".
        $_SESSION['empl']['lokasitugas']."%' and a.kodebudget='OTHER' and a.tipebudget='ESTATE' and a.tutup=0 and a.tahunbudget=".$thnbudgetHeader." limit ".$offset.",".$limit." ";
		}		
		//echo $sLoad;
		//exit("Error:".$sLoad);		
		...
		foreach($resSebaran as $key=>$row) {
				$res.="<tr class=rowcontent style='cursor:pointer;' id=barisSebaran".$XX.$key.">";
				...
				$dtClik="onclick=\"getForm('Sebaran','<fieldset style=\'width:520px;height:400px;\'>".
						...
						"width:480px\'></div><input type=hidden id=keyId2".$XX." value=".$row['kunci']." /></fieldset>',".
						$row['rupiah'].",".$row['jumlah'].",'".$row['kodebudget']."',event,true);\"";
		
				$res.="<td><input type=checkbox onclick=sebarkanRawat('".$row['kunci']."',".
						$key.",this,".$row['rupiah'].",".$row['jumlah'].",'".$XX."'); title='Sebarkan sesuai proporsi diatas'></td>";
				$res.="<td >".($key+1)."</td>";
		...
		if(isset($page))
		{
			...
			if($page <=1 )
			{
				$res.="<button class=mybutton>".$_SESSION['lang']['pref']."</button>";
			}else{
				$j = $page - 1;
				if ($XX=="s"){
				$res.="<button class=mybutton onclick=cariBastSebarRawat(".($j).");>".$_SESSION['lang']['pref']."</button>";
				}else{
				$res.="<button class=mybutton onclick=cariBastSebarLain(".($j).");>".$_SESSION['lang']['pref']."</button>";
				}
				
			}
				if ($XX=="s"){
					$res.="<select name=select1 onchange=choosePage(this);>";
				}else{
					$res.="<select name=select1L onchange=choosePageL(this);>";
				}
			...
			$j = $page + 1;
			if ($XX=="s"){
				$res.="<button class=mybutton onclick=cariBastSebarRawat(".($j).");>".$_SESSION['lang']['lanjut']."</button>";
			}else{
				$res.="<button class=mybutton onclick=cariBastSebarLain(".($j).");>".$_SESSION['lang']['lanjut']."</button>";
			}
