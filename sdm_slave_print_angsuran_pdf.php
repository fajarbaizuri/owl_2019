...
switch ($val){
	...
	break;
	case 'blmlunas':
		$valstat=$_SESSION['lang']['belumlunas'];
			if($_SESSION['org']['tipeorganisasi']=='HOLDING')
			{			    
				$str="select a.*,u.namakaryawan from ".$dbname.".sdm_angsuran a, ".$dbname.".datakaryawan u
				      where a.karyawanid=u.karyawanid
	...
	break;
	case 'notactive':
		   $valstat=$_SESSION['lang']['tidakaktif'];
			if($_SESSION['org']['tipeorganisasi']=='HOLDING')
			{			    
				$str="select a.*,u.namakaryawan from ".$dbname.".sdm_angsuran a, ".$dbname.".datakaryawan u
				      where a.karyawanid=u.karyawanid
...
//================================
	...
	$no=0;
		...
		$pdf->Cell(20,5,($bar->active==1?"Aktif":"Tdk.Aktif"),0,1,'C');			  			
		}
	$pdf->Output();	
