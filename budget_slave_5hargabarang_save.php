...
if($what=='update')
{
// UPDATE `owl`.`bgt_masterbarang` SET `hargasatuan` = '22',
//`hargalalu` = '20' WHERE `bgt_masterbarang`.`regional` = 'DKI' AND `bgt_masterbarang`.`tahunbudget` =2012 AND `bgt_masterbarang`.`kodebarang` = '31300001';
...
if($what=='edit')
{
$str="select * from ".$dbname.".bgt_masterbarang where tahunbudget='".$tahunbudget."' and regional='".$regional."' and kodebarang='".$kodebarang."'
      ";
$adadata=false;
$res=mysql_query($str);
...
$str="select * from ".$dbname.".bgt_masterbarang where tahunbudget='".$tahunbudget."' and regional='".$regional."'
      ";
$tutupdata=0;
$res=mysql_query($str);
while($bar=mysql_fetch_object($res))
...
// UPDATE `owl`.`bgt_masterbarang` SET `hargasatuan` = '22',
//`hargalalu` = '20' WHERE `bgt_masterbarang`.`regional` = 'DKI' AND `bgt_masterbarang`.`tahunbudget` =2012 AND `bgt_masterbarang`.`kodebarang` = '31300001';
    if($adadata==true)$str="UPDATE ".$dbname.".bgt_masterbarang SET `hargasatuan` = '".$hargasatuan."', `variant` = '0',
