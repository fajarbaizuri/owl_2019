	...
	echo" <table class=sortable cellspacing=1 border=0>
	  ...
	  <td rowspan=2>".$_SESSION['lang']['jmlhBrg']."</td>
  	  <td rowspan=2>".$_SESSION['lang']['hargasatuan']."</td>
          <td colspan=2>".$_SESSION['lang']['pemenuhan']."</td>
          <td rowspan=2>".$_SESSION['lang']['kurs']."</td>
	  </tr>
          <tr><td>Loco</td><td>Franco</td></tr>
	  </thead><tbody>
	";
	 ...
	 class PDF extends FPDF
        {
            function Header() {
				...
				$this->Cell(16/100*$width,$height,$_SESSION['lang']['NoKontrak'],1,0,'C',1);		
		...
		if($row>0)
		{
			while($rDet=mysql_fetch_assoc($qDet))
			{
				...
				$pdf->Cell(16/100*$width,$height,$rDet['nokontrak'],1,0,'L',1);	
