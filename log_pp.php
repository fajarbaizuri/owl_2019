	...
	<table cellspacing="1" border="0" id="opl">
	 <tr>
		...
		<tr>
			...
			<button class=mybutton id="dtl_pem" onclick=cekno()><?php echo $_SESSION['lang']['save']?></button>			</td>
		</tr>
	</table>
</fieldset><br />
<br />
...
//$klq="select a.karyawanid,a.namakaryawan,a.lokasitugas,a.bagian from ".$dbname.".`datakaryawan` a where a.karyawanid!='".$_SESSION['standard']['userid']."' and a.kodejabatan in ('18','286','16','13','272','279','17','76','283') and (tanggalkeluar>=DATE_FORMAT(NOW(), '%Y-%m-%d') or tanggalkeluar='0000-00-00') order by a.namakaryawan asc";
//$klq="select a.karyawanid,a.namakaryawan,a.lokasitugas,a.bagian from ".$dbname.".`datakaryawan` a where  a.kodejabatan in ('17','13','76','272','283','34','282') and (tanggalkeluar>=DATE_FORMAT(NOW(), '%Y-%m-%d') or tanggalkeluar='0000-00-00') order by a.namakaryawan asc";
...
}elseif ($_SESSION['empl']['lokasitugas']=='FBAO'){
...
}elseif ($_SESSION['empl']['lokasitugas']=='CBGM'){
		$klq="select a.karyawanid,a.namakaryawan,a.lokasitugas,a.bagian from ".$dbname.".`datakaryawan` a where a.karyawanid!='".$_SESSION['standard']['userid']."' and a.kodejabatan in ('286') and (tanggalkeluar>=DATE_FORMAT(NOW(), '%Y-%m-%d') or tanggalkeluar='0000-00-00') order by a.namakaryawan asc";
}else{
		$klq="select a.karyawanid,a.namakaryawan,a.lokasitugas,a.bagian from ".$dbname.".`datakaryawan` a where a.karyawanid!='".$_SESSION['standard']['userid']."' and a.kodejabatan in ('18') and (tanggalkeluar>=DATE_FORMAT(NOW(), '%Y-%m-%d') or tanggalkeluar='0000-00-00') order by a.namakaryawan asc";
}
