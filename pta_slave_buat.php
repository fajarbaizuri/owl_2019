...
switch($method){
	...
	$sCek="select distinct notransaksi from ".$dbname.".pta_ht where notransaksi='".$pta."'";
        ...
        if($rCek<1)
        {
            $simpanht="INSERT INTO ".$dbname.".pta_ht(notransaksi,tanggal,kelompok,penjelasan)
     ...
     case'loaddata':
        ...
        while($row=mysql_fetch_assoc($qLoad))
        {
            ...
            $tab.= 
                "<tr class=rowcontent>
                    <td align=center><img src=images/delete1.jpg class=resicon  title='Delete' onclick=\"delData('".$row['notransaksi']."',
                    '".$row['jenispta']."','".$row['alokasibiaya']."','".$row['kodevhc']."','".$row['kodebarang']."','".$row['noakun']."');\" ></td>
     ...
     break;
     case'getData':
         $sCek="select distinct status1 from ".$dbname.".pta_ht where notransaksi='".$notransaksi."'";
         $qCek=mysql_query($sCek) or die(mysql_error($conn));
         $rCek=mysql_fetch_assoc($qCek);
         if($rCek['status1']!=0)
         {
             exit("Error:Persetujuan Sudah Terisi");
         }
        $sLoad="select * from ".$dbname.".pta_dt 
     ...
     case'daftarData':
		...
		while($rlvhc=mysql_fetch_assoc($qlvhc))
		{
               ...
               echo"<td align=center><img src=images/application/application_edit.png class=resicon  title='Print' onclick=\"editData('".$rlvhc['notransaksi']."')\"></td>";
...
//        exit('error:'.$sDel);
	...
	case'getKegiatan':
        ...
        $sKeg="select distinct kodekegiatan,namakegiatan from ".$dbname.".setup_kegiatan where noakun like '%".$_POST['noakun']."%' order by kodekegiatan";
