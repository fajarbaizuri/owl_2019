...
$strPO="select nopo from ".$dbname.".log_transaksiht where notransaksi='".$_POST['notransaksi']."' limit 1;";
$resPO=mysql_query($strPO);
$barPO=mysql_fetch_object($resPO);
$LINKPO=" PO:".$barPO->nopo;
}else{
$LINKPO="";
}
       ...
       #proses data
        ...
        # Prep Header
            ...
            $dataRes['detail'][] = array(
                ...
                'nourut'=>$noUrut,
                'noakun'=>$akunbarang,
                'keterangan'=>'Pembelian barang '.$namabarang.' '.$_POST['jumlah']." ".$_POST['satuan'].$LINKPO,
                'jumlah'=>$_POST['hartot'],
                'matauang'=>'IDR',
                'kurs'=>'1',
    ...
    if($akunbarang=='')
        exit("Error: Noakun barang belum ada untuk transaksi".$_POST['notransaksi']);
                    ...
                    # Prep Header
                        ...
                        # Data Detail
                        $noUrut = 1;
                         $keterangan="Retur Pemakaian barang ".$namabarang." ".$_POST['jumlah']." ".$_POST['satuan']." ".$_POST['keterangan'];
...
#ambil noakun barang
    ...
    if($akunbarang=='' or $akunspl=='')
     ...
     if($ptGudang !=$ptpengguna)
     {
        ...
        if($akunspl=='')
           exit("Error: Akun intraco  atau interco belum ada untuk unit ".substr($_POST['kodegudang'],0,4)); 
	    #ambil akun interco
         $strA="select akunhutang from ".$dbname.".keu_5caco where kodeorg='".$pengguna."' and jenis='inter'";
            $resA=mysql_query($strA);
            $interco='';
            while($barA=mysql_fetch_object($resA))
            {
                $interco=$barA->akunhutang;
            }
         $inter=$interco;   
        if($inter=='')
         ...
         $str="select akunhutang from ".$dbname.".keu_5caco where kodeorg='".substr($_POST['kodegudang'],0,4)."' and jenis='intra'";
            ...
            exit("Error: Akun intraco  atau interco belum ada untuk unit ".substr($_POST['kodegudang'],0,4));    
			 ...
			 $strA="select akunhutang from ".$dbname.".keu_5caco where kodeorg='".$pengguna."' and jenis='intra'";
            $resA=mysql_query($strA);
            $intraco='';
            while($barA=mysql_fetch_object($resA))
            {
                $intraco=$barA->akunhutang;
            } 
          $inter=$intraco;  
         if($inter=='')
            exit("Error: Akun intraco  atau interco belum ada untuk unit ".$pengguna);    
