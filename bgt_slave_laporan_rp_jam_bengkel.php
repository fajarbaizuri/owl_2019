...
//untuk exel detail
	...
	$data3.="<table cellspacing=1 border=1 class=sortable width=109%>
		...
		$no=0;
		$sList="select * from ".$dbname.".bgt_biaya_ws_per_jam a, ".$dbname.".bgt_budget b
		 where a.tahunbudget = b.tahunbudget and kodetraksi='".$_GET['kdTrak']."' and kodews='".$_GET['kdWs']."' and a.tahunbudget='".$_GET['thnbudget']."' ";
		//exit("Error.$sList");
		$qList=mysql_query($sList) or die(mysql_error());
		while($bar=mysql_fetch_object($qList))
		{
...
switch($proses)
{
	...
	<table cellspacing=1 border=0 class=sortable width=107%>
		...
		$no=0;
		$sList="select * from ".$dbname.".bgt_biaya_ws_per_jam a, ".$dbname.".bgt_budget b where a.tahunbudget = b.tahunbudget and 	kodetraksi='".$kdTrak."' and kodews='".$kdeWs."' and a.tahunbudget='".$thnbudget."' ";
		//exit("Error.$sList");
		$qList=mysql_query($sList) or die(mysql_error());
		while($bar=mysql_fetch_object($qList))
		{
