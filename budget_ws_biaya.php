...
//pilihan kodebudget tab1   2x perubahan 
#update indra
    /*$str="select kodebudget,nama from ".$dbname.".bgt_kode
...
/*$str="select kodebudget,nama from ".$dbname.".bgt_kode
        where kodebudget between 'M-331' and 'M-352'
        ";	*/		
	...
	$str="select kodebudget,nama from ".$dbname.".bgt_kode where kodebudget not in ('EXPL-LEMBUR','EXPL-UPAHBHL','EXPL-UPAHSKU','KONTRAK','M-311','M-312','M-313','M-381','SDM-BHL','SDM-KHT','SDM-KNT','SUPERVISI','PKSM') order by kodebudget asc";	
    $optmaterial1="";
    $res=mysql_query($str);
    while($bar=mysql_fetch_object($res))
    {
...
/*	$str="select kodebudget,nama from ".$dbname.".bgt_kode
	where kodebudget='TOOL' order by kodebudget asc";
    //$optmaterial1="";
    $res=mysql_query($str);
    while($bar=mysql_fetch_object($res))
    {
...
//pilihan kodebudget tab2   2x perubahan 
   /* $str="select kodebudget,nama from ".$dbname.".bgt_kode
        where kodebudget like 'TOOL%' ";*/
		
/*$str="select kodebudget,nama from ".$dbname.".bgt_kode
        where kodebudget='M-375'
        ";*/		
...
$frm[0].="<table cellspacing=1 border=0>
    ...
    <tr><td>".$_SESSION['lang']['jmlhPersonel']." </td><td>:</td><td>
        <input type=text class=myinputtext onkeyup=\"jumlahkan0();\" id=jumlahpersonel0 name=jumlahpersonel0 onkeypress=\"return angka_doang(event);\" maxlength=8 style=width:150px; /></td></tr>
...
$frm[3].="<table cellspacing=1 border=0><thead>
    ...
    <tr><td>".$_SESSION['lang']['totalbiaya']."</td><td>:</td><td>
        <input type=text class=myinputtext id=totalbiaya3 name=totalbiaya3 onkeypress=\"return angka_doang(event);\" maxlength=15 style=width:150px; /></td></tr>
    <tr><td colspan=3>
