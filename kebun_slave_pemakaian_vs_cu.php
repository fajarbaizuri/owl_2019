...
$str1="select notransaksi,left(kodeorg,4) as kodeorg,kodebarang,sum(kwantitas) as kwantitas,tanggal 
       ...
       group by left(kodeorg,4),kodebarang,tanggal order by tanggal";
//echo $str1;
$query1=mysql_query($str1);
while($res1=mysql_fetch_assoc($query1))
{
    $notrans[$res1['tanggal']]=$res1['notransaksi'];
...
$str2="select left(kodeblok,4) as kodeorg,kodebarang,sum(kwantitas) as kwantitas,tanggal 
       ...
       group by left(kodeblok,4),kodebarang,tanggal order by tanggal";
//echo $str2;
        ...
        foreach($kodebarang as $brng)
        {
            if($kwantitas1[$dtTgl][$brng]!=0||$kwantitas2[$dtTgl][$brng]!=0)
            {
                ...
                $stream.="<td align=center id=kdbrng>".$brng."</td> 
                            ...
                            <td align=right title=Click style=\"cursor:pointer;\" onclick=showAplikasi(event,".$brng.",'".$dtTgl."','".$kdorg."')>".number_format($kwantitas1[$dtTgl][$brng],2)."</td> 
                            <td align=right title=Click style=\"cursor:pointer;\" onclick=showCu(event,".$brng.",'".$dtTgl."','".$kdorg."')>".number_format($kwantitas2[$dtTgl][$brng],2)."</td>
...
switch($proses)
{
      ...
      case'getForm':
        ...
        $sAplikasi="select a.notransaksi,a.tanggal,a.kodeorg,a.kwantitas,b.hasilkerja
                   ...
                   where a.kodeorg like '%".$_GET['kdorg']."%' and a.kodebarang = ".$_GET['kdbrng']." and a.tanggal='".$_GET['tgl']."'";
//        echo $sAplikasi;
          ...
          echo"<fieldset>
           ...
           <table class=sortable cellspacing=1 cellpadding=2  border=0>
              <thead>
                ...
                WHERE kodebarang = ".$_GET['kdbrng']." and tanggal='".$_GET['tgl']."' and kodeblok like '%".$_GET['kdorg']."%' ";
//                echo $sCu;
//                exit('error:'.$sCu);
