...
switch($proses) {
    ...
    case 'showHeadList':
		...
		$query = selectQuery($dbname,'pabrik_ops_gensetht',$cols,"tanggal between '".
			$tanggal1."' and '".$tanggal2."' and ".$where,$order,false,10,1);
		$data = fetchData($query);
		$totalRow = getTotalRow($dbname,'pabrik_ops_gensetht');
		$listKary = '';
    ...
    # Form Edit Header
    case 'showEdit':
		$query = selectQuery($dbname,'pabrik_ops_gensetht',"*",
    ...
    case 'add':
		...
		$query = insertQuery($dbname,'pabrik_ops_gensetht',$data,$cols);
    ...
    case 'edit':
		...
		$query = updateQuery($dbname,'pabrik_ops_gensetht',$data,$where);
		if(!mysql_query($query)) {
			echo "DB Error : ".mysql_error();
		}
		break;
    case 'delete':
		...
		$query = "delete from `".$dbname."`.`pabrik_ops_gensetht` where ".$where;
		if(!mysql_query($query)) {
			echo "DB Error : ".mysql_error();
			exit;
		}
