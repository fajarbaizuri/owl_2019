...
switch($level) {
   ...
   $query1 = "select b.jumlah, b.tipetransaksi,b.keterangan2,b.notransaksi,b.tanggal from ".$dbname.".keu_kasbankdt b 
              left join ".$dbname.".keu_kasbankht a on b.notransaksi=a.notransaksi
              where b.noakun = '".$param['noakun']."' and b.kodeorg='".$kodeorg."' and
	...
	if($mode!='excel') {
	    ...
	    $saldoKM = number_format($saldoKM,0);
...
switch($mode) {
	...
	# Saldo
	$pdf->SetFont('Arial','B',9);
	$pdf->Cell($lenJudul/100*$width,$height,'SALDO','LR',0,'C',1);
	...
	$pdf->Ln();
	# Jumlah
	$pdf->Cell($lenJudul/100*$width,$height,'J u m l a h','LR',0,'C',1);
	$pdf->SetFont('Arial','',9);
	...
	# Mengetahui dll
	$pdf->Cell(1/3*$width,$height,'Mengetahui,','L',0,'C',0);
	$pdf->Cell(1/3*$width,$height,'Diperiksa Oleh,','',0,'C',0);
	$pdf->Cell(1/3*$width,$height,'Yang Membuat,','R',0,'C',0);
	$pdf->Ln();
	...
	# Saldo
	$tab .= "<tr class='rowcontent'>";
        $tab .= "<td colspan='4' align='center'>Saldo</td>";
