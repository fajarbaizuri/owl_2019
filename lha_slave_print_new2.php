...
#---------------------------------------------------------------
$strA="SELECT  IF(B.kodekegiatan=0,'611010101',B.kodekegiatan) as kodekegiatan,left(B.kodeorg,6) as afd FROM ".$dbname.".`kebun_aktifitas` A LEFT JOIN ".$dbname.".`kebun_prestasi` B 
ON A.notransaksi=B.notransaksi where A.kodeorg = '".$kdOrg."' and A.jurnal='1' and A.tanggal >='".$tgl1."' and A.tanggal <='".$tgl2."' and A.tipetransaksi='PNN' and B.kodeorg like '%".$kdAfd."%' and B.kodekegiatan like '%%' group by B.kodekegiatan,left(B.kodeorg,6) order by left(B.kodeorg,6),B.kodekegiatan asc;";
...
{     
 ...
 //echo $strA;
#---------------------------------------------------------------
#Ambil Prestasi Kegiatan Panen
#---------------------------------------------------------------
$str1="SELECT count(A.notransaksi) as rotasi, IF(A.kodekegiatan=0,'611010101',A.kodekegiatan) as kodekegiatan,left(A.blok,6) as afd,A.blok as kodeorg,sum(A.hasilkg) as kerja,sum(A.jumhk) as hk,sum(A.jumhk * A.proumr) as bumr,sum(A.jumhk * A.propre)as bpremi,sum(A.jumhk * A.prodenda)as bdenda 
FROM ".$dbname.".`kebun_kehadiran_proporsi_pnn` A  where A.kodeorg = '".$kdOrg."' and A.tanggal >='".$tgl1."' and A.tanggal <='".$tgl2."' and A.blok like '%".$kdAfd."%' and A.kodekegiatan like '%%' group by A.kodekegiatan,A.blok  order by A.blok asc;";
...
{     
	...
	$TAMPUNG[$bar1->kodekegiatan][$bar1->afd][$bar1->kodeorg]['bdenda']=$bar1->bdenda;
}
//echo $str1;
#---------------------------------------------------------------
...
".$dbname.".`kebun_kehadiran_proporsi_pnn` A LEFT JOIN ".$dbname.".`kebun_pakaimaterial` B  ON A.notransaksi=B.notransaksi 
    ...
    if($proses=='getAfdAll'){
		...
		if(empty($START)){
			...
			else
			{
				...
				$data="<button onclick=\"zExcel(event,'lha_slave_print_new2.php','".$arr."')\" class=\"mybutton\" name=\"excel\" id=\"excel\">Excel</button>
                  ...
                  <thead >";
					...
					*/
					$data.="
					<td colspan='5' style='min-width:310px'><span style='max-width:310px;display:block;'>Tenaga Kerja</span></td>";
					/*
					...
					*/
					...
					<td style='min-width:80px'><span style='max-width:80px;display:block;'>Denda(Rp.)</span></td>
						...
						foreach($TAMPUNG[$key][$key2] as $key3 =>$baris3)
						{
							...
							if (count($BAHANVL[$key][$key2][$key3])==0){
								...
								*/
								...
								//Denda(Rp.)
								$TAMPIL[$key][$key2][$key3]['00000000']['I']=$baris3['bdenda'];
								//Biaya TK(Rp.)
								$TAMPIL[$key][$key2][$key3]['00000000']['J']=($baris3['bumr']+$baris3['bpremi'])-$baris3['bdenda'];
								/*
								//Nama Bahan
								...
								//Total Biaya (Rp)
								$TAMPIL[$key][$key2][$key3]['00000000']['O']=($baris3['bumr']+$baris3['bpremi'])-$baris3['bdenda'];
								*/
								//Hasil Kerja
								$TAMPIL[$key][$key2][$key3]['00000000']['P']=$baris3['hasilkerja'];
								//Rp/Satuan
								$TAMPIL[$key][$key2][$key3]['00000000']['Q']=(($baris3['bumr']+$baris3['bpremi'])-$baris3['bdenda'])/$baris3['hasilkerja'];
								//HK/Satuan
								...
								*/
								...
								$TAMPIL[$key][$key2][$key3]['00000000']['I']=$baris3['bdenda'];
								//Biaya TK(Rp.)
								$TAMPIL[$key][$key2][$key3][$key4]['J']=($baris3['bumr']+$baris3['bpremi'])-$baris3['bdenda'];
								/*
								//Nama Bahan
								...
								//Total Biaya (Rp)
								$TAMPIL[$key][$key2][$key3][$key4]['O']=(($baris3['bumr']+$baris3['bpremi'])-$baris3['bdenda'])+$baris4['biayabrg'];
								*/
								//Hasil Kerja
								$TAMPIL[$key][$key2][$key3][$key4]['P']=$baris3['hasilkerja'];
								//Rp/Satuan
								$TAMPIL[$key][$key2][$key3][$key4]['Q']=((($baris3['bumr']+$baris3['bpremi'])-$baris3['bdenda'])+$baris4['biayabrg'])/$baris3['hasilkerja'];
								//HK/Satuan
			...
			asort($TAMPIL);
						...
						foreach($baris2 as $key3 =>$baris3)
						{
							...
							$data.="<tr  class=rowcontent>
								...
								*/
								...
								$B_DENDA=$TAMPUNG[$key][$key2][$key3]['bdenda'];
								$B_TK=($B_UMR+$B_PREMI)-$B_DENDA;
								
								$TB_HK=$TB_HK+$B_HK;
								$TB_UMR=$TB_UMR+$B_UMR;
								$TB_PREMI=$TB_PREMI+$B_PREMI;
								$TB_DENDA=$TB_DENDA+$B_DENDA;
								$TB_TK=$TB_TK+$B_TK;
							$data.="	
								...
								<td rowspan='".Count($baris3)."' align='right'>".number_format($B_DENDA)."</td>
				...
				$data.="
                  <thead >
					...
					<td align='right' style='min-width:80px'><span style='text-align:right;max-width:80px;display:block;'>".number_format($TB_DENDA)."</span></td>
