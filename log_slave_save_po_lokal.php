<?php
	...
	switch($proses)
	{
			...
			if(($jmlh_pesan=='')||($hrg_satuan==''))
			{
                        ...
                        elseif($resjmlh['realisasi']<$jmlh_pesan)
				{
					...
					exit();
                        ...
                        if($res<1)
                        {
			$strx="insert into ".$dbname.".log_poht (`nopo`,`tanggal`,`kodesupplier`,`subtotal`,`diskonpersen`,`nilaidiskon`,`ppn`,`nilaipo`,`tanggalkirim`,`lokasipengiriman`,`syaratbayar`,`uraian`,`purchaser`,`kodeorg`,`lokalpusat`,`tgledit`) 
                            values ('".$nopo."','".$tgl_po."','".$supplier_id."','".$sub_total."','".$disc."','".$nilai_dis."','".$nppn."','".$nilai_po."','".$tanggl_kirim."','".$lokasi_kirim."','".$cr_pembayaran."','".$ketUraian."','".$_SESSION['standard']['userid']."','".$rKdorg['kodeorg']."','1','".$tgl_po."')";
	 ...
	 <tbody>";
                ...
                while ($res = mysql_fetch_object($query)) {
					...
					else
					{
                                         ...
                                         if(($res->stat_release==0)||is_null($res->stat_release))
					 {
						...
						$release_po_button="<img src=images/application/application_key.png class=resicon onclick=release_po('".$no."') title='Pilih Penandatangan' />";
					 }           
		...
		if($rcek['statuspo']==1)
		{
			echo"warning : No. PO : ".$nopo." Sedang Dalam Proses Verifikasi";
			exit();
		}
		else    $tglSkrng=date("Y-m-d");
		{
			...
			foreach($_POST['kdbrg'] as $row =>$isi)
			{
				...
				{
					echo "warning : Jumlah Pesan (".$jmlh_pesan.") Tidak Boleh Lebih Besar Dari Jumlah Realisasi (".$resjmlh['realisasi'].")";
					exit();
				}	
			}
		}
...
//echo "warning:masuk '===='";exit();
		...
		$rest=mysql_fetch_assoc($query);
					...
					echo"
						...
						<button class=mybutton onclick=proses_release_po() title=\"Pilih Penandatangan\" >".$_SESSION['lang']['tandatangan']."</button>
