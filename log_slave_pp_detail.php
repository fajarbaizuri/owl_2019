    ...
    switch($_POST['proses']) {
        case 'detail_add' :
            ...
            if(($data['kd_brg']=='') or ($data['jmlhDiminta']=='') or ($tglsdt<$rtglpp) or $days<7) {
                echo "Error : Data Tidak Boleh Kosong Dan Tanggal Tidak Boleh Lebih Kecil Dari Tanggal PP, Min 7 Hari Dari Tanggal PP";
                exit;
            }
...
//    $table .= "<td>"."<a href=# onclick=addNewRow(detailBody,true)><img src='images\newfile.png'></a>"."</td>";
    ...
    if($data!=array()) {
		...
		*/	
        ...
        array('','onkeypress'=>'return tanpa_kutip(event)','onmousemove'=>'setCalendar(this.id)','readonly'=>'readonly','class=myinputtext'))."</td>";
	...
	$table .= "<td>".makeElement("tgl_sdt_".$i."",'txt','',array('','onkeypress'=>'return tanpa_kutip(event)','onmousemove'=>'setCalendar(this.id)','readonly'=>'readonly','class=myinputtext'))."</td>";
