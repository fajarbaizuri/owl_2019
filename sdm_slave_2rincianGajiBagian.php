...
//        $qDataGapok="select sum(jumlah) as gapok,karyawanid from ".$dbname.".sdm_5gajipokok 
//            where idkomponen in (select id from ".$dbname.".sdm_ho_component WHERE `plus` =1) group by karyawanid";
        ...
        $qLembur="select sum(uangkelebihanjam) as lembur,karyawanid from ".$dbname.".sdm_lemburdt where substr(kodeorg,1,4)='".substr($kdOrg, 0,4)."' 
...
switch($proses)
{
	...
	if(($tgl2=="")&&($tgl1==""))
	{
		echo"warning: Periode Gaji tidak boleh kosong";
		exit();
	}
	...
	if($jmlHari>31)
	{
		echo"warning:Range Periode Gaji tidak valid";
		exit();
	}
...
//               echo"</pre>";//exit();
	...
	if(($tgl2=="")&&($tgl1==""))
	{
		echo"warning: Periode Gaji tidak valid";
		exit();
	}
...
//                    $this->Cell(12/100*$width,$height,"Jamsostek",1,1,'C',1);
			if($afdC==false) {
				//$pdf->Cell(3/100*$width,$height,$nomor,1,0,'C',1);
	...
	$tab.="<table border=0 cellspacing=1>
            ...
            <tr><td align=left colspan=2>".$_SESSION['lang']['unit']." :</td><td align=left colspan=4>".$kodeOrg."</td></tr>
            </table>";
