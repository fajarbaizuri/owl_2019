...
{	
	...
	<tbody>";
		...
		$slvhc="select * from ".$dbname.".vhc_penggantianht where kodeorg in (".substr($kodeOrg,0,$pnjgn).")  order by `notransaksi` desc limit ".$offset.",".$limit."";
		...
		if($rCek['kodejabatan']==120 || $rCek['kodejabatan']==94)
		{
		echo"<td>
	...
	<tbody>";
		...
		else
		{
			...
			if($txt_search!='')
			{
				$where="and notransaksi LIKE  '%".$txt_search."%' order by notransaksi desc";
			}
			elseif($txt_tgl!='')
			{
				$where="and tanggal LIKE '%".$txt_tgl."%' order by notransaksi desc";
			}
			elseif(($txt_tgl!='')&&($txt_search!=''))
			{
				$where="and notransaksi LIKE '%".$txt_search."%' and tanggal LIKE '%".$txt_tgl."%' order by notransaksi desc";
			}
		...
		{
			$strx="select * from ".$dbname.".vhc_penggantianht where kodeorg='".$lokasi."' ".$where." order by notransaksi desc ";
		}
		else
		{
...
*/		
		...
		$sql2="select count(*) as jmlhrow from ".$dbname.".vhc_penggantianht where kodeorg in (".substr($kodeOrg,0,$pnjgn).") ".$where."";
		$query2=mysql_query($sql2) or die(mysql_error());
		while($jsl=mysql_fetch_object($query2)){
		$jlhbrs= $jsl->jmlhrow;
		}

		$strx="select * from ".$dbname.".vhc_penggantianht where  kodeorg in (".substr($kodeOrg,0,$pnjgn).")  ".$where." limit ".$offset.",".$limit."";
