...
//                echo "warning: (".$nopo.">>".$gudang.")"; exit;    
  ...
  //cek pemilik PO
		...
		$str="select kodeorg from ".$dbname.".log_poht where nopo = '".$nopo."'";
		$res=mysql_query($str);
		while($bar=mysql_fetch_object($res))$ptPO=$bar->kodeorg;
                
//                echo "warning: (".$ptgudang.">>".$ptPO.")";
                if(($ptgudang!=$ptPO)and($ptgudang!='')){ echo "warning: PO milik PT lain (gudang:".$ptgudang." << PO:".$ptPO.")"; $exit = exit; }
...
function createForm($nopo,$notransaksi='')
{
	...
	echo"<table class=sortable cellspacing=1 border=0>
		 ...
		 <tr class=rowheader>
		   ...
		   <td>".$_SESSION['lang']['belumterima']."</td>
