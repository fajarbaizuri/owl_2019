...
$aresta="SELECT noakun, namaakun FROM ".$dbname.".keu_5akun
    WHERE length(noakun)=7 and (left(noakun,5) between '12201' and '12204')
    ORDER BY noakun";
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$str="SELECT noakun, setahun FROM ".$dbname.".bgt_summary_biaya_vw 
    WHERE tahunbudget = '".$tahun."' and unit = '".$unit."' and (left(noakun,5) between '12201' and '12204')";
...
$str="SELECT a.*, b.noakun FROM ".$dbname.".bgt_lbm_volume_kebun_vw a
    ...
    WHERE a.tahunbudget = '".$tahun."' and a.kebun = '".$unit."' and (left(b.noakun,5) between '12201' and '12204')";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
    WHERE tanggal like '".$periode."%' and nojurnal like '%".$unit."%' and (left(noakun,5) between '12201' and '12204')
    GROUP BY noakun";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    ...
    WHERE a.tanggal like '".$periode."%' and a.unit = '".$unit."' and (left(b.noakun,5) between '12201' and '12204')
...
$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
    WHERE (tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and nojurnal like '%".$unit."%' and (left(noakun,5) between '12201' and '12204')
    GROUP BY noakun";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    WHERE (a.tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and a.unit = '".$unit."' and (left(b.noakun,5) between '12201' and '12204')
    GROUP BY a.kodekegiatan";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$tab.="<table border=0>
     <tr>
        <td colspan=8 align=left><font size=3>21. BIAYA PEMBUKAAN LAHAN (TB)</font></td>
...
if($proses!='excel')$tab.=$judul;
    ...
    <tr>
    <td align=right colspan=3 ".$bg.">Luas Areal TB:</td>
...
switch($proses)
{
    ...
    function Header() {
        ...
        $this->Cell($width/2,$height,'19. BIAYA PEMBUKAAN LAHAN (TB)',NULL,0,'L',1);
        ...
        $this->Cell((3/100*$width)+(($wlain+$wkiri)/100*$width),$height,'Luas Areal TB:',0,0,'R',1);	
