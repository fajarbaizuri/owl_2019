...
switch($method) {
    ...
    if ($_POST['tipekategori']=='1'){
		$sqlB ="select jenisvhc as kode,namajenisvhc as nama from `".$dbname."`.`vhc_5jenisvhc`   where  jenisvhc like 'AB%' ";
	}else{
		$sqlB ="select kodevhc as kode,nopol as nama from `".$dbname."`.`vhc_5master` where kodeorg='".$_SESSION['empl']['lokasitugas']."' and jenisvhc like 'AB%' ";
	}
	...
	case 'insert':
		...
		$sRicek="select * from ".$dbname.".vhc_kendaraan_premi where `kategori`='1' and `kodeorg`='".$kdOrg."' and `kegiatan`='".$kdKeg."' and `kendaraan`='".$kdKen."'  and  `operator`='".$kdpremi."' ";
		$qRicek=mysql_query($sRicek) or die(mysql_error($conn));
		$rRicek=mysql_num_rows($qRicek);
		if($rRicek>0) {
			$sDel="delete from ".$dbname.".vhc_kendaraan_premi
					where `kategori`='1' and `kodeorg`='".$kdOrg."' and `kegiatan`='".$kdKeg."' and `kendaraan`='".$kdKen."'  and  `operator`='".$kdpremi."' ";	    
			if(mysql_query($sDel)) {
				$sDel2="insert into ".$dbname.".vhc_kendaraan_premi ( `kategori`, `kodeorg`,`kegiatan`, `kendaraan`, `operator`, `satuan`, `insentif_1`, `insentif_2`, `b_trep_1`, `lb_trep_1`, `b_trep_2`, `lb_trep_2`, `b_trep_3`, `lb_trep_3`, `b_trep_4`, `lb_trep_4`,`jenis`)
					values ('1','".$kdOrg."','".$kdKeg."','".$kdKen."','".$kdpremi."','".$satuan."','".$uains1."','".$uains2."','".$uadb."','".$ualb."', 0, 0, 0, 0, 0, 0,'".$kdTipe."')";
