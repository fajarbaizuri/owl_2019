...
// cari kegiatan
$kegiatan="SELECT kodekegiatan, namakegiatan FROM ".$dbname.".setup_kegiatan WHERE ".$kodekode." order by kodekegiatan";
...
$kegiatan="SELECT kodekegiatan, namakegiatan FROM ".$dbname.".setup_kegiatan WHERE ".$kodekode2." order by kodekegiatan";
$query=mysql_query($kegiatan) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
    $dzArr[$res['kodekegiatan']][kode]=$res['kodekegiatan'];
...
if($proses!='excel')$tab.="<br>12. 1. EFISIENSI PEMELIHARAAN TM (HERBISIDA)"; else{
    $tab.="<br><table border=0>
         <tr><td colspan=11 align=left><font size=3>12. 1. EFISIENSI PEMELIHARAAN TM (HERBISIDA)</font></td></tr> 
    </table>";
}
...
if($proses!='excel')$tab.="<br>12. 2. EFISIENSI PEMELIHARAAN TBM (HERBISIDA)"; else{
    $tab.="<br><table border=0>
         <tr><td colspan=11 align=left><font size=3>12. 2. EFISIENSI PEMELIHARAAN TBM (HERBISIDA)</font></td></tr> 
    </table>";
}
    $tab.="<table cellpadding=1 cellspacing=1 border=".$brdr." class=sortable style='width:100%;'>
    <thead class=rowheader>
...
switch($proses)
{
    ...
    function Header() {
        ...
        $this->Cell($width/2,$height,'12. EFISIENSI PEMELIHARAAN TANAMAN ',NULL,0,'L',1);
        ...
        $this->Cell($width,$height,'12.1 EFISIENSI PEMELIHARAAN TM (HERBISIDA)',0,0,'L',1);	
        $this->Ln();
...
// pdf array content =========================================================================
    ...
    $pdf->Cell($width,$height,'12.2 EFISIENSI PEMELIHARAAN TBM (HERBISIDA)',0,0,'L',1);	
    $pdf->Ln();
