...
$kdPabrik=$_POST['kdPabrik__4'];
$kdUnit=$_POST['kdUnit__4'];
$periode=$_POST['periode__4'];
...
switch($proses)
{
    ...
    if($kdUnit==''){ // kode unit = seluruhnya, kolom menampilkan unit
		...
		$qweri="select a.tanggal, a.kodecustomer,b.namasupplier,a.jumlahtandan1,a.kgpotsortasi, (a.beratbersih-a.kgpotsortasi) as beratbersih from ".$dbname.".pabrik_timbangan a LEFT JOIN ".$dbname.".log_5supplier b ON a.kodecustomer=b.kodetimbangan where a.tanggal like '".$periode."%' AND a.kodecustomer like '%".$kdUnit."%' and a.kodeorg='' and a.millcode like '%".$kdPabrik."%' and a.kodebarang='40000003' order by a.kodecustomer";
        ...
        if($barisnya>0)
        {
            while($isi=mysql_fetch_assoc($datanya))
            {
                ...
                $isinya[$isi['kodecustomer']][$tanggal]+=$isi['beratbersih'];
                $kolom[$isi['kodecustomer']]=$isi['namasupplier'];
            }
        }
		//---------------------------------
