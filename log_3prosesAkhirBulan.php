...
while($bar=mysql_fetch_object($res))
{
 ...
 $strA="select periode,DATE_FORMAT(tanggalmulai, '%Y-%m') as newperiode from ".$dbname.".setup_periodeakuntansi where kodeorg like '".$_SESSION['empl']['lokasitugas']."' and tutupbuku =0 order by periode desc limit 1";
$resA=mysql_query($strA);
$optper="<option value=''></option>";
while($barA=mysql_fetch_object($resA))
{
	$optper.="<option value='".$barA->periode."'>".$barA->newperiode."</option>";
}
/*
$optper='';
for($x=0;$x<15;$x++)
