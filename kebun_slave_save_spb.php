...
$noSpb=isset($_POST['noSpb'])?$_POST['noSpb']:'';
$tanggal=isset($_POST['tgl'])?tanggalsystem($_POST['tgl']):'';
$bjrHsl=isset($_POST['bjr'])?$_POST['bjr']:'';
$jjngHsl=isset($_POST['jjng'])?intval($_POST['jjng']):'';
$brondolanHsl=isset($_POST['brondolan'])?intval($_POST['brondolan']):'';
$user_online=$_SESSION['standard']['userid'];
$kdOrg=isset($_POST['kdOrg'])?$_POST['kdOrg']:'';
$idDiv=isset($_POST['idDiv'])?$_POST['idDiv']:'';
$matang=isset($_POST['matang'])?$_POST['matang']:'';
$mentah=isset($_POST['mentah'])?$_POST['mentah']:'';
$busuk=isset($_POST['busuk'])?$_POST['busuk']:'';
$lwtmatang=isset($_POST['lwtmatang'])?$_POST['lwtmatang']:'';
$kdOrg=isset($_POST['kdOrg'])?$_POST['kdOrg']:'';
...
//print_r($_POST);
        switch($proses)
        {
                ...
                case'amblBjr':
                        $sBlok = selectQuery($dbname,'setup_blok','tahuntanam',"kodeorg='".$blok."'");
                        $resBlok = fetchData($sBlok);
                        $sStpBlok="select bjr from ".$dbname.".kebun_5bjr where kodeorg='".
                                substr($blok,0,6)."' and thntanam = ".$resBlok[0]['tahuntanam']." and tahunproduksi=".substr($_POST['periode'],0,4);
                        //echo $sStpBlok;
...
*/		
                ...
                if($rCek<1)
                {
                        $sIns="insert into ".$dbname.".kebun_spbht (`nospb`, `kodeorg`, `tanggal`,`nopolisi`,`operator`,`updateby`) values ('".$noSpb."','".$kdOrg."','".$tanggal."','".$nopol."','".$op."','".$user_online."')"; //echo"warning:".$sIns;exit();
                        //exit("Error:$sIns");
                        if(mysql_query($sIns))
                        {
                ...
                $periode=$tglThn."-".$tglBln;
                        ...
                        '".tanggalnormal($rlvhc['tanggal'])."','1','".$periode."');\">
                ...
                {
                         $slvhc="select * from ".$dbname.".kebun_spbht where  kodeorg='".$_SESSION['empl']['lokasitugas']."' ".$where." order by nospb desc limit ".$offset.",".$limit."";
                        $ql2="select count(*) jmlhrow from ".$dbname.".kebun_spbht where kodeorg='".$_SESSION['empl']['lokasitugas']."' ".$where." order by nospb desc";			 
                }
                else
                {
                        $slvhc="select * from ".$dbname.".kebun_spbht where kodeorg='".$_SESSION['empl']['lokasitugas']."' ".$where." order by nospb desc limit ".$offset.",".$limit."";
                ...
                if($rCek>0)
                {
                        if(($data['jjng']=='') or ($data['brondolan']=='') or ($data['bjr']=='') ) {
                                ...
                                {
                                                        $sUpDetail="update ".$dbname.".kebun_spbdt set blok='".$blok."',jjg='".$jjngHsl."',bjr='".$bjrHsl."',brondolan='".$brondolanHsl."',mentah='".$mentah."',busuk='".$busuk."',matang='".$matang."',lewatmatang='".$lwtmatang."',kgbjr='".$kgBjr."' where nospb='".$noSpb."' and blok='".$oldBlok."'";
                ...
                break;
				 case'getPlatData':
				 $kdOpr=isset($_POST['kdOpr'])?$_POST['kdOpr']:'';
                //echo"warning:masuk";
                if($kdOpr=='0')
                {
					echo "";
				...
				$sORg="select a.*,b.nopol,c.namakaryawan from ".$dbname.".vhc_5operator a left join ".$dbname.".vhc_5master b on a.vhc=b.kodevhc 
left join ".$dbname.".datakaryawan c ON a.karyawanid=c.karyawanid where c.lokasitugas='".$kdOrg."' and a.aktif=1 and a.karyawanid='".$kdOpr."'";
