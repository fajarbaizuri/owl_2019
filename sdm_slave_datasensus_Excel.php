...
{
$str="select a.*,b.namajabatan,c.namagolongan,d.tipe,e.kelompok 
      from ".$dbname.".datasensus a left join
      ".$dbname.".sdm_5jabatan b on a.kodejabatan=b.kodejabatan left join
	  ".$dbname.".sdm_5golongan c on  a.kodegolongan=c.kodegolongan left join
	  ".$dbname.".sdm_5tipekaryawan d on  d.id=a.tipekaryawan left join
	  ".$dbname.".sdm_5pendidikan e on a.levelpendidikan=e.levelpendidikan
	  where 1=1 ".$where;    
	  
$strd="select b.*,a.namakaryawan,c.kelompok, case b.status when 1 then 'Y' when 0 then 'T' end as statusx
       from ".$dbname.".sdm_sensuskeluarga b
       left join ".$dbname.".datasensus a
