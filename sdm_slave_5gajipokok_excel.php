...
switch ($method)
{
       ...
       case 'dataDetail':
        ...
        where  idkomponen in (31,30,2,1) and tahun='".$thn."'";
         //  exit("Error:".$sJumlah);
	 ...
	 <tbody>";
        ...
        $sql="select distinct a.karyawanid,b.namakaryawan,b.tanggalmasuk,idkomponen,bagian,COALESCE(ROUND(DATEDIFF('".$thnDatalalu."',b.tanggalmasuk)/365,2),0) as masakerja,tipekaryawan from ".$dbname.".sdm_5gajipokok a left join ".$dbname.".datakaryawan 
            b on a.karyawanid=b.karyawanid where tahun='".$thn."' and idkomponen in (31,30,2,1) and lokasitugas='".$_SESSION['empl']['lokasitugas']."' order by idkomponen";
        //exit("Error:".$sql);
	...
	$res=mysql_query($sql);
		while($bar=mysql_fetch_assoc($res))
		{
                    ...
                    $stream.="<td>".number_format($barGapok[1][$bar['karyawanid']],0)."</td>";
