...
switch($proses) {
    ...
    case 'showHeadList':
		...
		$query = selectQuery($dbname,'pabrik_ops_klarifikasiht',$cols,"tanggal between '".
			$tanggal1."' and '".$tanggal2."' and ".$where,$order,false,10,1);
		$data = fetchData($query);
		$totalRow = getTotalRow($dbname,'pabrik_ops_klarifikasiht');
		$listKary = '';
    ...
    # Form Edit Header
    case 'showEdit':
		$query = selectQuery($dbname,'pabrik_ops_klarifikasiht',"*",
    ...
    case 'add':
		...
		$query = insertQuery($dbname,'pabrik_ops_klarifikasiht',$data,$cols);
    ...
    case 'edit':
		...
		$query = updateQuery($dbname,'pabrik_ops_klarifikasiht',$data,$where);
		if(!mysql_query($query)) {
			echo "DB Error : ".mysql_error();
		}
		break;
    case 'delete':
		...
		$query = "delete from `".$dbname."`.`pabrik_ops_klarifikasiht` where ".$where;
		if(!mysql_query($query)) {
			echo "DB Error : ".mysql_error();
			exit;
		}
