...
$aresta="SELECT noakun, namaakun FROM ".$dbname.".keu_5akun
    WHERE ((length(noakun)=7)or(length(noakun)=5)) and noakun like '621%'
    ORDER BY noakun";
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
//  WHERE tahunbudget = '".$tahun."' and unit = '".$unit."' and noakun like '621%'";
if ($afdId==$unit){
	$str="SELECT noakun, setahun FROM ".$dbname.".bgt_summary_biaya_vw 
	WHERE tahunbudget = '".$tahun."' and unit = '".$unit."' and noakun like '621%'";
}else{
	$str="SELECT noakun, setahun FROM ".$dbname.".bgt_summary_biaya_afd_vw  
	WHERE tahunbudget = '".$tahun."' and  unit like '".$afdId."%' and noakun like '621%'"; 
}
...
//$str="SELECT a.*, b.noakun FROM ".$dbname.".bgt_lbm_volume_kebun_vw a
  //  LEFT JOIN ".$dbname.".setup_kegiatan b on a.kegiatan=b.kodekegiatan
    //WHERE a.tahunbudget = '".$tahun."' and a.kebun = '".$unit."' and b.noakun like '621%'";
...
if ($afdId==$unit){
    ...
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kegiatan=b.kodekegiatan
    WHERE a.tahunbudget = '".$tahun."' and a.afdeling like '".$afdId."%' and b.noakun like '621%'";	
}
...
//$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
  //  WHERE tanggal like '".$periode."%' and nojurnal like '%".$unit."%' and noakun like '621%'
    //GROUP BY noakun";

$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
  WHERE tanggal like '".$periode."%' and kodeblok like '".$afdId."%' and noakun like '621%'
GROUP BY noakun";	
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
//$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
  //  LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
   // WHERE a.tanggal like '".$periode."%' and a.unit = '".$unit."' and b.noakun like '621%'
    //GROUP BY a.kodekegiatan";

$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    WHERE a.tanggal like '".$periode."%' and a.kodeorg like '".$afdId."%' and b.noakun like '621%'
    GROUP BY a.kodekegiatan";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
//$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
//    WHERE (tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and nojurnal like '%".$unit."%' and noakun //like '621%'
//    GROUP BY noakun";
	
$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
    WHERE (tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and kodeblok like '".$afdId."%' and noakun like '621%'
    GROUP BY noakun";
...
//$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
 //   LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
  //  WHERE (a.tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and a.unit = '".$unit."' and b.noakun like //'621%'
   // GROUP BY a.kodekegiatan";
	
$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    WHERE (a.tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and a.kodeorg = '".$afdId."' and b.noakun like '621%'
    GROUP BY a.kodekegiatan";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$tab.="<table border=0>
     <tr>
        <td colspan=8 align=left><font size=3>18. BIAYA PEMELIHARAAN TM</font></td>
...
if($proses!='excel')$tab.=$judul;
    ...
    <tr>
    <td align=right colspan=3 ".$bg.">Luas Areal TM:</td>
...
switch($proses)
{
    ...
    $dte=date("YmdHis");
    $nop_="lbm_biayapemeliharan_tm_".$afdId.$periode;
    if(strlen($tab)>0)
    {
    ...
    function Header() {
        ...
        $this->Cell($width/2,$height,'18. BIAYA PEMELIHARAAN TM',NULL,0,'L',1);
        ...
        $this->Cell((3/100*$width)+(($wlain+$wkiri)/100*$width),$height,'Luas Areal TM:',0,0,'R',1);	
