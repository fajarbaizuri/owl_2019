...
$str="select *,
	  ...
	  from ".$dbname.".datasensus where karyawanid=".$karyawanid ." limit 1";
$res=mysql_query($str);
...
//=======================Riwayat Pekerjaan
	...
	//$pdf->Cell(
//loop isinya
	 $str="select * from ".$dbname.".sdm_sensuscv where karyawanid=".$karyawanid." order by bulanmasuk desc";
	 $res=mysql_query($str);
	 $no=0;
	 $mskerja=0;
	 while($bar=mysql_fetch_object($res))
...
//loop isinya
	 $str="select *,case sertifikat when 0 then 'N' else 'Y' end as bersertifikat 
	       from ".$dbname.".sdm_sensustraining
...
//loop isinya
		 $str="select *,case aktif when 1 then 'Yes' when 0 then 'No' end as status from ".$dbname.".sdm_sensusalamat where karyawanid=".$karyawanid." order by nomor desc";
		 $res=mysql_query($str);
		 $no=0;
		 while($bar=mysql_fetch_object($res))
		 {
