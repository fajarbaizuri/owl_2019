...
if($regDt!='')
{
    $sUnit="select distinct kodeunit from ".$dbname.".bgt_regional_assignment where regional='".$regDt."'";    
}
else
{
...
$sTot="select distinct sum(hargasatuan*jumlahpesan) as total,matauang from 
       ".$dbname.".log_po_vw where substr(kodebarang,1,1)='9' 
...
{
    if($rTot['matauang']!='IDR')
    {
        $rTot['total']=$rTot['total']*$krsDt[$rTot['matauang']];
        $krs[$rTot['matauang']]=$krsDt[$rTot['matauang']];
    }
    $totKapi+=$rTot['total'];
...
$sTot="select distinct sum(hargasatuan*jumlahpesan) as total,matauang from 
       ".$dbname.".log_po_vw where substr(kodebarang,1,1)='9'  ".$whr."
