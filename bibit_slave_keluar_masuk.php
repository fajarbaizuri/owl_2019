...
switch($proses)
{
        case'saveTab1':
           ...
           {
               exit(" Error:warning data sudah di posting");
           }
        break;
        case'loadDataStock':
                ...
                $tglSkrng=date("Y-m-d");
                $thnSkrng=date("Y");
                $sql2="select * from ".$dbname.".bibitan_mutasi where kodeorg like '%".$_SESSION['empl']['lokasitugas']."%' group by batch,kodeorg order by tanggal desc";
                $query2=mysql_query($sql2) or die("Error ".mysql_error($conn));
                $jlhbrs=mysql_num_rows($query2);
                if($jlhbrs!=0)
                {
                $sData="select  batch,kodeorg,sum(jumlah) as jumlah from ".$dbname.".bibitan_mutasi where kodeorg like '%".$_SESSION['empl']['lokasitugas']."%' and post=1 group by batch,kodeorg order by tanggal desc limit ".$offset.",".$limit." ";
               // exit("error".$sData);
        ...
        case'loadData1':
                ...
                $sql2="select * from ".$dbname.".bibitan_mutasi where kodetransaksi='TMB' and  kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc ";
                $query2=mysql_query($sql2) or die("Error ".mysql_error($conn));
                $jlhbrs=mysql_num_rows($query2);
                if($jlhbrs!=0)
                {
                $sData="select distinct kodetransaksi, jumlah,batch,kodeorg,tanggal,post,flag from ".$dbname.".bibitan_mutasi  where kodetransaksi='TMB' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc limit ".$offset.",".$limit." ";
               // exit("error".$sData);
                $qData=mysql_query($sData) or die("Error ".mysql_error($conn));
                while($rData=mysql_fetch_assoc($qData))
                {
        ...
        case'loadData2':
                ...
                $sql2="select * from ".$dbname.".bibitan_mutasi where kodetransaksi='TPB' and  kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc ";
                $query2=mysql_query($sql2) or die("Error ".mysql_error($conn));
                $jlhbrs=mysql_num_rows($query2);
                if($jlhbrs!=0)
                {
                $sData="select distinct kodetransaksi, jumlah,batch,kodeorg,tanggal,post,flag,tujuan,keterangan from ".$dbname.".bibitan_mutasi  where kodetransaksi='TPB' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc limit ".$offset.",".$limit." ";
               // exit("error".$sData);
                $qData=mysql_query($sData) or die("Error ".mysql_error($conn));
                while($rData=mysql_fetch_assoc($qData))
                {
        ...
        break;
         case'loadData3':
                ...
                $sql2="select * from ".$dbname.".bibitan_mutasi where kodetransaksi='AFB' and  kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc ";
                $query2=mysql_query($sql2) or die("Error ".mysql_error($conn));
                $jlhbrs=mysql_num_rows($query2);
                if($jlhbrs!=0)
                {
                $sData="select distinct kodetransaksi, jumlah,batch,kodeorg,tanggal,post,flag,tujuan,keterangan from ".$dbname.".bibitan_mutasi  where kodetransaksi='AFB' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc limit ".$offset.",".$limit." ";
               // exit("error".$sData);
                $qData=mysql_query($sData) or die("Error ".mysql_error($conn));
                while($rData=mysql_fetch_assoc($qData))
                {
        ...
        break;
         case'loadData5':
                ...
                $sql2="select * from ".$dbname.".bibitan_mutasi where kodetransaksi='DBT' and  kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc ";
                $query2=mysql_query($sql2) or die("Error ".mysql_error($conn));
                $jlhbrs=mysql_num_rows($query2);
                if($jlhbrs!=0)
                {
                $sData="select distinct kodetransaksi, jumlah,batch,kodeorg,tanggal,post,flag,keterangan from ".$dbname.".bibitan_mutasi  where kodetransaksi='DBT' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc limit ".$offset.",".$limit." ";
               // exit("error".$sData);
                $qData=mysql_query($sData) or die("Error ".mysql_error($conn));
                while($rData=mysql_fetch_assoc($qData))
                {
        ...
        case'loadData7':
                ...
                $sql2="select * from ".$dbname.".bibitan_mutasi where kodetransaksi='PNB' and  kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc ";
                $query2=mysql_query($sql2) or die("Error ".mysql_error($conn));
                $jlhbrs=mysql_num_rows($query2);
                if($jlhbrs!=0)
                {
                $sData="select distinct * from ".$dbname.".bibitan_mutasi  where kodetransaksi='PNB' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tanggal desc limit ".$offset.",".$limit." ";
               // exit("error".$sData);
                $qData=mysql_query($sData) or die("Error ".mysql_error($conn));
                while($rData=mysql_fetch_assoc($qData))
                {
                    ...
                    if($data==0)
                    {
                       ...
                       // $tab.="<img id='detail_edit' &nbsp; style='cursor:pointer;' title='Edit ".$rData['batch']."' class=zImgBtn onclick=\"filField7('".$rData['kodetransaksi']."','".$rData['batch']."','".$rData['kodeorg']."','".tanggalnormal($rData['tanggal'])."','".substr($rData['jumlah'],1)."','".$rData['kodevhc']."','".$rData['sopir']."','".$rData['intex']."','".$rData['pelanggan']."','".$rData['lokasipengiriman']."','".$rData['penanggungjawab']."','".$rData['afdeling']."','".$rData['jenistanam']."','".$rData['rit']."')\" src='images/application/application_edit.png'/>";
                        $tab.="&nbsp;<img  style='cursor:pointer;' title='Delete ".$rData['batch']."' class=zImgBtn onclick=\"delField7('".$rData['tanggal']."','".$rData['kodetransaksi']."','".$rData['batch']."','".$rData['kodeorg']."','".$rData['rit']."','".$rData['kodevhc']."')\" src='images/application/application_delete.png'/>";
                        $tab.="&nbsp;<img  style='cursor:pointer;' title='Posting Data ".$rData['batch']."' class=zImgBtn onclick=\"postingData7('".$rData['tanggal']."','".$rData['kodetransaksi']."','".$rData['batch']."','".$rData['kodeorg']."','".$rData['rit']."','".$rData['kodevhc']."','".$rData['jumlah']."')\" src='images/skyblue/posting.png'/>";
                        $tab.="&nbsp;<img  style='cursor:pointer;' title='PDF ".$rData['batch']."' class=resicon  src='images/pdf.jpg'/></td>";
                    }
                    else
                    {
                        $tab.="<td><img  style='cursor:pointer;' title='Posting Data ".$rData['batch']."' class=resicon  src='images/pdf.jpg'/></td>";   
                    }
	...
	case'update1':
           ...
           {
               exit(" Error: Data Sudah di Posting");
           }
	break;
        ...
        break;
         case'delData2':
          ...
          // exit("Error".$scek);
           ...
           {
               exit(" Error:Sudah di posting");
           }
        break;
        case'delData3':
           $sDelete2="delete from ".$dbname.".bibitan_mutasi where kodeorg='".$kdOrg."' and kodetransaksi='".$kodeTrans."' and batch='" .$batchVar."' and tanggal='".$_POST['tanggal']."'";
               if(!mysql_query($sDelete2))
               { 
                   echo "DB Error : ".$sDelete2."\n".mysql_error($conn);
        ...
        break;
         case'delData7':
           $sDelete2="delete from ".$dbname.".bibitan_mutasi where kodeorg='".$kdOrg."' and kodetransaksi='".$kodeTrans."' and rit='".$jmlRit."' and kodevhc='".$kdvhc."' and batch='" .$batchVar."'  and tanggal='".$_POST['tanggal']."'";
               if(!mysql_query($sDelete2))
               { 
                   echo "DB Error : ".$sDelete2."\n".mysql_error($conn);
        ...
        case'postData':
           ...
           {
               exit(" Error:Data tidak ada atau Sudah di posting");
           }
        break;
        case'postData2':
           ...
           {
               exit(" Error:Jumlah ".$jmlhBibitan." Lebih Besar dari Total Stok Yang Ada ".$rcek2['totalBibitan']." pada batch".$batchVar." lokasi ".$kdOrg);
           }
       ...
       case'postData5':          
        ...
        case'postData7':
          ...
          // exit("Error".$rcek2['totalBibitan']);
           ...
           $scek="select distinct post from ".$dbname.".bibitan_mutasi where kodeorg='".$kdOrg."' and kodetransaksi='".$kodeTrans."' and rit='".$jmlRit."' and kodevhc='".$kdvhc."' and batch='" .$batchVar."' and tanggal='".$_POST['tanggal']."'";
          // exit("Error".$scek);
           $qcek=mysql_query($scek) or die("Error ".mysql_error($conn));
           $rcek=mysql_fetch_assoc($qcek);
           //exit("Error".$rcek['post']);
           if($rcek['post']=='0')
           {
               $sDel="update ".$dbname.".bibitan_mutasi set post=1 where kodeorg='".$kdOrg."' and kodetransaksi='".$kodeTrans."' and rit='".$jmlRit."' and kodevhc='".$kdvhc."' and batch='" .$batchVar."' and post='0' and tanggal='".$_POST['tanggal']."'";
               //exit("Error".$sDel);
               if(!mysql_query($sDel))
               {
                   echo "DB Error : ".$sDel."\n".mysql_error($conn);
        ...
        case'saveTab2':
           ...
           {
               exit (' Error, Data Sudah Ada/posted');
           }
        ...
        case'saveTab3':
           ...
           if($jmlhBibitan>$rcek2['totalBibitan'])
           {
               exit(" Error:Jumlah ".$jmlhBibitan." Lebih Besar dari Total Stok Yang Ada ".$rcek2['totalBibitan']." untuk batch ".$batchVar." pada lokasi ".$kdOrg);
           }
           //$sGetData="select distinct "
        ...
        case'saveTab5':
           ...
           if($jmlhBibitan>$rcek2['totalBibitan'])
           {
               exit(" Error:Jumlah ".$jmlhBibitan." Lebih Besar dari Total Stok Yang Ada ".$rcek2['totalBibitan']." untuk batch ".$batchVar." pada lokasi ".$kdOrg."\n doubletoon max. jadi 2 untuk 1 bibit");
           }
