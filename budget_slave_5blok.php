...
if($jmlh>=1)
{
    ...
    {   $sCekOpt="select tahunbudget,kodeblok,statusblok from ".$dbname.".bgt_blok  
        ...
        {
            //$arrOptTopo[$afdIdBr]="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
           // exit("Error");
...
switch($proses)
{
        ...
        echo $rCek;
            ...
            {
                exit("Error:Tahun Budget Sudah Di tutup");
            }
        break;
	case'getPreview':
        ...
        {
            //$stat=$rCek;
            $sCek2="select tahuntanam as tahunbudget,statusblok,luasareaproduktif,jumlahpokok,topografi,kodeorg,tahuntanam from ".$dbname.".setup_blok where kodeorg like '%".$afdId."%' ";
            
        }
        elseif($jmlh>1)
        {
            $sCek2="select tahunbudget, kodeblok as kodeorg, hathnlalu as luasareaproduktif, hathnini, pokokthnlalu as jumlahpokok, pokokthnini , statusblok, topografi,thntnm as tahuntanam,hanonproduktif,lcthnini,pokokproduksi from ".$dbname.".bgt_blok 
        ...
        $b=1;
            ...
            while($res=mysql_fetch_assoc($qCek2))
            {
                $no+=1;
				$res['pokokproduksi'];
                $res['hathnini']==''?$res['hathnini']=$res['luasareaproduktif']:$res['hathnini']=$res['hathnini'];
                $res['pokokthnini']==''?$res['pokokthnini']=$res['jumlahpokok']:$res['pokokthnini']=$res['pokokthnini'];
                $res['pokokproduksi']==''?$res['pokokproduksi']=$res['pokokthnini']:$res['pokokproduksi']=$res['pokokproduksi'];
				//echo $res['pokokproduksi']."_";
				
                $tab.="<tr class=rowcontent id=rew_".$no.">";
                $tab.="<td>".$no."</td>";
                $tab.="<td>".$thnAngrn."</td>
					<td id=kdBlok_".$no.">".$res['kodeorg']."</td>
					
					<td align=right onclick='getData(".$no.")' style='cursor:pointer;' id=luas_".$no.">".$res['luasareaproduktif']."</td>";
		...
		$totarepkprod+=$res['luasareaproduktif'];
		$totha+=$res['hathnini'];	
		$totjmpkk+=$res['jumlahpokok'];	
		
		//ini
		$totpkkthnini+=$res['pokokthnini'];		
		
		$totlc+=$res['lcthnini'];
		$tothanon+=$res['hanonproduktif'];
		$totpkkprod+=$res['pokokproduksi'];
		
            }
		$tab.="
		<thead><tr>
			<td colspan=3>Total</td>
			<td align=right>".$totarepkprod."</td>
			<td align=right>".$totha."</td>
			<td align=right>".$totjmpkk."</td>
			<td align=right>".$totpkkthnini."</td>
			<td colspan=3></td>
			<td align=right>".$totlc."</td>
			<td align=right>".$tothanon."</td>
			<td align=right>".$totpkkprod."</td>
	...
	break;
        ...
        case'insertAll':
           ...
           if($pokokThnIni==''||$haThnIni=='')
           {
                ...
                if(mysql_query($sDel))
                { exit("Error:Ha Thn Ini,Ha Thn Lalu Tidak Boleh Kosong");}
                else
                {    echo "DB Error : ".$sDel."\n".mysql_error($conn);}
           }
            if($rCek!=1)
            {
                ...
                if($rCktopo['topografi']!='')
                {
                    ...
                    if($rCek2>0)
                    {
                        ...
                        $sInsert="insert into ".$dbname.".bgt_blok (tahunbudget, kodeblok, hathnlalu, hathnini, pokokthnlalu, pokokthnini, statusblok, topografi, thntnm, hanonproduktif, sumber, updateby,hamutasi,pokokmutasi,pokokproduksi,lcthnini) 
                        value ('".$thnAngrn."','".$kdBlok."','".$haThnLalu."','".$haThnIni."','".$pkkThnLalu."','".$pokokThnIni."','".$statBlok."','".$topoGrafi."','".$thnTmn."','".$haNon."','LAMA','".$_SESSION['standard']['userid']."','".$hamutasi."','".$pokokMutasi."','".$pkkProduktif."','".$lcThnini."')";
                        //exit("Error".$sInsert);
                ...
                else
                {
                    $sDel="delete from ".$dbname.".bgt_blok where tahunbudget='".$thnAngrn."' and substr(kodeblok,1,6)='".substr($kdBlok,0,6)."' and sumber='LAMA'";
                        if(mysql_query($sDel))
                        {exit("Error:Topografi Kosong,Silahkan Input Topografi pada Setup Blok.");}
                        else
                        {    echo "DB Error : ".$sDel."\n".mysql_error($conn);}
                }
            }
            ...
            {
                exit("Error:Budget Sudah di Tutup");
            }
       ...
       // $tab.="<tr><td colspan='12' align=center><button class=mybutton  onclick=saveAllBr(1)  style=cursor:pointer;>".$_SESSION['lang']['save']." ".$_SESSION['lang']['all']."</button></td></tr>";
		...
		<button class=mybutton onclick=cariBast(".($page+1).");>".$_SESSION['lang']['lanjut']."</button>
           ...
           // echo"waring:masuk";
//           
//        $sCek="select * from ".$dbname.".bgt_blok where tahunbudget='".$thnAngBr."' and kodeblok like '%".$afdIdBr."%' and sumber='BARU'";
//        //exit("Error".$sCek);
//        $qCek=mysql_query($sCek) or die(mysql_error($conn));
//        $rCek=mysql_num_rows($qCek);
//        echo $rCek;
...
/*            if(strlen($kdBlokBr)<10)
            {
                exit("Error:Kode Blok Tidak Boleh Kosong atau kurang dari 4 Karakter");
            }*/
            if($thnTmnBr=='')
            {
                exit("Error:Tahun Tanam Tidak Boleh Kosong");
            }
            ...
            if($haThnIniBr==''||$pokokThnIniBr=='')
            {
                $sDel="delete from ".$dbname.".bgt_blok where tahunbudget='".$thnAngrnOld."' and kodeblok='".$oldBlok."' and topografi='".$topoGrafOld."' and sumber='BARU'";
                    ...
                    if(mysql_query($sDel))
                    {
                        exit("Error:Ha Tahun Ini dan Pokok Tahun Ini, tidak boleh kosong");                        
            ...
            if($rCek!=1)
            {
                ...
                if($rCek2>0)
                {
                    ...
                    if(mysql_query($sDel))
                    {
                        ...
                        {
                            exit("Error:Pokok Produktif Tidak Boleh Besar Dari Pokok Tahun Ini");
                        }
                        $sInsert="insert into ".$dbname.".bgt_blok (tahunbudget, kodeblok,  hathnini,  pokokthnini, statusblok, topografi, thntnm, hanonproduktif, sumber, updateby,pokokproduksi,lcthnini) 
                              value ('".$thnAngBr."','".$kdBlokBr."','".$haThnIniBr."','".$pokokThnIniBr."','".$statBlokBr."','".$topoGrafiBr."','".$thnTmnBr."','".$haNonBr."','BARU','".$_SESSION['standard']['userid']."','".$pkkProdukBr."','".$lcThnBr."')";
        ...
        $jlhbrs= $jsl->jmlhrow;
            ...
            while($rLoad=  mysql_fetch_assoc($qLoad))
            {
                ...
                if($rLoad['closed']!=1)
                {
                $tab.="<td align=center><img onclick=datakeExcel2(event,'".$rLoad['tahunbudget']."','".$afd."','".$rLoad['sumber']."') src=images/excel.jpg class=resicon title='MS.Excel'>&nbsp;
                    <img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"fillField('".$rLoad['tahunbudget']."','".$rLoad['kodeblok']."','".$rLoad['sumber']."','".$rLoad['topografi']."');\"></td>";
                }
                else
                { $tab.="<td align=center><img onclick=datakeExcel2(event,'".$rLoad['tahunbudget']."','".$afd."','".$rLoad['sumber']."') src=images/excel.jpg class=resicon title='MS.Excel'></td>";}
                $tab.="</tr>";
            }
             $tab.="
		<tr class=rowheader><td colspan=11 align=center>
       ...
       // $tab.="<tr><td colspan='12' align=center><button class=mybutton  onclick=saveAllBr(1)  style=cursor:pointer;>".$_SESSION['lang']['save']." ".$_SESSION['lang']['all']."</button></td></tr>";
        ...
        $offset=$page*$limit;
//        $ql2="select distinct count(substring(kodeblok,1,6)) as jmlhrow from ".$dbname.".bgt_blok where  sumber='LAMA' and kodeblok like '%".$_SESSION['empl']['lokasitugas']."%' group by tahunbudget,kodeblok order by tahunbudget desc ";//exit("Error".$ql2);
//        $query2=mysql_query($ql2) or die(mysql_error());
//        while($jsl=mysql_fetch_object($query2)){
//        $jlhbrs= $jsl->jmlhrow;
//        }
