...
$aresta="SELECT sum(beratbersih) as bi FROM ".$dbname.".pabrik_timbangan
    WHERE nospb like '%".$unit."%' and DATE_FORMAT(tanggal, '%Y-%m') = '".$periode."'"; 
...
$aresta="SELECT sum(beratbersih) as sdbi FROM ".$dbname.".pabrik_timbangan
    WHERE nospb like '%".$unit."%' and (substr(tanggal,1,10) between '".$tahun."-01-01 00:00:00' and LAST_DAY('".$periode."-15'))";
...
while($res=mysql_fetch_assoc($query))
{
    ...
    $kegpanen[$res['noakun']]['namaakun']=$res['noakun'].":".$res['namaakun'];
}   
...
while($res=mysql_fetch_assoc($query))
{
    ...
    $kegpemel[$res['noakun']]['namaakun']=$res['noakun'].":".$res['namaakun'];
}   

// cari kegiatan tidak langsung
$aresta="SELECT noakun, namaakun FROM ".$dbname.".keu_5akun
    WHERE length(noakun)=7 and noakun like '711%'
    ORDER BY noakun";
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
    $kegtidak[$res['noakun']]['noakun']=$res['noakun'];
    $kegtidak[$res['noakun']]['namaakun']=$res['noakun'].":".$res['namaakun'];
}   
...
$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
    WHERE tanggal like '".$periode."%' and kodeorg like '%".$unit."%'
    GROUP BY noakun";
...
//$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
  //  WHERE (substr(tanggal,1,10) between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and nojurnal like '%".$unit."%'
	...
	$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
    WHERE (substr(tanggal,1,10) between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and kodeorg like '%".$unit."%'
  GROUP BY noakun";
...
$tab.="<table border=0>
     <tr>
        <td colspan=8 align=left><font size=3>20. BIAYA PRODUKSI</font></td>
...
if($proses!='excel')$tab.=$judul;
    ...
    <tr>
    <td align=right colspan=2 ".$bg.">Produksi (kg):</td>
...
// PANEN DAN PENGUMPUL =========================================================================
    if(!empty($kegpanen)){
        ...
        $tab.= "<td>PANEN DAN PENGUMPUL</td><td colspan=12>&nbsp;</td>";
        $tab.= "</tr>";
        $totalpanen=Array();
        $no=1;
        ...
        $tab.= "<tr class=rowcontent>";
        $tab.= "<td colspan=2 align=center>BIAYA PANEN DAN PENGUMPUL</td>";
...
// PEMELIHARAAN TM =========================================================================
    if(!empty($kegpanen)){
        ...
        $tab.= "<td>PEMELIHARAAN TM</td><td colspan=12>&nbsp;</td>";
        $tab.= "</tr>";
        $totalpemel=Array();
        $no=1;
        ...
        $tab.= "<tr class=rowcontent>";
        $tab.= "<td colspan=2 align=center>BIAYA PEMELIHARAAN TM</td>";
...
// BIAYA TIDAK LANGSUNG (OVER HEAD) =========================================================================
    ...
    $tab.= "<tr class=rowcontent>";
    $tab.= "<td colspan=2 align=center>TOTAL BIAYA PRODUKSI</td>";
...
switch($proses)
{
    ...
    function Header() {
        ...
        $this->Cell($width/2,$height,'20. BIAYA PRODUKSI (RP./KG)',NULL,0,'L',1);
...
// PANEN DAN PENGUMPUL =========================================================================
    if(!empty($kegpanen)){
        ...
        $pdf->Cell($wkiri/100*$width,$height,'PANEN DAN PENGUMPUL',1,0,'L',1);	
        ...
        $pdf->Cell(($wkiri/100*$width)+(3/100*$width),$height,'BIAYA PANEN DAN PENGUMPUL',1,0,'C',1);	
...
// PEMELIHARAAN TM =========================================================================
    if(!empty($kegpanen)){
        ...
        $pdf->Cell($wkiri/100*$width,$height,'PEMELIHARAAN TM',1,0,'L',1);	
        $pdf->Cell(12*$wlain/100*$width,$height,'',1,0,'R',1);	
        $pdf->Ln();
        $totalpemel=Array();
        $no=1;
        ...
        $pdf->Cell(($wkiri/100*$width)+(3/100*$width),$height,'BIAYA PEMELIHARAAN TM',1,0,'C',1);	
...
// BIAYA TIDAK LANGSUNG (OVER HEAD) =========================================================================
    ...
    $totalbiaya['311']=$totalpanen['311']+$totalpemel['311']+$totaltidak['311'];
    $totalbiaya['312']=$totalpanen['312']+$totalpemel['312']+$totaltidak['312'];
    $pdf->Cell(($wkiri/100*$width)+(3/100*$width),$height,'TOTAL BIAYA PRODUKSI',1,0,'C',1);	
