...
<script language=javascript1.2 src='js/datasensus.js'></script>
<?
...
else if(trim($_SESSION['org']['induk']!=''))//user unit hanya dapat menempatkan pada unitnya dan anak unitnya
{
 ...
 // $optlokasitugas=ambilLokasiTugasDanTurunannya('option',$_SESSION['empl']['lokasitugas']);
...
// get sistem penggajian enum
		$optagama='';
		$arragama=getEnum($dbname,'datasensus','agama');
...
$optGoldar='';
$arrenum=getEnum($dbname,'datasensus','golongandarah');
...
//get Golongan darah from enum
$optJK='';
$arrenum=getEnum($dbname,'datasensus','jeniskelamin');
foreach($arrenum as $key=>$val)
{
...
// get sistem penggajian enum
		$optsisgaji='';
		$arrsgaj=getEnum($dbname,'datasensus','sistemgaji');
...
$optstkawin='';
$arrsstk=getEnum($dbname,'datasensus','statusperkawinan');
...
//first tab content=======================================================		
$frm[0]="<fieldset><legend>INPUT SENSUS KARYAWAN</legend>
		 ...
		 <tr>
			<td align=right>".$_SESSION['lang']['tanggalkeluar']."</td><td><input type=text class=myinputtext id=tanggalkeluar size=26 maxlength=10 onkeypress=\"return false;\" onmousemove=setCalendar(this)></td> 
		...
		";
//Tab experiences=================================
$optbln="<option value=''>Bulan</option>";
for($x=1;$x<13;$x++)
{
	if($x<10)
	   $bln="0".$x;
...
$optthn="<option value=''>Tahun</option>";
for($x=0;$x<60;$x++)
{
	$thn=date('Y')-$x;
...
$opthubk='';
$arrenum=getEnum($dbname,'sdm_sensuskeluarga','hubungankeluarga');
foreach($arrenum as $key=>$val)
{
...
//get enum untuk hub keluarga
		$optstk='';
		$arrenum=getEnum($dbname,'sdm_sensuskeluarga','status');
		foreach($arrenum as $key=>$val)
		{
...
$frm[5]="<fieldset style='width:155px;height:180px;'>
		 ...
		 <iframe frameborder=0 width=350px height=70px name=winForm id=winForm src=sdm_form_upload_sensus_photo.php>
		 </iframe>
		 </fieldset>
