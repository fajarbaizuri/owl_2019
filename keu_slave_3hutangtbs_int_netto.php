...
$str="select `keterangan` from ".$dbname.". keu_jurnaldt_vw where `noreferensi` like 'TBS_INT%' and noakun='1150101'"; 
 ...
 while($bar=mysql_fetch_array($res))
 {
	...
	$str2=" SELECT a.kodeorg,b.namaorganisasi as nama, a.kgpotsortasi,a.kodecustomer,a.beratbersih as netto,substr(a.tanggal,1,10) as tanggal,(a.beratbersih/(a.jumlahtandan1+a.jumlahtandan2+a.jumlahtandan3)) as bjr 
			FROM ".$dbname.".pabrik_timbangan a left join ".$dbname.".organisasi b ON a.kodeorg=b.kodeorganisasi
			WHERE a.tanggal between '".$TglFrom[2]."-".$TglFrom[1]."-".$TglFrom[0]." 00:00:00' and '".$TglTo[2]."-".$TglTo[1]."-".$TglTo[0]." 23:59:59' and a.kodeorg like '___E' order by a.kodeorg,a.tanggal";	
		...
		if ($val['bjr'] == 1) {
			...
			if ($val['kg'] == 1) {
				if ($rowH['bjr'] >= 3 && $rowH['bjr'] < 5 && $rowH['normal'] > 0){
					/*
					$stream.="<tr class=rowcontent id='row".$k."'>";
					$stream.="<td id='no".$k."'>".$k."</td>
					<td id='kebun".$k."'>".$rowH['nama']."</td>
					<td id='tgl".$k."'>".tanggalnormal($rowH['tgl'])."</td>";
					$stream.="<td id='kg".$k."'>".number_format($rowH['normal'],2)."</td>
					<td id='kg".$k."'>".number_format($val['harga']*$rowH['normal']."</td>";		
...
foreach($POINTER as $key) {
	...
	foreach($AYRA[$key] as $view=>$key1) {
						...
						//if ($totalBulan==0){
							$FIX[$Z]['no']=$Z;
							$FIX[$Z]['nm']=$key;
							//$FIX[$Z]['tgl']="#".$_POST['tgl1']."#".$_POST['tgl2']."#";
							$FIX[$Z]['tgl']=$key1['tgl'];
							$FIX[$Z]['kg']+=$key1['kg']; 
							$FIX[$Z]['rp']+=$key1['rp']; 
							
						}elseif ($totalBulan==1){ 
							if (intval($blnDb[2].$blnDb[1])==intval($TglFrom[2].$TglFrom[1])){
							    $FIX[$Z]['no']=$Z;
								$FIX[$Z]['nm']=$key;
								$FIX[$Z]['tgl']="#".$_POST['tgl1']."#".jumlah_hari($TglFrom[1],$TglFrom[2])."-".$TglFrom[1]."-".$TglFrom[2]."#"; 
							    $FIX[$Z]['kg']+=$key1['kg']; 
							    $FIX[$Z]['rp']+=$key1['rp']; 
							}else{
								$FIX[$Z]['no']=$Z;
								$FIX[$Z]['nm']=$key;
								$FIX[$Z]['tgl']="#01-".$TglTo[1]."-".$TglTo[2]."#".$TglTo[0]."-".$TglTo[1]."-".$TglTo[2]."#"; 
								//$FIX[$Z]['tgl']="#01-".$TglTo[1]."-".$TglTo[2]."#".$_POST['tgl2']."#";
							    $FIX[$Z]['kg']+=$key1['kg']; 
								$FIX[$Z]['rp']+=$key1['rp']; 
							}
							if ($tempkey!=$key){
							...
							if (intval($blnDb[2].$blnDb[1])==intval($TglFrom[2].$TglFrom[1])){
							...
							}elseif (intval($blnDb[2].$blnDb[1])==intval($TglTo[2].$TglTo[1])){
								$FIX[$Z]['no']=$Z;
								$FIX[$Z]['nm']=$key;
								$FIX[$Z]['tgl']="#01-".$TglTo[1]."-".$TglTo[2]."#".$_POST['tgl2']."#";
							    $FIX[$Z]['kg']+=$key1['kg']; 
								...
								//$newdate = strtotime("+ ".$F." month",strtotime($TglFrom[2]."-".$TglFrom[1]."-01"));
							//	$newdate = date('d-m-Y', $newdate);
							//	$TglBaru = explode('-',$newdate);  
							//	if (intval($blnDb[2].$blnDb[1])==intval($TglBaru[2].$TglBaru[1])){
									//$FIX[$Z]['no']=intval($TglBaru[2].$TglBaru[1]);
									$FIX[$Z]['no']=$Z;
									$FIX[$Z]['nm']=$key;
									//$FIX[$Z]['tgl']="#01-".$TglBaru[1]."-".$TglBaru[2]."#".jumlah_hari($TglBaru[1],$TglBaru[2])."-".$TglBaru[1]."-".$TglBaru[2]."#";
									$FIX[$Z]['tgl']="##";
									$FIX[$Z]['kg']+=$key1['kg']; 
									$FIX[$Z]['rp']+=$key1['rp'];  
...
echo"<button class=mybutton onclick=prosesInternal(1) id=btnproses>Process</button>
