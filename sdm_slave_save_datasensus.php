...
switch($method){
		case 'delete':
			$strx="delete from ".$dbname.".datasensus where karyawanid=".$karyawanid;
		break;
		case 'update':
                  
			$strx="update ".$dbname.".datasensus set 
			       `nik`			='".$nik."',
		...
		case 'insert':
			...
			$iCek="select distinct nik from ".$dbname.".datasensus where nik='".$nik."'";
			$ada=true;
			...
			$strx="insert into ".$dbname.".datasensus(
			  `nik`,`namakaryawan`,
		...
		//$levelpendidikan=$_POST['levelpendidikan'];
		$aCek="select levelpendidikan from ".$dbname.".sdm_sensuspendidikan where levelpendidikan ='".$levelpendidikan."' ";
    ...
    if(mysql_query($strx))
	{
	   //whenever not deleting, return value as below to javascript
		if($method!='delete')
		{
			$karid='';
			$nama='';
			$str="select karyawanid,namakaryawan from ".$dbname.".datasensus where
