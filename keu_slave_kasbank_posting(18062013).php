...
if($tmpJml!=$dataH[0]['jumlah']) {
    echo "Warning : Jumlah pada Header tidak sama dengan Detail\n";
...
$dataRes['detail'][] = array(
  ...
  if($totalJumlahOto!=0){
    ...
    $dataResoto['header']['nojurnal']=$nojuroto;
    $dataResoto['header']['kodejurnal']=$kodejurnal;
    $dataResoto['header']['totaldebet']=$totalJumlahOto; 
    $dataResoto['header']['totalkredit']=$totalJumlahOto*(-1);
    $dataResoto['header']['noreferensi']=$pembayarhutang.$dataH[0]['notransaksi'];    
  }
$konteroto++;
...
if($errorDB=='') {
    if($dataResoto['header']['nojurnal']!=''){
      $queryH = insertQuery($dbname,'keu_jurnalht',$dataResoto['header']);
      if(!mysql_query($queryH)) {
          $errorDB .= "Header oto :".mysql_error()."\n";
      }    
  }
}
# Detail
