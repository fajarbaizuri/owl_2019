<?php
	...
	{
		$nopp=$_POST['rnopp'];
		$induk_org=substr($nopp,-4);
		$kode_org=$_POST['kode_org'];
		$str="SELECT * FROM ".$dbname.".`log_prapoht` WHERE `nopp`='".$nopp."'";
		/*$str="SELECT a.*, b.karyawanid, b.namakaryawan, b.lokasitugas, b.kodejabatan,d.namajabatan
				FROM ".$dbname.".`log_prapoht` a
				INNER JOIN ".$dbname.".`datakaryawan` b ON a.kodeorg = b.lokasitugas
				INNER JOIN ".$dbname.".`sdm_5jabatan` d ON b.kodejabatan = d.kodejabatan
				WHERE b.kodeorganisasi = '".$kode_org."' or b.lokasitugas='".$kode_org."' and b.tipekaryawan='0'"; *///echo $str;
		  ...
		  if($res=mysql_query($str))
		  {
			...
			while($bar=mysql_fetch_object($res))
			{
				...
				$no+=1;
			?>
				<tr class=rowcontent style='cursor:pointer;' onclick=\"setDraft(<?php echo '".$bar->karyawanid."'?>,<?php  echo '".$bar->nopp."'?>)\" title='Click' >
					  <td class=firsttd colspan=3>".$no."</td>
						<tr >
							<td>No. PP</td>
							<td>:</td>
							<td><input id="nopp" type="text" readonly="readonly" value="<?php echo $bar->nopp?>" /></td>
						</tr>
						<tr>
							<td>Nama Karyawan</td>
							<td>:</td>
	...
	if(isset($_POST['hnopp']))
	{
		...
		$str="select * from ".$dbname.".`log_prapoht` where `nopp`='".$nopp."'"; //echo $str;
		 /* $str="SELECT * FROM ".$dbname.".`log_prapoht` a 
		  inner join ".$dbname.".`datakaryawan` b on a.kodeorg=b.lokasitugas 
		  inner join ".$dbname.".`sdm_5jabatan` c on b.kodejabatan=c.kodejabatan  where `nopp`='".$nopp."' and b.karyawanid='".$krywn."'"; //echo $str;*/
		  if($res=mysql_query($str))
		  {
			echo"<table class=data cellspacing=1 border=0>
				 <thead>
	...
	$offset=$page*$limit;
		...
		if($txt_search==''&&$txt_tgl=='')
		{
			if($_SESSION['empl']['tipelokasitugas']=='HOLDING')
			{
                            ...
                            //$strx="select * from ".$dbname.".log_prapoht where ".$where2." order by tanggal desc";
		...
		else
		{
			if($_SESSION['empl']['tipelokasitugas']=='HOLDING')
			{
				/*$strx="select * from ".$dbname.".log_prapoht a inner join ".$dbname.".log_prapodt b on a.nopp=b.nopp
				where substring(a.nopp,16,4)='".$_SESSION['empl']['lokasitugas']."' and b.create_po='0'".$where."group by a.nopp order by a.nopp desc limit ".$offset.",".$limit."";*/
				//$strx="select * from ".$dbname.".log_prapoht where substring(nopp,16,4)='".$_SESSION['empl']['lokasitugas']."' ".$where." order by tanggal desc";
                            $sCek="select bagian from ".$dbname.".datakaryawan where karyawanid='".$_SESSION['standard']['userid']."'";
                            $qCek=mysql_query($sCek) or die(mysql_error($conn));
                            $rCek=mysql_fetch_assoc($qCek);
                            // echo $rCek['bagian'];
			...
			else
			{
				/*$strx="select * from ".$dbname.".log_prapoht a inner join ".$dbname.".log_prapodt b on a.nopp=b.nopp
				where substring(a.nopp,16,4)='".$_SESSION['empl']['lokasitugas']."' and b.create_po='0'".$where."group by a.nopp order by a.nopp desc limit ".$offset.",".$limit."";*/
                            $sql="select count(*) jmlhrow from ".$dbname.".log_prapoht where RIGHT(nopp,4)='".$_SESSION['empl']['lokasitugas']."' ".$where." order by tanggal desc";
                            $str="select * from ".$dbname.".log_prapoht where RIGHT(nopp,4)='".$_SESSION['empl']['lokasitugas']."' ".$where." order by tanggal limit ".$offset.",".$limit."";
