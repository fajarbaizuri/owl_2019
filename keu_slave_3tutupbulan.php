...
else
{
    ...
    {
        echo "Masih ada kas/bank yang belum di poasting:\n";
        $no=0;
        while($bar=mysql_fetch_object($res))
        {
           $no+=1;
                ...
                if(!mysql_query($queryUpd)) {
                    ...
                    if(!mysql_query($queryIns)) {
                        ...
                        if(!mysql_query($queryRB)) {
                            echo 'Error Rollback Insert : '.mysql_error();
                            exit;
                        }
                    }
                }
        ...
        foreach($salak as $key=>$val){
            if($salak[$key]!=''){
               ...
               {      
                   if(!mysql_query($temp))
                   {
                       exit("Error insert saldo awal ".mysql_error($conn)."atau Alokasi Biaya Belum benar, silakan periksa di keu_jurnaldt_vw".$temp);
                   }  
               }
               else #jika bulan 12
               {                     
