...
/*
$query1 = selectQuery($dbname,'datakaryawan','karyawanid,tipekaryawan,namakaryawan',"tipekaryawan in(2,3,4) and ".
    "lokasitugas='".$_SESSION['empl']['lokasitugas']."' and ".
    "(tanggalkeluar>='".$tanggal2."' or tanggalkeluar='0000-00-00') and alokasi=0 and sistemgaji='Harian'".
    " and ( tanggalmasuk<='".$tanggal2."' or tanggalmasuk='0000-00-00' or tanggalmasuk is null)");
	$query1 = "select a.karyawanid,b.tipekaryawan,b.namakaryawan from ".$dbname.".kebun_kehadiran_vw a left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
				where b.tipekaryawan in(2,3,4) and b.lokasitugas='".$_SESSION['empl']['lokasitugas']."' and 
	(b.tanggalkeluar>='".$tanggal2."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0 and b.sistemgaji='Harian' and a.unit='".$_SESSION['empl']['lokasitugas']."'
	and ( b.tanggalmasuk<='".$tanggal2."' or b.tanggalmasuk='0000-00-00' or b.tanggalmasuk is null) group by a.karyawanid";
	
	$query1 = "select karyawanid,tipekaryawan,namakaryawan from ".$dbname.".datakaryawan  where 
tipekaryawan in(2,3,4) and lokasitugas='".$_SESSION['empl']['lokasitugas']."' and (tanggalkeluar>='".$tanggal2."' or 
tanggalkeluar='0000-00-00') and alokasi=0 and sistemgaji='Harian' and 
(tanggalmasuk<='".$tanggal2."' or tanggalmasuk='0000-00-00' or tanggalmasuk is null) 
and karyawanid in (SELECT distinct karyawanid FROM ".$dbname.".kebun_kehadiran_vw 
WHERE unit = '".$_SESSION['empl']['lokasitugas']."' AND tanggal >=  '".$tanggal1."' AND tanggal <=  '".$tanggal2."')";
	*/	
	 //if($_SESSION['empl']['lokasitugas']!="TDBE"){
		 /*
...
$query1 = "select karyawanid,tipekaryawan,namakaryawan from ".$dbname.".datakaryawan  where 
tipekaryawan in(2,3,4) and lokasitugas='".$_SESSION['empl']['lokasitugas']."'  and alokasi=0 and sistemgaji='Harian' and 
(tanggalkeluar>='".$tanggal2."' or tanggalkeluar='0000-00-00') and
karyawanid in (SELECT distinct karyawanid FROM ".$dbname.".kebun_prestasi_vw 
WHERE unit = '".$_SESSION['empl']['lokasitugas']."' AND tanggal >=  '".$tanggal1."' AND tanggal <=  '".$tanggal2."')
OR karyawanid in (SELECT distinct karyawanid FROM ".$dbname.".kebun_kehadiran_vw 
WHERE unit = '".$_SESSION['empl']['lokasitugas']."' AND tanggal >=  '".$tanggal1."' AND tanggal <=  '".$tanggal2."')";
	*/
...
# ambil gaji pokok per hari untuk KHT
    ...
    $strgjh = "select a.karyawanid,sum(jumlah)/25 as gjperhari from ".$dbname.".sdm_5gajipokok a left join 
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
               ...
               and a.idkomponen in(1) and sistemgaji='Harian'
...
#6 Premi dan penalty =======================================================================
   ...
   $stru3="select distinct(tanggal)
           from ".$dbname.".vhc_kendaraan_detail_vw a left join 
          ".$dbname.".datakaryawan b on a.idkaryawan=b.karyawanid
           where b.tipekaryawan in(2,3,4) and b.lokasitugas='".$_SESSION['empl']['lokasitugas']."' 
           and  (b.tanggalkeluar>='".$tanggal2."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
           and a.tanggal>='".$tanggal1."' and a.tanggal<='".$tanggal2."'     
           and a.posting=0 and b.sistemgaji='Harian' order by tanggal";
   ...
   if(mysql_num_rows($resu1)>0 or mysql_num_rows($resu2)>0 or mysql_num_rows($resu3)>0)
   {
    ...
    #6.3.2 Get Premi Kegiatan Panen    dan gaji pokok BHL
     ...
     #6.3.3 Get Premi Transport dan gaji pokok BHL
        ...
        $query7="select sum(DISTINCT a.upahpro) AS `upah`,
sum(DISTINCT a.premipro) AS `premi`,
sum(DISTINCT a.umknpro) AS `umkn`,
sum(DISTINCT a.insentivpro) AS `insentiv`,
sum(DISTINCT a.upahpro+a.premipro+a.umknpro+a.insentivpro) AS `total`,a.idkaryawan as karyawanid 
               from ".$dbname.".vhc_kendaraan_detail_vw a left join 
              ".$dbname.".datakaryawan b on a.idkaryawan=b.karyawanid
               where b.tipekaryawan in(2,3,4) and b.lokasitugas='".$_SESSION['empl']['lokasitugas']."' 
               and  (b.tanggalkeluar>='".$tanggal2."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
               and a.pemilikalat='".$_SESSION['empl']['lokasitugas']."' 
               and a.tanggal>='".$tanggal1."' and a.tanggal<='".$tanggal2."'     
               and b.sistemgaji='Harian' group by a.idkaryawan";
         ...
         foreach($premRes2 as $idx => $val)
        {
             ...
             {    
                 if(isset ($premi[$val['karyawanid']]))
                     $premi[$val['karyawanid']]+=$val['premi']+$val['insentiv'];
                 else
                     $premi[$val['karyawanid']]=$val['premi']+$val['insentiv'];
             }
              if($val['umkn']>0)
             {              
                 if(isset ($uangmkn[$val['karyawanid']]))
                     $uangmkn[$val['karyawanid']]+=$val['umkn'];
                 else
                     $uangmkn[$val['karyawanid']]=$val['umkn'];   
             }
           #gapok KHL
		...
		#Cari Tanggal Transaksi BKM Perawatan dan Panen
		 $stA="select tanggal,karyawanid,umr as upahabsen FROM ".$dbname.".kebun_kehadiran_new_vw  where  unit='".$_SESSION['empl']['lokasitugas']."' 
and tanggal>='".$tanggal1."' and tanggal<='".$tanggal2."'  order by karyawanid asc,tanggal asc;"; 
$resu1A = mysql_query($stA); 
		 while($bar=mysql_fetch_object($resu1A))
       {
			$gajiBKM[$bar->tanggal][$bar->karyawanid]=true;
       }	   
	
			   
	$stB1="select tanggal,karyawanid,upahkerja as upahabsen FROM ".$dbname.".kebun_prestasi_vw  where  unit='".$_SESSION['empl']['lokasitugas']."' 
	and tanggal>='".$tanggal1."' and tanggal<='".$tanggal2."'  order by karyawanid asc,tanggal asc;"; 
	
	$resu1Ba = mysql_query($stB1); 
		 while($bara=mysql_fetch_object($resu1Ba))
       {
			$gajiBKM[$bara->tanggal][$bara->karyawanid]=true;
       }	   
        ...
        $strup="select a.karyawanid,(b.jumlah/25) as upahabsen FROM ".$dbname.".sdm_absensidt_vw a 
			  ...
			  */
			 $strup="select a.tanggal,a.karyawanid,b.upah as upahabsen FROM ".$dbname.".sdm_absensidt_vw a 
                left join ".$dbname.".sdm_5golongan b on a.kodegolongan=b.kodegolongan and nilaihk=1
               where  substr(a.kodeorg,1,4)='".$_SESSION['empl']['lokasitugas']."' 
               and a.tanggal>='".$tanggal1."' and a.tanggal<='".$tanggal2."' 
               and a.tipekaryawan=4"; 
        ...
        {        
			if (empty($gajiBKM[$val['tanggal']][$val['karyawanid']])){
				if(empty ($gapokbhl[$val['karyawanid']])){
