...
$sKend="select b.jenisvhc,c.namabarang,a.kodevhc,a.tahunperolehan from ".$dbname.".vhc_5master a 
        ...
        and b.jenisvhc in (select distinct jenisvhc from ".$dbname.".vhc_5jenisvhc where kelompokvhc='KD')";
//exit("error:".$sKend);
//echo $sKend;
...
if($proses=='excel')
{
    ...
    $tab.="
    <table>
    <tr><td colspan=5 align=left><b>25.3 TRANSIT KENDARAAN</b></td><td colspan=7 align=right><b>".$_SESSION['lang']['bulan']." : ".substr(tanggalnormal($periode),1,7)."</b></td></tr>
	...
	<thead class=rowheader>";
        ...
        $tab.="<td rowspan=3>Jenis</td>";
        $tab.="<td rowspan=3>Nama Kend</td>";
        $tab.="<td rowspan=3>KODE MESIN/KEND</td>";
        $tab.="<td rowspan=3>Thn Perolehan</td>";
        $tab.="<td rowspan=3>ANGGARAN SETAHUN (KM)</td>";
        $tab.="<td colspan=4>REALISASI  PEMAKAIAN  FISIK</td>";
        $tab.="<td rowspan=3>RASIO S/D BLN INI (LTR/KM)</td>";
        $tab.="<td rowspan=3>ANGGARAN SETAHUN (Rp.000,-)</td>";
        $tab.="<td colspan=14>Realisasi Biaya Operasi (000)</td>";
        $tab.="<td colspan=4>COST / UNIT<br />Rp. / KM</td></tr>";
        $tab.="<tr><td colspan=2>KM</td>";
        $tab.="<td colspan=2>LTR</td>";
        $tab.="<td colspan=7>BI</td>";
        $tab.="<td colspan=7>SBI</td>";
        $tab.="<td rowspan=2>ANGGARAN SETAHUN</td>";
        $tab.="<td colspan=2>REALISASI</td></tr>";
        $tab.="<tr><td>BI</td><td>s/d BI</td><td>BI</td><td>s/d BI</td>";
        $tab.="<td>Gaji</td><td>Pre/Lembur</td><td>BBM/Plumas</td><td>S.Cadang</td><td>Reprasi</td><td>Asuransi</td><td>Total</td>";
        $tab.="<td>Gaji</td><td>Pre/Lembur</td><td>BBM/Plumas</td><td>S.Cadang</td><td>Reprasi</td><td>Asuransi</td><td>Total</td>";
        $tab.="<td>BI</td><td>s/d BI</td>";
        $tab.="</tr></thead><tbody>";
        foreach($dtKend as $lstKend)
        {
            ...
            $rTotal[$lstKend]=$biGaji[$lstKend]+$biLembur[$lstKend]+$biBbm[$lstKend]+$biSukuCdng[$lstKend]+$biReparasi[$lstKend]+$biAsuransi[$lstKend];
            ...
            $rTotalSbi[$lstKend]=$sBiGaji[$lstKend]+$sBiLembur[$lstKend]+$sBiBbm[$lstKend]+$sBiSukuCdng[$lstKend]+$sBiReparasi[$lstKend]+$sBiAsuransi[$lstKend];
 ...
 foreach($dtKend as $lstKend)
        {
            $i+=1;
                ...
                $pdf->Cell(50,$height,number_format($rasioDt[$lstKend],2),TBLR,0,'R',1);
