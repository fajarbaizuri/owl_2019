...
//$aresta="SELECT sum(hathnini) as luasareal FROM ".$dbname.".bgt_blok
 //   WHERE kodeblok like '".$unit."%' and statusblok in ('TBM') and tahunbudget = '".$tahun."'";
	$aresta="SELECT sum(hathnini) as luasareal FROM ".$dbname.".bgt_blok
    WHERE kodeblok like '".$afdId."%' and statusblok in ('TBM') and tahunbudget = '".$tahun."'";
...
$aresta="SELECT noakun, namaakun FROM ".$dbname.".keu_5akun
    WHERE ((length(noakun)=7)or(length(noakun)=5)) and (left(noakun,3) between '122' and '125')
    ORDER BY noakun";
$query=mysql_query($aresta) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
// cari kegiatan
$kegiatan="SELECT DISTINCT noakun, namakegiatan, satuan FROM ".$dbname.".setup_kegiatan";
...
//$str="SELECT noakun, setahun FROM ".$dbname.".bgt_summary_biaya_vw 
  //  WHERE tahunbudget = '".$tahun."' and unit = '".$unit."' and (left(noakun,3) between '122' and '125')";
	
if ($afdId==$unit){
	$str="SELECT noakun, setahun FROM ".$dbname.".bgt_summary_biaya_vw 
  WHERE tahunbudget = '".$tahun."' and unit = '".$unit."' and (left(noakun,3) between '122' and '125')";
}else{
	$str="SELECT noakun, setahun FROM ".$dbname.".bgt_summary_biaya_afd_vw 
    WHERE tahunbudget = '".$tahun."' and unit like '".$afdId."%' and (left(noakun,3) between '122' and '125')";
}	
...
//$str="SELECT a.*, b.noakun FROM ".$dbname.".bgt_lbm_volume_kebun_vw a
  //  LEFT JOIN ".$dbname.".setup_kegiatan b on a.kegiatan=b.kodekegiatan
    //WHERE a.tahunbudget = '".$tahun."' and a.kebun = '".$unit."' and (left(b.noakun,3) between '122' and '125')";
if ($afdId==$unit){
	$str="SELECT a.*, b.noakun FROM ".$dbname.".bgt_lbm_volume_kebun_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kegiatan=b.kodekegiatan
  WHERE a.tahunbudget = '".$tahun."' and a.kebun = '".$unit."' and (left(b.noakun,3) between '122' and '125')";
}else{
	$str="SELECT a.*, b.noakun FROM ".$dbname.".bgt_lbm_volume_afdeling_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kegiatan=b.kodekegiatan
    WHERE a.tahunbudget = '".$tahun."' and a.afdeling like '".$afdId."%' and (left(b.noakun,3) between '122' and '125')";
}	
...
//$str="SELECT noakun, rp".$bulan." as bi FROM ".$dbname.".bgt_summary_biaya_vw 
  ...
  if ($afdId==$unit){
	...
	$str="SELECT noakun, rp".$bulan." as bi FROM ".$dbname.".bgt_summary_biaya_afd_vw 
    WHERE tahunbudget = '".$tahun."' and unit like '".$afdId."%' and (left(noakun,3) between '122' and '125')";
}	
...
if ($afdId==$unit){
  ...
  WHERE a.tahunbudget = '".$tahun."' and a.kebun = '".$unit."' and (left(b.noakun,3) between '122' and '125')";
	...
	$str="SELECT a.*, b.noakun FROM ".$dbname.".bgt_lbm_porsi_afdeling_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kegiatan=b.kodekegiatan
    WHERE a.tahunbudget = '".$tahun."' and a.afdeling like '".$afdId."%' and (left(b.noakun,3) between '122' and '125')";	
}	
...
if ($afdId==$unit){
	...
	$str="SELECT noakun, ".$addstr." as jumlah FROM ".$dbname.".bgt_summary_biaya_afd_vw 
    WHERE tahunbudget = '".$tahun."' and unit like '".$afdId."%' and (left(noakun,3) between '122' and '125')";
}	
...
$str="SELECT a.kegiatan, ".$porsi.", b.noakun FROM ".$dbname.".bgt_lbm_porsi_afdeling_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kegiatan=b.kodekegiatan
    WHERE a.tahunbudget = '".$tahun."' and a.afdeling like '".$afdId."%' and (left(b.noakun,3) between '122' and '125')";		
}	
...
//$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
  //  WHERE tanggal like '".$periode."%' and nojurnal like '%".$unit."%' and (left(noakun,3) between '122' and '125')
    //GROUP BY noakun";
	
	$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
    WHERE tanggal like '".$periode."%' and kodeblok like '".$afdId."%' and (left(noakun,3) between '122' and '125')
    GROUP BY noakun";
...
//$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
  //  LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    //WHERE a.tanggal like '".$periode."%' and a.unit = '".$unit."' and (left(b.noakun,3) between '122' and '125')
    //GROUP BY a.kodekegiatan";
	
	$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    WHERE a.tanggal like '".$periode."%' and a.kodeorg like '".$afdId."%' and (left(b.noakun,3) between '122' and '125')
    GROUP BY a.kodekegiatan";
...
//$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
  //  WHERE (tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and nojurnal like '%".$unit."%' and //(left(noakun,3) between '122' and '125')
 //   GROUP BY noakun";
	
	$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt 
    WHERE (tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and kodeblok like '".$afdId."%' and (left(noakun,3) between '122' and '125')
    GROUP BY noakun";
...
//$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
  //  LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
   // WHERE (a.tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and a.unit = '".$unit."' and //(left(b.noakun,3) between '122' and '125')
  //  GROUP BY a.kodekegiatan";
	
	$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    WHERE (a.tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and a.kodeorg = '".$afdId."' and (left(b.noakun,3) between '122' and '125')
    GROUP BY a.kodekegiatan";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$tab.="<table border=0>
     <tr>
        <td colspan=8 align=left><font size=3>19. BIAYA PEMELIHARAAN TBM</font></td>
        <td colspan=6 align=right>".$_SESSION['lang']['bulan']." : ".$optBulan[$bulan]." ".$tahun."</td>
     </tr> 
     <tr><td colspan=14 align=left>".$_SESSION['lang']['unit']." : ".$optNm[$afdId]." (".$afdId.")</td></tr>   
</table>";
}
else
{ 
...
if($proses!='excel')$tab.=$judul;
    ...
    <tr>
    <td align=right colspan=3 ".$bg.">Luas Areal TBM:</td>
...
switch($proses)
{
    ...
    function Header() {
        ...
        $this->Cell($width/2,$height,'19. BIAYA PEMELIHARAAN TBM',NULL,0,'L',1);
        $this->Cell($width/2,$height,$_SESSION['lang']['bulan']." : ".$optBulan[$bulan]." ".$tahun,NULL,0,'R',1);
        $this->Ln();
        $this->Cell($width,$height,$_SESSION['lang']['unit']." : ".$optNm[$afdId]." (".$afdId.")",NULL,0,'L',1);
        $this->Ln();
        $this->Ln();

        $height = 15;
        $this->SetFont('Arial','B',7);
        $this->Cell((3/100*$width)+(($wlain+$wkiri)/100*$width),$height,'Luas Areal TBM:',0,0,'R',1);	
