...
switch ($method)
{
 case 'insert_detail_pp' :
    ...
    if($jmlh_realisai==0)
    {
        echo "Warning: Jumlah Realisasi Tidak Boleh Nol ".$jmlh_realisai."";
        exit ();
    }
    else
    {
        ...
        if(mysql_query($sql))
        {
            ...
            while($res=mysql_fetch_assoc($query))
            {
                ...
                else
                {
                    ...
                    {
                        exit("Error:Jumlah Realiasi Tidak Boleh Lebih kecil dari Jumlah Barang yang di pesan");
                    }
                    $sql2="update ".$dbname.".log_prapodt set purchaser='".$purchaser."',lokalpusat='".$lokal."',realisasi='".$jmlh_realisai."',tglAlokasi='".$tglHrini."' where kodebarang='".$kd_brng."' and nopp='".$nopp."'";
                    //echo "warning:".$sql2;exit ();
                    $query2=mysql_query($sql2) or die(mysql_error());
...
echo" <table class=\"sortable\" cellspacing=\"1\" border=\"0\">
	...
	$offset=$page*$limit;
			...
			//$txt_tgl=explode("-",$_POST['tglCari']);
                        //$txt_tgls=$txt_tgl[2]."-".$txt_tgl[1]."-".$txt_tgl[0];
		...
		if($_POST['tglCari']!='')
		{
			...
			if($statPP==1)
			{
				$strx="SELECT  distinct a.`tanggal`, a.`close`,b.*  FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
                                    WHERE a.close = '2' and b.status='0' and create_po!='0' ".$where."  ORDER BY a.nopp asc  limit ".$offset.",".$limit." ";//echo $strx;			//echo "warning:".$strx; exit();
				

				$sql="SELECT  distinct  a.`tanggal`,  a.`close`, b.* FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
                                    WHERE a.close = '2' and b.status='0' and create_po!='0' ".$where."   ORDER BY a.nopp asc ";
			}
			else if($statPP==0)
			{
				$strx="SELECT distinct  a.`tanggal`,  a.`close`, b.*  FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
                                    WHERE a.close = '2' and b.status='0'  and create_po='0'  ".$where."   ORDER BY a.nopp asc  limit ".$offset.",".$limit." ";//echo $strx;
                        ...
                        else if($statPP==2)
                        {
                           // exit("Error:Pernah disini");
                            $strx="SELECT   distinct a.`tanggal`, a.`persetujuan1`, a.`persetujuan2`, a.`persetujuan3`, a.`persetujuan4`, a.`persetujuan5`, a.`close`, a.`hasilpersetujuan1`, a.`hasilpersetujuan2`, a.`hasilpersetujuan3`, a.`hasilpersetujuan4`, a.`hasilpersetujuan5`, a.`tglp1`, a.`tglp2`, a.`tglp3`, a.`tglp4`, a.`tglp5`,b.*  
                                   FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
                                   WHERE a.close = '2' and b.status='0' and b.create_po='0'  ".$where."   ORDER BY a.tanggal limit ".$offset.",".$limit."";
			...
			$query=mysql_query($sql) or die(mysql_error());
                        ...
                        $jlhbrs= $jsl;
//			
//			while($jsl=mysql_num_rows($query)){
//			$jlhbrs= $jsl;
//			}
...
//			$starttime=strtotime($Tgl2);//time();// tanggal sekarang
//			$endtime=strtotime($tgl1);//tanggal pembuatan dokumen
...
//			$klq="select * from ".$dbname.".`datakaryawan` where  bagian='PUR' or kodejabatan='23' or kodejabatan='24' order by namakaryawan asc ";
			...
			else			
			{
				...
				echo"<tr class=rowcontent id='tr_".$no."'>
				  ...
				  echo"<td align=\"center\">";
                                   ...
                                   {
				  echo"
                                  <img src=images/application/application_add.png class=resicon title='Diajukan Kembali' onclick=\"getDataPP5('".$bar->nopp."')\" />&nbsp;";
                                   }
                                  echo"
                                  <img src=images/application/application_edit.png class=resicon  title='Edit Data' onclick=\"searchBrg('".$no."','".$_SESSION['lang']['findBrg']."','<fieldset><legend>".$_SESSION['lang']['findnoBrg']."</legend>Find<input type=text class=myinputtext id=no_brg><button class=mybutton onclick=findBrg()>Find</button></fieldset><div id=container></div><input type=hidden id=notrans_".$no." name=notrans_".$no." value=".$bar->nopp."><input type=hidden id=kdbrg_".$no." name=kdbrg_".$no."  value=".$bar->kodebarang."><input type=hidden id=nomor name=nomor  value=".$no.">',event);\">&nbsp;
                                  <img src=images/application/application_go.png class=resicon title='Diajukan Kembali' onclick=\"ajukanForm('".$bar->nopp."')\" /></td>";
				  }
				  else
				  {
				 ...
				 if($stat==1)
				  ...
				  {
//						  <select name=lokalpusat_".$no." id=lokalpusat_".$no." '".$ckh."'>".$optLokasi."</select>
						  echo"
						  <td align=center><input type=checkbox id=lokalpusat_".$no."  ".$ckh." /> Lokal
						  </td>";
						  echo"
	...
	else
	{
     ...
     <td>Ganti Barang</td>
	 <td>".$_SESSION['lang']['chat']."</td>
 	 <td>".$_SESSION['lang']['tanggal']." PB</td>
	 <td>".$_SESSION['lang']['jmlhDiminta']."</td>
	 <td>".$_SESSION['lang']['jmlh_disetujui']."</td>
	 <td>".$_SESSION['lang']['purchaser']."</td>
	 <td>".$_SESSION['lang']['lokasitugas']."</td>
	...
	$offset=$page*$limit;
	$sql="select count(*) as jmlhrow FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
               WHERE a.close = '2' and b.status='0' and b.create_po='0' and substr(tanggal,1,4)='".$thnSkrng."' ORDER BY a.tanggal desc";
	//echo $sql;
	$query=mysql_query($sql) or die(mysql_error());
	...
	$str="SELECT   a.`tanggal`, a.`persetujuan1`, a.`persetujuan2`, a.`persetujuan3`, a.`persetujuan4`, a.`persetujuan5`, a.`close`, a.`hasilpersetujuan1`, a.`hasilpersetujuan2`, a.`hasilpersetujuan3`, a.`hasilpersetujuan4`, a.`hasilpersetujuan5`, a.`tglp1`, a.`tglp2`, a.`tglp3`, a.`tglp4`, a.`tglp5`,b.*  FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
                WHERE a.close = '2' and b.status='0' and b.create_po='0' and substr(tanggal,1,4)='".$thnSkrng."'   ORDER BY a.tanggal desc limit ".$offset.",".$limit."";
//	$str="SELECT   a.`tanggal`, a.`persetujuan1`, a.`persetujuan2`, a.`persetujuan3`, a.`persetujuan4`, a.`persetujuan5`, a.`close`, a.`hasilpersetujuan1`, a.`hasilpersetujuan2`, a.`hasilpersetujuan3`, a.`hasilpersetujuan4`, a.`hasilpersetujuan5`, a.`tglp1`, a.`tglp2`, a.`tglp3`, a.`tglp4`, a.`tglp5`,b.*,c.nopo FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp LEFT JOIN ".$dbname.".log_podt c ON b.nopp=c.nopp  
//            WHERE a.close = '2' and b.status='0' and b.create_po='0' and (c.nopo IS NULL or c.nopo='') ".$where." ORDER BY a.tanggal desc limit ".$offset.",".$limit."";
	if(mysql_query($str))
	{
            $res=mysql_query($str);
		$total=mysql_num_rows($res);
		echo"<tr><td colspan=16>Total Barang: ".$jlhbrs."</td></tr>";
		while($bar=mysql_fetch_object($res))
		{			
			$koderorg=substr($bar->nopp,15,4);	
...
//			$jmlHari=date("j", $diff) - 1;
			...
			$optPur="<option value=''></option>";
//			$klq="select * from ".$dbname.".`datakaryawan` where  bagian='PUR' or kodejabatan='23' or kodejabatan='24' order by namakaryawan asc ";
			$klq="select karyawanid,namakaryawan from ".$dbname.".`datakaryawan` where  (bagian='PUR'or kodejabatan='120')  and (tanggalkeluar>'".date('Y-m-d')."' or  tanggalkeluar='0000-00-00') order by namakaryawan asc ";
 //echo $klq;
			$qry=mysql_query($klq) or die(mysql_error());
			...
			echo"<tr class=rowcontent id='tr_".$no."'>
				  ...
				  if($stat==1)
				  {
					  echo "<td align=center colspan=5>". $nopo."</td>";
				  }
				  else 
				  {
//					  <select name=lokalpusat_".$no." id=lokalpusat_".$no." '".$ckh."'>".$optLokasi."</select>
						  echo"
						  <td align=center><input type=checkbox id=lokalpusat_".$no."  ".$ckh." /> Lokal</td>
						    <td align=center title=\"Selisih Tanggal PP dengan Tanggal Hari ini\" >".$jmlHari."</td>
						  <td ".$stat_view."><img src=images/save.png class=resicon  title='Save' onclick=\"AddPur('".$no."');\"></td>
						  <td align=center><img src=images/application/application_edit.png class=resicon  title='Edit Data' onclick=\"EditPur('".$no."');\"></td><td align=center><img src=images/pdf.jpg class=resicon  title='Print' onclick=\"masterPDF('log_prapoht','".$bar->nopp."','','log_slave_print_log_pp',event);\"></td>";
				 }
	 ...
	 <tbody>";
		...
		if($_GET['tglCari']!='')
		{
			...
			if($statPP==1)
			{
				$strx="SELECT  distinct a.`tanggal`, a.`persetujuan1`, a.`persetujuan2`, a.`persetujuan3`, a.`persetujuan4`, a.`persetujuan5`, a.`close`, a.`hasilpersetujuan1`, a.`hasilpersetujuan2`, a.`hasilpersetujuan3`, a.`hasilpersetujuan4`, a.`hasilpersetujuan5`, a.`tglp1`, a.`tglp2`, a.`tglp3`, a.`tglp4`, a.`tglp5`,b.*  
                                       FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
                                       WHERE a.close = '2' and b.status='0' and create_po!='0' ".$where."  ORDER BY a.nopp asc ";//echo $strx;			//echo "warning:".$strx; exit();
			...
			else if($statPP==0)
			{
				$strx="SELECT  distinct a.`tanggal`, a.`persetujuan1`, a.`persetujuan2`, a.`persetujuan3`, a.`persetujuan4`, a.`persetujuan5`, a.`close`, a.`hasilpersetujuan1`, a.`hasilpersetujuan2`, a.`hasilpersetujuan3`, a.`hasilpersetujuan4`, a.`hasilpersetujuan5`, a.`tglp1`, a.`tglp2`, a.`tglp3`, a.`tglp4`, a.`tglp5`,b.*   
                                       FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp 
                                       WHERE a.close = '2' and b.status='0'  and create_po='0'  ".$where."  ORDER BY a.nopp asc  ";//echo $strx;
		...
		//exit("Error:Pernah disini___".$strx);
		...
		{			
			...
			$bar->lokalpusat!=0?$chk="Lokal":$chk="Pusat";
                        $sPoDetHrg="select distinct hargasatuan from ".$dbname.".log_podt where  kodebarang='".$bar->kodebarang."' order by nopo desc";
				$qPoDetHrg=mysql_query($sPoDetHrg) or die(mysql_error());
				$rCekHrg=mysql_fetch_assoc($qPoDetHrg);
			$stream.="<tr>
 ...
 $dte=date("His");
  ...
  $gztralala = gzopen("tempExcel/".$nop_.".xls.gz", "w9");
	...
	// echo $kolom;
	 ...
	 <fieldset><legend>Persetujuan</legend>
					...
					echo"
						...
						<button class=mybutton onclick=forwardPP() title=\"Diajukan Kembali Untuk Verivikasi\" id=Ajukan >".$_SESSION['lang']['diajukan']."</button>
					
						<button class=mybutton onclick=cancel() title=\"Menutup Form Ini\">".$_SESSION['lang']['cancel']."</button>
						</td></tr></table><br /> 
	 ...
	 if($ar==5)
	 {
		 echo"warning:Tidak dapat diajukan lagi";
		 exit();
	 }
	 else
	 {
	...
	// echo"warning:".$sUp;exit();
	 ...
	 {
		 $tglSkrng=date("Y-m-d");
		 $sUpdatePP="update ".$dbname.".log_prapoht set komentar".$kolom."='".$comment."',hasilpersetujuan".$kolom."='3',tglp".$kolom."='".$tglSkrng."',persetujuan".$kolom."='".$userid."' where nopp='".$nopp."'";
		 if(mysql_query($sUpdatePP))
		 {
			 $sql3="update ".$dbname.".log_prapodt set status='3',ditolakoleh='".$userid."' where nopp='".$nopp."'";
	 ...
	 {
		 echo"warning:Hubungi Administrator";
		 exit();
	 }
	 break;
	...
	case 'rejected_some_input' :
		...
		if($rCek>1)
		{
			...
			{
				//exit("Error:a");
				$sql2="update ".$dbname.".log_prapodt set status='3',ditolakoleh='".$userid."',alasanstatus='".$alsnDtolak."' where".$where;
				//exit("Error:a".$sql2);
				if($query2=mysql_query($sql2))
				{
					echo"";
	...
	break;
       ...
       $tab.="Data Terakhir Bulan : <span id=tglPeriode>".$periode."</span><br /><br />";
       ...
       $tab.="<table border=0 cellspacing=1 cellpading=0><thead>
          ...
          // $tab.="<td rowspan='2' align=center>Total Item</td><td rowspan='2' align=center>Terbit PO</td><td rowspan='2' align=center>Outstanding PO</td><td rowspan='2' align=center>% Out TOtal</td></tr>";
           ...
           $tab.="<tr class=rowcontent><td colspan=2>Total All Item</td>";
            foreach($kdOrg as $brsData2 => $rData2)
            {
           ...
           $tab.="</tr>";
           //<td align=right>".number_format($totalSemua,0)."</td><td align=right>".number_format($totalBlm,0)."</td><td align=right>".$persenTot."</td></tr>"; 
          // $tab.="<tr class=rowcontent><td colspan=".$col.">Barang Belum di Verivikasi</td><td>".$rAll['jmlh']."</td></tr>"; 
        ...
        case'detailSum':
            ...
            $tab.="<fieldset><legend>Detail Summary</legend>";
...
//                    $sDt="select count(kodebarang) as jmlhPo,kodeorg,purchaser,substr(tanggal,1,7) as period from 
//                    ".$dbname.".log_podt a left join ".$dbname.".log_poht b  on a.nopo=b.nopo 
//                    where purchaser='".$userid."' and kodeorg='".$rData2['kodeorganisasi']."' and substr(tanggal,1,7)='".$rPeriode['periode']."'";
                   //exit("Error".$sDt);
                    $qDt=mysql_query($sDt) or die(mysql_error());
                    $rDt=mysql_fetch_assoc($qDt);
                    ...
                    ///echo $sDt2;
//                    $sDt2="select count(kodebarang) as jmlhPo2,kodeorg,purchaser,substr(tanggal,1,7) as period from 
//                    ".$dbname.".log_podt a left join ".$dbname.".log_poht b  on a.nopo=b.nopo 
//                    where purchaser='".$userid."' and kodeorg='".$rData2['kodeorganisasi']."' and stat_release=1 and substr(tanggal,1,7)='".$rPeriode['periode']."'";
                    $qDt2=mysql_query($sDt2) or die(mysql_error());
                    $jmlhPo2=mysql_num_rows($qDt2);
                    $rDt2=mysql_fetch_assoc($qDt2);
...
//                }
	 ...
	 <td bgcolor=#DEDEDE align=center valign=middle>".$_SESSION['lang']['nopp']."</td>
         <td bgcolor=#DEDEDE align=center valign=middle>".$_SESSION['lang']['kodebarang']."</td>
	 <td bgcolor=#DEDEDE align=center valign=middle>".$_SESSION['lang']['namabarang']."</td>
 	 <td bgcolor=#DEDEDE align=center valign=middle>".$_SESSION['lang']['tanggal']." PP</td>
	 <td bgcolor=#DEDEDE align=center valign=middle>".$_SESSION['lang']['jmlhDiminta']."</td>
	 <td bgcolor=#DEDEDE align=center valign=middle>".$_SESSION['lang']['jmlh_disetujui']."</td>
	 ...
	 <tbody>";
        $sql="SELECT   kodeorg,b.purchaser,jumlah,realisasi,a.tanggal,b.kodebarang,b.nopp FROM 
            ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp
              WHERE  b.status='0'  and kodeorg='".$kodeorg."' and substr(tanggal,1,7) like '%".$periode."%' and purchaser='".$userid."'";
       // exit("error".$sql);
//$sql="SELECT   kodeorg,purchaser,jumlah,realisasi,a.tanggal,b.kodebarang,b.nopp FROM 
//".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp
//LEFT JOIN ".$dbname.".log_podt c ON b.nopp = c.nopp  
//WHERE  b.status='0' and purchaser='".$userid."' and kodeorg='".$kodeorg."' and substr(tanggal,1,7) like '%".$periode."%' and c.nopo!='' group by b.kodebarang";
        if(mysql_query($sql))
	{
	$res=mysql_query($sql);
		while($bar=mysql_fetch_object($res))
         ...
         case 'dataDetailEx':
           ...
           // exit("Error".$sPur);
            ...
            while($rPeriode=mysql_fetch_assoc($qPeriode))
            {
                ...
                foreach ($qData as $brsData2 =>$rData2)
                {
                    $sDt="SELECT   count(kodebarang) as jmlhPo,kodeorg,b.purchaser,substr(tanggal,1,7) as periode FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp
                                    WHERE  b.status='0' and kodeorg='".$rData2['kodeorganisasi']."' and substr(tanggal,1,7) like '%".$rPeriode['periode']."%' and c.nopo='' and purchaser='".$userid."' group by b.kodebarang";
//                    $sDt="select count(kodebarang) as jmlhPo,kodeorg,purchaser,substr(tanggal,1,7) as period from 
//                    ".$dbname.".log_podt a left join ".$dbname.".log_poht b  on a.nopo=b.nopo 
//                    where purchaser='".$userid."' and kodeorg='".$rData2['kodeorganisasi']."' and substr(tanggal,1,7)='".$rPeriode['periode']."'";
                   //exit("Error".$sDt);
                    ...
                    $sDt2="SELECT   kodeorg,b.purchaser,substr(tanggal,1,7) as periode FROM ".$dbname.".log_prapoht a LEFT JOIN ".$dbname.".log_prapodt b ON a.nopp = b.nopp
                           LEFT JOIN ".$dbname.".log_podt c ON b.nopp = c.nopp  
                           WHERE  b.status='0' and kodeorg='".$rData2['kodeorganisasi']."' and substr(tanggal,1,7) like '%".$rPeriode['periode']."%' and c.nopo!='' and purchaser='".$userid."'  group by b.kodebarang ";
//                    $sDt2="select count(kodebarang) as jmlhPo2,kodeorg,purchaser,substr(tanggal,1,7) as period from 
//                    ".$dbname.".log_podt a left join ".$dbname.".log_poht b  on a.nopo=b.nopo 
...
//			$klq="select * from ".$dbname.".`datakaryawan` where  bagian='PUR' or kodejabatan='23' or kodejabatan='24' order by namakaryawan asc ";
 ...
 //echo $klq;
	 ...
	 <tr><td colspan=3><button class=mybutton onclick=saveSemua2(1) id=saveAll2 title=Simpan Semua>".$_SESSION['lang']['save']." ".$_SESSION['lang']['all']."</button><button class=mybutton onclick=cancel()>".$_SESSION['lang']['cancel']."</button></td></tr>
	 </tbody>
	 </table><br />";
	...
	else
	{
	 ...
	 if($purchaser=='')
	 {
		 exit("Error:Purchaser Tidak Boleh Kosong");
	 }
