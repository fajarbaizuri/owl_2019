...
switch($proses) {
    case 'list' :
       ...
       $cols1 = "karyawanid,namakaryawan,tipekaryawan,kodejabatan,subbagian,bagian,tanggalmasuk,".
            "COALESCE(ROUND(DATEDIFF('".$tglTerakhir[1]."',tanggalmasuk)/365,2),0) as masakerja";
        $where1 = "tipekaryawan in (1,2,3) and alokasi=0 and lokasitugas='".
        ...
        $cols2 = "karyawanid,namakaryawan,tipekaryawan,kodejabatan,subbagian,bagian,tanggalmasuk,".
            "COALESCE(ROUND(DATEDIFF('".$tglTerakhir[1]."',tanggalmasuk)/365,2),0) as masakerja";
        $where2 = "tipekaryawan in (1,2,3) and alokasi=0 and lokasitugas='".
            $_SESSION['empl']['lokasitugas']."' and sistemgaji='".$param['jnsGaji']."' and
            (tanggalkeluar>'".$row['tanggalmulai']."' or tanggalkeluar='0000-00-00')";
...
//                }
                ...
                if($rGaji['jumlah']!=''&&$rGaji['jumlah']!=0)
                {
                    ...
                    and idkomponen in (select distinct id from ".$dbname.".sdm_ho_component where plus=1) and tahun=".$param['tahun'];
...
//        $whereGj .= ") and idkomponen in (".$param['jenis'].") and kodeorg='".
//            $_SESSION['empl']['lokasitugas']."' and periodegaji='".$param['periodegaji']."'";
//        $qGj = selectQuery($dbname,'sdm_gaji','karyawanid,idkomponen,pengali,jumlah',$whereGj);
//        $tmpGj = fetchData($qGj);
//        $baris=count($tmpGj);
//        if($baris=='')
//        {
...
//            }
//            }
//            $whereGj .= ") and idkomponen in (select distinct id from ".$dbname.".sdm_ho_component where plus=1) and kodeorg='".
//            $_SESSION['empl']['lokasitugas']."' and periodegaji='".$param['periodegaji']."'";
//            //$qGj = selectQuery($dbname,'sdm_gaji','karyawanid,idkomponen,pengali,jumlah',$whereGj);
//            $qGj="select karyawanid,idkomponen,pengali,sum(jumlah) as jumlah from ".$dbname.".sdm_gaji where ".$whereGj." group by karyawanid";
//            $tmpGj = fetchData($qGj);
//        }
        $resGj = array();
            ...
            if(isset($row[$param['jenis']])) {
                ...
                $data[$i]['pengali'] = $row[$param['jenis']]['pengali'];
				 ...
				 //exit("Error".$param['jenis']);
					...
					if($msKerja<12&&$msKerja>=3)
					{
					  $data[$i]['gajipokok']=$row[1]['jumlah'];					  $row['1']['pengali']=$msKerja/12;
					}
					elseif($msKerja>=12)
					{
						$row['1']['pengali']=1;
...
//            $dataShow[$key]['subbagian'] = $optNmorg[$row['subbagian']];
            $dataShow[$key]['id'] = $optKary[$row['id']]['nama'];
            $dataShow[$key]['tipe'] = $optTipe[$row['tipe']];
        ...
        #======== Make Table ==========
        $tmpColTab = 'namakaryawan,tipekaryawan,lokasitugas,bagian,kodejabatan,tmk,masakerja,gajipokok,pengali,jumlah,action';
        ...
        foreach($dataShow as $key=>$row) {
            ...
            {
            if($dataExist[$key]==1) {
                $table .= "<tr id='tr_".$key."' class='rowcontent' style='background:#00FF00'>";
            } else {
...
//                echo"</pre>";
                ...
                if($head=='pengali') {
                    $table .= makeElement($head.'_'.$key.'_text','textnum',$cont,
                        array('style'=>'width:70px;text-align:center','onkeyup'=>"updJumlah(".$key.")"));
                } elseif($head=='jumlah') {
                    $table .= makeElement($head.'_'.$key.'_text','textnum',$cont,
            ...
            # Action
            $table .= "<td>";
            $table .= makeElement('save_'.$key,'btn',$_SESSION['lang']['save'],
                array('onclick'=>"saveIt(".$key.")"));
            $table .= "</td>";
            $table .= "</tr>";
        }
        }
    ...
    case 'post':
        ...
        if($rCek['tutupbuku']!=0)
        {
            echo"warning:Data Tidak Dapat Tersimpan, Periode Sudah Tidak Aktif";
            exit();
        }
        # Insert
