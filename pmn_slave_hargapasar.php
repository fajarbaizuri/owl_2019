...
$where="tanggal='".$tglHarga."' and kodeproduk='".$kdBarang."' and pasar='".$idPasar."'";
	switch($proses)
	{
            ...
            case'insert':
                ...
                if($rCek<1)
                {
            $sIns="insert into ".$dbname.".pmn_hargapasar (tanggal, kodeproduk, pasar, satuan, harga, matauang) 
                   values ('".$tglHarga."','".$kdBarang."','".$idPasar."','".$satuan."','".$hrgPasar."','".$idMatauang."')";
                if(!mysql_query($sIns))
		...
		if($res=mysql_query($str))
		{
                    ...
                    if($barisData>0)
                    {
                        ...
                        <td><img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"fillField('".tanggalnormal($bar->tanggal)."','".$bar->kodeproduk."','".$bar->satuan."','".$bar->pasar."','".$bar->matauang."','".$bar->harga."');\">
