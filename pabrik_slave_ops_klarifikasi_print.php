...
switch($proses) {
    case 'all':
		...
		## Get Data
		$q = selectQuery($dbname,'pabrik_ops_klarifikasiht','*',"tanggal between '".
    ...
    case 'detail':
		$align = array('C','C','C','C');
		$length = array(25,25,25,25);
		$cols = "waktu,vaccumtemp,vaccumtekanan,purifier1temp";
		$colArr = explode(',',$cols);
		...
		$q = selectQuery($dbname,'pabrik_ops_klarifikasidt',$cols,"notransaksi='".
			$param['notransaksi']."'");
		$data = fetchData($q);
