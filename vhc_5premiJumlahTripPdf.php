...
//			$this->Cell(80,5,$_SESSION['standard']['username'],'',1,'L');
	...
	$pdf->SetY(40);
	$pdf->Cell(178,5,$_SESSION['lang']['premiTransportRit'],0,1,'C');	
	$pdf->Ln();	
	$pdf->SetFont('Arial','B',9);	
	$pdf->SetFillColor(220,220,220);
    $pdf->Cell(8,5,'No',1,0,'L',1);
	$pdf->Cell(30,5,$_SESSION['lang']['keycode'],1,0,'C',1);	
	$pdf->Cell(20,5,$_SESSION['lang']['nomor'],1,0,'C',1);
	$pdf->Cell(35,5,$_SESSION['lang']['vhc_jenis_pekerjaan'],1,0,'C',1);
	$pdf->Cell(35,5,$_SESSION['lang']['jumlahtrip'],1,0,'C',1);
	$pdf->Cell(25,5,$_SESSION['lang']['vhc_posisi'],1,0,'C',1);		
	$pdf->Cell(25,5,$_SESSION['lang']['nilaipremi'],1,1,'C',1);
	
	
	//$pdf->Cell(25,5,'Total',1,1,'C',1);
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','',9);
		$str="select keycode,nomor,tipeangkutan,jumlahtrip,rate,jobposition from ".$dbname.".kebun_5ratetransport where jarakdari='' order by nomor asc";
		$re=mysql_query($str);
		$no=0;
		...
		while($res=mysql_fetch_assoc($re))
		{
			...
			$pdf->Cell(8,5,$no,1,0,'L',1);
			$pdf->Cell(30,5,$res['keycode'],1,0,'C',1);	
			$pdf->Cell(20,5,$res['nomor'],1,0,'C',1);
			$pdf->Cell(35,5,$res['tipeangkutan'],1,0,'C',1);
			$pdf->Cell(35,5,$res['jumlahtrip'],1,0,'C',1);
			$pdf->Cell(25,5,$arrPos[$res['jobposition']],1,0,'L',1);		
			$pdf->Cell(25,5,number_format($res['rate'],2),1,1,'R',1);
		}
