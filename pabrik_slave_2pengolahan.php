...
switch($level) {
    case '0':
        # Data
		/*
        $cols = "a.tanggal,sum(jamstagnasi) as stagnasi,sum(jamdinasbruto) as bruto,sum(jumlahlori) as lori,sum(a.tbsdiolah) as diolah,sum(oer) as cpo,sum(oerpk) as pk,nopengolahan";
        $cols2 = "nomor,tanggal,jamstagnasi,jamoperasional,jumlahlori,tbsdiolah,cpo,kernel,detail";
        $cols2e = "nomor,tanggal,jamstagnasi,jamoperasional,jumlahlori,tbsdiolah,cpo,kernel";
       ...
       // $query = selectQuery($dbname,'pabrik_pengolahan',$cols,$where)." group by tanggal";
        $query="select distinct ".$cols." from ".$dbname.".pabrik_pengolahan a left join ".$dbname.".pabrik_produksi b 
				 ...
				 */
		$cols = "a.tanggal,sum(a.jamstagnasi) as stagnasi,sum(a.jamdinasbruto) as bruto,sum(a.jumlahlori) as lori,sum(a.tbsdiolah) as diolah,b.oer as cpo,b.oerpk as pk,nopengolahan";
        $cols2 = "nomor,tanggal,jamstagnasi,jamoperasional,jumlahlori,tbsdiolah,cpo,kernel,detail";
        $cols2e = "nomor,tanggal,jamstagnasi,jamoperasional,jumlahlori,tbsdiolah,cpo,kernel";
        $where = "a.kodeorg='".$param['kodeorg']."' and left(a.tanggal,7)='".$currPeriod."'";
       // $query = selectQuery($dbname,'pabrik_pengolahan',$cols,$where)." group by tanggal";
...
switch($mode) {
    ...
    $j=0;    
        foreach($data as $key=>$row) {
            ...
            foreach($row as $head=>$cont) {
                {
                        ...
                        if(($i==1)){ // jam stagnasi
                                $jamstag=fixHours($dataShow[$key][$head]);
                                if($jamstag=="0:00")$jamstag="";
                        ...
                        if(($i==7)){ // detail
                                if(($mode!='excel')){
                                $tab .= "<td align='".$alignPrev[$i]."'><img onclick=\"viewDetail('".$dataShow[$key][$head]."','".$tanggal."','".$kodeorg."','".$periode_tahun."','".$periode_bulan."',event);\" title=\"Click untuk melihat detail\" class=\"resicon\" src=\"images/icons/clipboard_sign.png\"></td>";					
                                }
                        }else{ // lori & tbs
            ...
            $tab .= "</tr>";
        $tdiolah+=$dataShow[$key]['diolah'];
       $tcpo+=$dataShow[$key]['cpo'];
       $tpk+=$dataShow[$key]['pk'];   
        }     
        $tab.="<tr><td colspan=5>Total</td>
                    <td>".number_format($tdiolah)."</td>
                    <td>".number_format($tcpo)."</td>
                    <td>".number_format($tpk)."</td> 
                     </tr>";   
        $tab .= "</tbody>";
        $tab .= "</table>";
        /** Output Type **/
