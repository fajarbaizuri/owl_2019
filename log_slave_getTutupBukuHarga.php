...
//dicancel karena hanya akan menghitung yang sudah di posting saja pada table saldo bulanan
			...
			$stra="select distinct kodegudang from ".$dbname.".log_transaksiht where  tanggal>='".$awal."' and tanggal<='".$akhir."' and post=0";		      
			$resA=mysql_query($stra);
			$jlhNotPost=0;
			$jlhNotPost=mysql_num_rows($resA);	
			
			if($jlhNotPost>0)
			{
				while($bar=mysql_fetch_object($resA))
				{
					echo "Gudang ".$bar->kodegudang.",\n";
				}
