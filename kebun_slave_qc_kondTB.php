...
$notransaksi=$afdelingId."/".$thnNo[2].$thnNo[1].$thnNo[0]."/KLTB";
...
$hpt=$_POST['hpt'];
	...
	switch($proses)
	{
            ...
            $sCek="select distinct notransaksi from ".$dbname.".kebun_qc_kondisitbht where notransaksi='".$notransaksi."'";
            $qCek=mysql_query($sCek) or die(mysql_error());
            $rcek=mysql_num_rows($qCek);
            if($rcek<1)
            {
                $sInsert="insert into ".$dbname.".kebun_qc_kondisitbht (notransaksi, afdeling, tanggal, asisten,petugas,updateby) 
            ...
            if($afdelingId==''||$idPetugas==''||$tgl==''||$idAsisten=='')
            {
             ...
             $sInsert="update ".$dbname.".kebun_qc_kondisitbht set asisten='".$idAsisten."', petugas='".$idPetugas."', updateby='".$_SESSION['standard']['userid']."' where notransaksi='".$notransaksi."'";
            if(!mysql_query($sInsert))
            {
                echo "DB Error : ".mysql_error($conn);
            }
            ...
            $ql2="select count(*) as jmlhrow from ".$dbname.".kebun_qc_kondisitbht a left join ".$dbname.".kebun_qc_kondisitbdt b on a.notransaksi=b.notransaksi
            ...
            if($jlhbrs!=0)
            {
                $sData="select distinct b.*,a.* from ".$dbname.".kebun_qc_kondisitbht a left join ".$dbname.".kebun_qc_kondisitbdt b on a.notransaksi=b.notransaksi
                ...
                while($rData=  mysql_fetch_assoc($qData))
                {
                    ...
                    $tab.="<td align=left>".$rData['chipping']."</td>";
                    $tab.="<td align=left>".$rData['stcking']."</td>";
		...
		<button class=mybutton onclick=cariPage(".($page+1).");>".$_SESSION['lang']['lanjut']."</button>
            ...
            $sCek="select distinct notransaksi,kodeorg from ".$dbname.".kebun_qc_kondisitbdt 
            ...
            if($rcek<1)
            {
                $sInsert="insert into ".$dbname.".kebun_qc_kondisitbdt (notransaksi, kodeorg, chipping, stcking, teras, kacangan) 
            ...
            $sInsert="update ".$dbname.".kebun_qc_kondisitbdt set chipping='".$piringan."', stcking='".$gwngan."', teras='".$pmupukn."', kacangan='".$hpt."'
                      where notransaksi='".$noTrans."' and kodeorg='".$kdBlok."'";
            //exit("Error".$sInsert);
                if(!mysql_query($sInsert))
                {
            ...
            break;
            case'delDetail':
                $sDel="delete from ".$dbname.".kebun_qc_kondisitbdt where notransaksi='".$noTrans."' and kodeorg='".$kdBlok."'";
                 if(!mysql_query($sDel))
                {
                    echo "DB Error : ".mysql_error($conn);
                }
            break;
            case'loadDetail':
            $sDetail="select distinct * from ".$dbname.".kebun_qc_kondisitbdt where notransaksi='".$noTrans."'";
            //    exit("Error".$sDetail);
            ...
            break;
            case'getDetail':
            $sData="select distinct * from ".$dbname.".kebun_qc_kondisitbdt where notransaksi='".$noTrans."' and kodeorg='".$kdBlok."'";
            $qData=mysql_query($sData) or die(mysql_error());
            $rData=mysql_fetch_assoc($qData);
            echo $rData['kodeorg']."###".$rData['chipping']."###".$rData['stcking']."###".$rData['teras']."###".$rData['kacangan'];
            break;
            case'delDetailAll':
             $sDel="delete from ".$dbname.".kebun_qc_kondisitbht where notransaksi='".$noTrans."'";
                 if(!mysql_query($sDel))
                {
                    echo "DB Error : ".mysql_error($conn);
                }
