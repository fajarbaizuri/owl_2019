...
$sBln="select distinct sum(jumlahpesan*hargasatuan) as total,matauang,kodebarang,sum(jumlahpesan) as jumlahpesan,
       satuan,matauang,substr(nopp,16,4) as unit
       from ".$dbname.".log_po_vw where substr(kodebarang,1,3)='311' 
       and tanggal like '".$periode."%' group by kodebarang,matauang,substr(nopp,16,4)   order by namabarang asc";
//echo $sBln;
$qBln=mysql_query($sBln) or die(mysql_error($conn));
while($rBln=mysql_fetch_assoc($qBln))
{
    $dtBrg[$rBln['kodebarang']]=$rBln['kodebarang'];
    $dtUang[$rBln['kodebarang'].$rBln['unit'].$rBln['matauang']]=$rBln['total'];
    $dtSatuan[$rBln['kodebarang'].$rBln['matauang']]=$rBln['satuan'];
    $dtJmlh[$rBln['kodebarang'].$rBln['unit']]=$rBln['jumlahpesan'];
    $dtMata[$rBln['matauang']]=$rBln['matauang'];
    if($rBln['jumlahpesan']!='0')
    {
        $dtUnit[$rBln['unit']]=$rBln['unit'];
    }
}
//$sBln="select distinct sum(jumlahpesan*hargasatuan) as total,matauang,kodebarang,sum(jumlahpesan) as jumlahpesan,matauang,substr(nopp,16,4)  as unit
//       from ".$dbname.".log_po_vw where substr(kodebarang,1,3)='311' 
...
//{
//    $dtBrg[$rBln['kodebarang']]=$rBln['kodebarang'];
//    $dtUangBlnini[$rBln['kodebarang'].$rBln['unit'].$rBln['matauang']]=$rBln['total'];
//    $dtJmlhblnIni[$rBln['kodebarang'].$rBln['unit']]=$rBln['jumlahpesan'];
//    $dtMata[$rBln['matauang']]=$rBln['matauang'];
//    $dtUnit[$rBln['unit']]=$rBln['unit'];
//}

$itungMtuang=count($dtMata);
$bg="";
$brdr=0;
if($proses=='excel')
{
...
if($proses!='excel')$tab.=$judul;
    ...
    {
        $tab.="<td colspan='".($itungMtuang+1)."' ".$bg.">".$lsUnit."</td>";
    }
    $tab.="</tr><tr>";
    foreach($dtUnit as $lsUnit3)
    {
        $tab.="<td  align=center ".$bg.">".$_SESSION['lang']['jumlah']."</td>";
        foreach($dtMata as $lstMat)
        {
            $tab.="<td  align=center ".$bg.">".$lstMat."</td>";
        }
    }
    //$optNmBrg
    $tab.="</tr></thead><tbody>";
    foreach($dtBrg as $lstKdbarang)
    {
        ...
        foreach($dtUnit as $lsUnit2)
        {
            $tab.="<td align=right>".number_format($dtJmlh[$lstKdbarang.$lsUnit2],0)."</td>";
            foreach($dtMata as $lstMata3)
            {
                $tab.="<td align=right>".number_format($dtUang[$lstKdbarang.$lsUnit2.$lstMata3],0)."</td>";
                $totNil[$lsUnit2.$lstMata3]+=$dtUang[$lstKdbarang.$lsUnit2.$lstMata3];
            }
            $totJmlh[$lsUnit2]+=$dtJmlh[$lstKdbarang.$lsUnit2];
        }
    }
    $tab.="<tr class=rowcontent>";
    ...
    foreach($dtUnit as $lsUnit3)
    {
        $tab.="<td align=right>".number_format($totJmlh[$lsUnit3],0)."</td>";
        foreach($dtMata as $lstMata3)
        {
            $tab.="<td align=right>".number_format($totNil[$lsUnit3.$lstMata3],0)."</td>";
        }
    }
    $tab.="</tr>";
    $tab.="</tbody></table>";
