...
switch($level) {
    case '0':
        ...
        # Mutasi Blok
        $cols = "kodeorg,tahuntanam,kodeorg,statusblok,luasareanonproduktif,luasareaproduktif,jumlahpokok,cadangan,okupasi,rendahan,sungai,rumah,kantor,pabrik,jalan,kolam,umum,tanggalpengakuan";
	 ...
	 else
	 {
        ...
        $query = selectQuery($dbname,'setup_blok',$cols,$where,"tahuntanam,kodeorg asc");
...
switch($mode) {
    case 'pdf':
        ...
        $align = explode(",","R,L,L,L,R,R,R,R,R");
        $length = explode(",","5,15,10,10,10,10,15,15,10");
    ...
    default:
        if($mode=='excel') {
            ...
            $tab = "<table id='arealstatement' class='sortable'>";
        ...
        foreach($data as $afd=>$row) {
            ...
            # Sub Total
            $tab .= "<tr class='rowcontent'>";
            $tab .= "<td colspan='4' align='right'><b>Sub Total Tahun Tanam ".$afd."</b></td>";
            foreach($total[$afd] as $head=>$val) {
                if($head=='kerapatan') {
        ...
        # Grand Total
        $tab .= "<tr class='rowcontent'>";
        $tab .= "<td colspan='3' align='right'><b>Grand Total</b></td>";
        foreach($gTotal as $head=>$val) {
            if($head=='kerapatan') {
                $tab .= "<td align='right'><b>".number_format($val,0)."</b></td>";
            } else {
