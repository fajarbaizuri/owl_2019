...
$lokasi=$_SESSION['empl']['lokasitugas'];
	...
	switch($proses)
	{
		...
		$ql2="select count(*) as jmlhrow from ".$dbname.".kebun_curahhujan where `kodeorg` = '".$lokasi."' order by `tanggal` desc";// echo $ql2;
		...
		$str="select * from ".$dbname.".kebun_curahhujan where `kodeorg` = '".$lokasi."' order by tanggal desc limit ".$offset.",".$limit."";
		...
		*/
		...
		if($rCek<1)
		{
			$sIns="insert into ".$dbname.".kebun_curahhujan (kodeorg, tanggal, pagi, sore, catatan) values ('".$kdORg."','".$daTtgl."','".$daTpagi."','".$daTsore."','".$note."')";
			if(mysql_query($sIns))
			echo"";
			else
		...
		if(($kdORg=='')||($daTpagi=='')||($daTsore=='')||($daTtgl=='')||($note==''))
		{
			...
			$sUpd="update ".$dbname.".kebun_curahhujan set  pagi='".$daTpagi."', sore='".$daTsore."', catatan='".$note."' where  kodeorg='".$kdORg."' and tanggal='".$daTtgl."'";
