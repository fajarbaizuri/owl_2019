...
# Absensi
$queryAbs = selectQuery($dbname,'kebun_kehadiran','jhk,umr,insentif',"notransaksi='".$param['notransaksi'.$AYRA]."'");
...
$selisihpost=abs($totalHk-$hkPres);
//exit("Error:$selisihpost");

#=== Cek if HK belum sama ===
//if(empty($dataAbs) or ($totalHk!=$hkPres)) {
if(empty($dataAbs) or ($selisihpost > 0.05)) {
...
if(empty($tmpKonter)) {
    ...
    $insParam = insertQuery($dbname,'keu_5kelompokjurnal',$dataParam);
    if(!mysql_query($insParam)) {
    ...
    $noCounter = $tmpKonter[0]['nokounter']+1;
}
...
foreach($dataD as $row) {
	...
	$totalJumlah += $costRawat * $row['jumlahhk'];
    $noUrut++;
}
