...
class PDF extends FPDF
{
	...
	$pdf->Cell(22,4,$_SESSION['lang']['kodenopol'],1,0,'C',1);
        $pdf->Cell(30,4,$_SESSION['lang']['transportasi'],1,0,'C',1);
	...
	elseif($kdBrg!='0')
	{
		...
		while($res=mysql_fetch_assoc($re))
		{
		   	...
		   	if($res['kodecustomer']!='')
			{
				...
				elseif($res['kodebarang']=='40000003')
				{
					...
					$tes3=mysql_fetch_assoc($tes2);
						...
						{
							$sSupp="select namasupplier  from ".$dbname.".log_5supplier where kodetimbangan='".$res['kodecustomer']."'";
						}
			...
			$pdf->Cell(22,4,$res['nokendaraan'],1,0,'L',1);
                        $pdf->Cell(30,4,$rTRP['TRPNAME'],1,0,'L',1);
		...
		$pdf->Cell(73+$tmb,4,'Total',1,0,'R',1);
