...
switch($proses) {
    case 'showDetail':
		...
		if(empty($optSj)) {
			...
			$nilai=makeOption($dbname,'log_baspk','jumlahrealisasi,jumlahrealisasi',"notransaksi='".$param['nopo']."' and statusjurnal='1'",$mode='0');	
			$kdblok=makeOption($dbname,'log_baspk','kodeblok,kodeblok',"notransaksi='".$param['nopo']."' and statusjurnal='1'",$mode='0');
			//$kdkeg=makeOption($dbname,'log_baspk','kodekegiatan,kodekegiatan',"notransaksi='".$param['nopo']."' and statusjurnal='1'",$mode='0');
			$tgl=makeOption($dbname,'log_baspk','tanggal,tanggal',"notransaksi='".$param['nopo']."' and statusjurnal='1'",$mode='0');
	...
	//	$namakeg=makeOption($dbname,'setup_kegiatan','kodekegiatan,namakegiatan',"kodekegiatan='".$kdkeg."'");
		...
		else
		{
			...
			$theForm2->_elements[1]->_attr['onchange'] = "getkeg()";
		}
		
		$theForm2->addEls('kodeblok',$_SESSION['lang']['kodeblok'],'','select','L',25,$kdblok);
		$theForm2->_elements[2]->_attr['onchange'] = "getkeg()";
		$theForm2->addEls('kodekegiatan',$_SESSION['lang']['kodekegiatan'],'','text','L',25);
		$theForm2->_elements[3]->_attr['disabled'] = 'disabled';
		$theForm2->addEls('tanggal2',$_SESSION['lang']['tanggal2'],'','select','L',25,$tgl);
    ...
    case 'edit':
		...
		$where = "noinvoice='".$param['noinvoice']."' and nosj='".
			...
			/*		$where = "noinvoice='".$param['noinvoice']."' and nosj='".
			$param['cond_nosj']."'and nilai='".$param['nilai']."' and kodeblok='".$param['kodeblok']."'and kodekegiatan='".$param['kodekegiatan']."'and tanggal2='".$param['tanggal2']."' ";*/
    ...
    case 'delete':
		$where = "noinvoice='".$param['noinvoice']."' and nosj='".$param['nosj']."' and nilai='".$param['nilai']."' and kodeblok='".$param['kodeblok']."'and kodekegiatan='".$param['kodekegiatan']."'and tanggal2='".$param['tanggal2']."'";
...
case 'getkeg':
	//exit("Error:MASUK");
		$sql="select kodeblok,kodekegiatan,tanggal from ".$dbname.".log_baspk WHERE notransaksi='".$param['nosj']."' and kodeblok='".$param['kodeblok']."' and jumlahrealisasi='".$param['nilai']."' ";	
		//echo $sql;
		$qry=mysql_query($sql) or die(mysql_error());
		$data=mysql_fetch_assoc($qry);
		echo $data['kodekegiatan'];
	break;
