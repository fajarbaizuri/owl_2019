...
$oldBlok=$_POST['oldBlok'];
	...
	switch($proses)
	{
		...
		case'amblBjr':
		$qBlok = "select tahuntanam from ".$dbname.".setup_blok where kodeorg='".$blok."'";
		$resBlok = fetchData($qBlok);
		
		$sStpBlok="select bjr from ".$dbname.".kebun_5bjr where kodeorg='".
			substr($blok,0,6)."' and tahunproduksi=".substr($_POST['periode'],0,4).
			" and thntanam=".$resBlok[0]['tahuntanam'];
		//echo $sStpBlok;
...
*/		
		...
		$periode=$tglThn."-".$tglBln;
			...
			echo"
			<td><img src=images/pdf.jpg class=resicon  title='Print' onclick=onclick=\"masterPDF('kebun_spbht','".$rlvhc['nospb']."','','kebun_spbPdf',event);\"></td>";
			}
