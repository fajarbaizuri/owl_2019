...
$str="select noakun,namaakun from ".$dbname.".keu_5akun where detail=1 and (noakun like '211%')";
$res=mysql_query($str);
...
$str1="select noakun,namaakun from ".$dbname.".keu_5akun where detail=1 and (noakun like '212%')";
...
while($bar1=mysql_fetch_object($res1))
{
 ...
 echo"<fieldset>
      <legend>Input Akun Rekanan</legend>
	  <table>
	  <tr>
	      <td>Id.Supplier</td><td><input type=text class=myinputtext disabled id=idsupplier></td>
	      <td>Nama.Bank</td><td><input type=text class=myinputtext id=bank onkeypress=\"return tanpa_kutip(event);\" size=30 maxlength=30></td>
	  </tr>	  
	  <tr>    
	     <td>No.Akun</td><td><select id=noakun>".$opt."</select></td> 
          <td>No.Rek</td><td><input type=text class=myinputtext id=rek onkeypress=\"return tanpa_kutip(event);\" size=30 maxlength=30></td>	  
 	  </tr>
	  <tr>
	      <td>Nama.Supplier</td><td><input type=text class=myinputtext id=namasupplier onkeypress=\return tanpa_kutip(event);\" size=30 maxlength=30 disabled></td>
	      <td>A/c Bank.A/N</td><td><input type=text class=myinputtext id=an onkeypress=\"return tanpa_kutip(event);\" size=30 maxlength=30></td>
	  </tr>
	  <tr>
	      <td>Akun.Pajak</td><td><select id=akunpajak>".$opt1."</select></td>
	      <td>No.Seri.Pajak</td><td><input type=text class=myinputtext id=noseripajak onkeypress=\"return tanpa_kutip(event);\" size=30 maxlength=30></td>
	  </tr>
	  <tr>
	      <td>NilaiHutang</td><td colspan=3><input type=text  onblur=\"change_number(this);\"class=myinputtextnumber id=nilaihutang onkeypress=\"return angka_doang(event);\" size=15 maxlength=15 value=0></td>
	  </tr>
	  </table>
