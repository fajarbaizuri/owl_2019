...
switch($proses)
{
	...
	if($tipeIntex==0)
	{
			if($unit==''){
				...
				$sIan="select supplierid from ".$dbname.". log_5supplier where kodetimbangan='".$unit."'";
                $qIan=mysql_query($sIan) or die(mysql_error());
                $rIan=mysql_fetch_assoc($qIan);
                $nmIam=$rIan['supplierid'];
				$where.=" and kodecustomer in( '".$unit."','".$nmIam."')"; //echo $where;		
			}
	}
	else if($tipeIntex!=0)
	{
...
echo $_SESSION['lang']['rPenerimaanTbs'];	
	...
	if($kdPabrik!='' and $unit=='' and $tipeIntex=='' and $tgl_1!='' and $tgl_2!='')
	{
		//$sData="select jammasuk,jamkeluar,notransaksi,kodeorg,jumlahtandan1 as jjg,beratbersih as netto,kodecustomer,substr(tanggal,1,10) as tanggal,supir,nokendaraan,nospb,thntm1,kgpotsortasi from ".$dbname.".pabrik_timbangan where millcode='".$kdPabrik."' and kodebarang='40000003' and tanggal between '".$tgl_1."000000' and '".$tgl_2."235959'  order by tanggal,kodeorg,kodecustomer";
		$sData="select jammasuk,jamkeluar,notransaksi,kodeorg,jumlahtandan1 as jjg,beratbersih as netto,kodecustomer,substr(tanggal,1,10) as tanggal,supir,nokendaraan,nospb,thntm1,kgpotsortasi from ".$dbname.".pabrik_timbangan where millcode='".$kdPabrik."' and kodebarang='40000003' and tanggal between '".$tgl_1."000000' and '".$tgl_2."235959'  order by notransaksi,tanggal,kodeorg,kodecustomer asc";
		
	}
	else
	{
		$sData="select jammasuk,jamkeluar,notransaksi,kodeorg,jumlahtandan1 as jjg,beratbersih as netto,kodecustomer,substr(tanggal,1,10) as tanggal,supir,nokendaraan,nospb,thntm1,kgpotsortasi from ".$dbname.".pabrik_timbangan where kodebarang='40000003' ".$where." order by notransaksi,tanggal,kodeorg,kodecustomer asc";
					...
					if ($tipeIntex==""){
						if ($unit==""){
							...
							if ($rData['kodeorg']==""){
								...
								if (substr($rData['kodecustomer'],0,1)!="5")
								{
...
/*
					...
					else if($kdPabrik!='' and $unit=='' and $tipeIntex=='' and $tgl_1!='' and $tgl_2!='')//tambahan indra
					{
						//if($rData['kodecustomer
						if($rData['kodecustomer']!='')
						{
							$nm=$namasupp[$rData['kodecustomer']];
						}
						else
						{
							$nm=$namaorg[$rData['kodeorg']];
						}
...
*/
					...
					<td align=right>".$rData['jammasuk']."</td>
					<td align=right>".$rData['jamkeluar']."</td>
                    </tr>";
                    $subtotbjr+=$bjr;
                    $subtota+=$rData['netto'];
                    $subTnandn+=$rData['jjg'];
					...
					//$ratabjr=$subtotbjr/$no;
					$ratabjr=$subtota/$subTnandn; 
            }
            echo"<thead>
                    <tr class=rowcontent >
                            ...
                            <td align=right>".number_format($ratabjr,2)."</td>
							<td colspan=2 ></td>
                    </tr>";
...
*/
    ...
    if($kdPabrik!='' and $unit=='' and $tipeIntex=='' and $tgl_1!='' and $tgl_2!='')
	{
		$sData="select notransaksi,kodeorg,jumlahtandan1 as jjg,beratbersih as netto,kodecustomer,substr(tanggal,1,10) as tanggal,supir,nokendaraan,nospb,thntm1,kgpotsortasi from ".$dbname.".pabrik_timbangan where kodebarang='40000003' and tanggal between '".$tgl_1."000000' and '".$tgl_2."235959'  order by tanggal,kodeorg,kodecustomer";
		//echo $sData;exit();
	}
	else
	{
		$sData="select notransaksi,kodeorg,jumlahtandan1 as jjg,beratbersih as netto,kodecustomer,substr(tanggal,1,10) as tanggal,supir,nokendaraan,nospb,thntm1,kgpotsortasi from ".$dbname.".pabrik_timbangan where kodebarang='40000003' ".$where." order by tanggal,kodeorg,kodecustomer";
	}
	*/
	if($kdPabrik!='' and $unit=='' and $tipeIntex=='' and $tgl_1!='' and $tgl_2!='')
	{
...
if ($tipeIntex==""){
            ...
            $tab.="<tr class=rowcontent bgcolor=#DEDEDE><td colspan=7 align=center>Total (KG)</td>
                         ...
                         <td align=right>".number_format($subtota,2)."</td>
                          ...
                          <td align=right>".number_format($ratabjr,2)."</td></tr>";
    }
    else
    {
