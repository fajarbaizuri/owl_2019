...
//$frm[1]='';
?>
<script type="text/javascript" src="js/vhc_5premiJumlahTrip.js"></script>
...
/*$sOptKey="select keycode from ".$dbname.".setup_mappremi where kodeorg='".$_SESSION['empl']['lokasitugas']."'";
...
//echo "warning:".$lokasi;
$optKeyCode='';
$sOptKey="select keycode from ".$dbname.".setup_mappremi where kodeorg='".$lokasi."' and keycode like '%TRANS02%'";
$qOptKey=mysql_query($sOptKey) or die(mysql_error());
while($rOptKey=mysql_fetch_assoc($qOptKey))
{
	$optKeyCode.="<option value=".$rOptKey['keycode']." >".$rOptKey['keycode']."</option>";
...
OPEN_BOX('',"<b>".$_SESSION['lang']['premiTransportRit']."</b>");
$frm[0].="<fieldset><legend>".$_SESSION['lang']['entryForm']."</legend>";
$frm[0].="<table cellspacing=1 border=0>
<tr><td>".$_SESSION['lang']['keycode']."</td><td>:</td><td>
<select id=keyCode name=keyCode style=width:150px;  onchange=cekNomor() ><option value=''></option>".$optKeyCode."</select>
...
<tr><td>".$_SESSION['lang']['jumlahtrip']."</td><td>:</td><td>
<input type=text id=jmlhTrip name=jmlhTrip class=myinputtextnumber maxlength=8  onkeypress=\"return angka_doang(event);\" style=width:150px; /></td></tr>
...
<tr><td>".$_SESSION['lang']['nilaipremi']."</td><td>:</td><td>
<input type=text id=detPremi name=detPremi class=myinputtextnumber maxlength=8  onkeypress=\"return angka_doang(event);\" style=width:150px;  />
</td></tr>
<tr><td colspan=3>
...
$frm[0].="<fieldset><legend>".$_SESSION['lang']['datatersimpan']."</legend><table cellspacing=1 border=0>
		...
		<td>".$_SESSION['lang']['jumlahtrip']."</td>
		<td>".$_SESSION['lang']['nilaipremi']."</td>
		<td>".$_SESSION['lang']['vhc_posisi']."</td>
		<td>Action</td>
		</tr></thead>
		<img src=images/pdf.jpg class=resicon  title='Print' onclick=\"masterPDF('kebun_5ratetransport','','','vhc_5premiJumlahTripPdf',event);\">
		<tbody id=contain>
...
<tr><td>".$_SESSION['lang']['nomor']."</td><td>:</td><td>
<input type=text class=myinputtextnumber id=nomDetail name=nomDetail maxlength=5 style=width:150px;  disabled/></td></tr>
