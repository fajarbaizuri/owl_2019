...
//array tahun tanam
$sThnTnm="select distinct thntnm from ".$dbname.".bgt_produksi_kbn_kg_vw where 	kodeunit='".$kdUnit."' and tahunbudget='".$thn[0]."'  order by thntnm asc";
if($afdId!='')
{
...
$addDt=" kodeunit='".$kdUnit."'";
$addDt2="substr(blok,1,4)='".$kdUnit."'";
if($afdId!='')
{
$sLuasRealisasi="select distinct sum(luasareaproduktif) as luas,tahuntanam from ".$dbname.".setup_blok 
                 where substr(kodeorg,1,6)='".$afdId."' group by tahuntanam";
$addDt="kodeblok like '".$afdId."%'";
$addDt2="substr(blok,1,6)='".$afdId."'";
}
...
$sKgTaon="select distinct sum(kgsetahun) as kgstaun,thntnm from ".$dbname.".bgt_produksi_kbn_kg_vw 
          where tahunbudget='".$thn[0]."' and ".$addDt." group by thntnm";
...
$sSensus="select distinct sum(kgsensus) as kgsensus, tahuntanam from ".$dbname.".kebun_spb_vs_rencana_blok_vw 
          where periode='".$periode."' and ".$addDt2." group by tahuntanam";
//exit("Error:".$sSensus);
...
{
    $sSensus="select distinct sum(kgsensus) as kgsensus,tahuntanam from ".$dbname.".kebun_spb_vs_rencana_blok_vw 
      where periode<'".$thn[0]."-07' and  ".$addDt2." group by tahuntanam";
//    $sSensus="select distinct sum(kgsensus) as kgsensus,tahuntanam from ".$dbname.".kebun_spb_vs_rencana_blok_vw 
...
{
    $sSensus="select distinct sum(kgsensus) as kgsensus,tahuntanam from ".$dbname.".kebun_spb_vs_rencana_blok_vw 
      where periode>'".$thn[0]."-06' and  ".$addDt2." group by tahuntanam";
//    $sSensus="select distinct sum(kgsensus) as kgsensus,tahuntanam from ".$dbname.".kebun_spb_vs_rencana_blok_vw 
//      where periode<='".$periode."' and substr(blok,1,4)='".$kdUnit."' group by tahuntanam";
    $qSensus=mysql_query($sSensus) or die(mysql_error());
    while($rSensus=mysql_fetch_assoc($qSensus))
...
$sRealaisasi="select distinct sum(nettotimbangan) as realisasi,tahuntanam from  ".$dbname.".kebun_spb_vs_rencana_blok_vw 
              where  periode<='".$period."' and  ".$addDt2." group by tahuntanam";
$qRealisasi=mysql_query($sRealaisasi) or die(mysql_error());
while($rRealisasi=mysql_fetch_assoc($qRealisasi))
{
...
$sRealaisasi="select distinct sum(nettotimbangan) as realisasi,tahuntanam from  ".$dbname.".kebun_spb_vs_rencana_blok_vw 
              where  substr(periode,1,4)='".$thnLalu."' and  ".$addDt2." group by tahuntanam";
$qRealisasi=mysql_query($sRealaisasi) or die(mysql_error());
while($rRealisasi=mysql_fetch_assoc($qRealisasi))
{
...
$sPotensi="select distinct tahuntanam,klasifikasitanah,jenisbibit from ".$dbname.".kebun_spb_vs_rencana_blok_vw 
           where  periode='".$periode."' and  ".$addDt2." ";
 //exit("error:".$sPotensi);
...
if($proses=='excel')
{
	...
	<tr>
        ...
        <td ".$bgcoloraja." rowspan=2>RATA-RATA UMUR TANAMAN</td>
        <td ".$bgcoloraja." rowspan=2>".$_SESSION['lang']['luas']." (Ha)</td>";
        $tab.="<td ".$bgcoloraja." colspan=3>".$_SESSION['lang']['anggaran']." (TON)</td>";
        $tab.="<td ".$bgcoloraja." colspan=2>SENSUS (TON)</td>";
        $tab.="<td ".$bgcoloraja." colspan=2>REALISASI (TON)</td>";
        $tab.="<td ".$bgcoloraja." colspan=2>% PENCAPAIAN REAL VS SNS</td>";
        $tab.="<td ".$bgcoloraja." colspan=2>% PENCAPAIAN REAL VS ANGGR</td>";
        $tab.="<td ".$bgcoloraja." rowspan=2>VARIAN FISIK SBI</td>";
        $tab.="<td ".$bgcoloraja." rowspan=2>S/D BI (PROD. TAHUN  LALU)</td><td ".$bgcoloraja." rowspan=2>SENSUS  SM-I/II</td>";
        $tab.="<td ".$bgcoloraja." rowspan=2>PRODUKSI TAHUN  LALU</td><td ".$bgcoloraja." rowspan=2>POTENSI PRODUKSI</td></tr>";
        $tab.="<tr><td ".$bgcoloraja." >SETAHUN</td><td ".$bgcoloraja." >BI</td>
               <td ".$bgcoloraja." >s/d BI</td><td ".$bgcoloraja." >BI</td><td ".$bgcoloraja." >s/d BI</td><td ".$bgcoloraja." >BI</td><td ".$bgcoloraja." >s/d BI</td>
               <td ".$bgcoloraja." >BI</td><td ".$bgcoloraja." >s/d BI</td><td ".$bgcoloraja." >BI</td><td ".$bgcoloraja." >s/d BI</td></tr>";
        $tab.="</thead>
	<tbody>";
        foreach($dtThnTnm as $lstThnTnm)
        {
           ...
           // $tab.="<td align=right>".number_format($lsRealisasi[$lstThnTnm],2)."</td>";
            ...
            $tab.="<td align=right>".number_format($sbiRealisasi[$lstThnTnm]-$kgSthnsBi[$lstThnTnm],2)."</td>";
           ...
           // $totLReali+=$lsRealisasi[$lstThnTnm];
            ...
            $tvar+=$sbiRealisasi[$lstThnTnm]-$kgSthnsBi[$lstThnTnm];
        }
           ...
           // $tab.="<td align=right>".number_format($totLReali,2)."</td>";
            ...
            $tab.="<td align=right>".number_format($totangVsRealibi,2)."</td>";
            $tab.="<td align=right>".number_format($tvar,2)."</td>";
        ...
        case'pdf':
           ...
           function Header() {
            ...
            global $afdId;
                ...
                $this->Cell(50,$height,"RATA-RATA",TLR,0,'C',1);
                $this->Cell(60,$height," ",TLR,0,'C',1);
                $this->Cell(150,$height," ",TLR,0,'C',1);
                $this->Cell(100,$height," ",TLR,0,'C',1);
                $this->Cell(100,$height," ",TLR,0,'C',1);
                $this->Cell(60,$height,"% ",TLR,0,'C',1);
                $this->Cell(70,$height,"% ",TLR,0,'C',1);
                $this->Cell(30,$height,"S/D BI ",TLR,0,'C',1);
                $this->Cell(55,$height," ",TLR,0,'C',1);
                $this->Cell(55,$height,"PRODUKSI",TLR,0,'C',1);
                $this->Cell(40,$height," ",TLR,1,'C',1);
                
                $this->Cell(25,$height,"Tahun",LR,0,'C',1);
                $this->Cell(50,$height,"UMUR",LR,0,'C',1);
//                $tinggiAkr=$this->GetY();
//                $ksamping=$this->GetX();
//                $this->SetY($tinggiAkr);
//                $this->SetX($ksamping-572);
                ...
                $this->Cell(100,$height,"SENSUS (TON)",LR,0,'C',1);
                $this->Cell(100,$height,"REALISASI (TON)",LR,0,'C',1);
                $this->Cell(60,$height,"REAL VS",LR,0,'C',1);
                $this->Cell(70,$height,"REAL VS",LR,0,'C',1);
                $this->Cell(30,$height,"(PROD.",LR,0,'C',1);
                $this->Cell(55,$height,"SENSUS",LR,0,'C',1);
                $this->Cell(55,$height,"TAHUN",LR,0,'C',1);
                $this->Cell(40,$height,"POTENSI",LR,1,'C',1);
                
                $this->Cell(25,$height,"Tanam",LR,0,'C',1);
                $this->Cell(50,$height,"TANAMAN",LR,0,'C',1);
                $this->Cell(60,$height," ",LR,0,'C',1);
                $this->Cell(150,$height," ",LR,0,'C',1);
                $this->Cell(100,$height," ",LR,0,'C',1);
                $this->Cell(100,$height," ",LR,0,'C',1);
                $this->Cell(60,$height,"SNS",LR,0,'C',1);
                $this->Cell(70,$height,"ANGGR",LR,0,'C',1);
                $this->Cell(30,$height,"TAHUN",LR,0,'C',1);
                $this->Cell(55,$height,"SM-I/II ",LR,0,'C',1);
                $this->Cell(55,$height,"LALU",LR,0,'C',1);
                $this->Cell(40,$height,"PRODUKSI",LR,1,'C',1);
                
                $this->Cell(25,$height," ",BLR,0,'C',1);
                $this->Cell(50,$height," ",BLR,0,'C',1);
                $this->SetFont('Arial','B',6);
                $this->Cell(30,$height,"ANGGR",TBLR,0,'C',1);
                $this->Cell(30,$height,"REAL",TBLR,0,'C',1);
                
                $this->Cell(50,$height,"SETAHUN",TBLR,0,'C',1);
                ...
                $this->Cell(50,$height,"BI",TBLR,0,'C',1);
                $this->Cell(50,$height,"s/d BI",TBLR,0,'C',1);
                $this->Cell(30,$height,"BI",TBLR,0,'C',1);
                $this->Cell(30,$height,"s/d BI",TBLR,0,'C',1);
                $this->Cell(35,$height,"BI",TBLR,0,'C',1);
                $this->Cell(35,$height,"s/d BI",TBLR,0,'C',1);
                $this->SetFont('Arial','B',7);
                $this->Cell(30,$height,"LALU)",BLR,0,'C',1);
