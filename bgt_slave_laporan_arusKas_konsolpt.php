...
//echo $sSum;
$qSum=mysql_query($sSum) or die(mysql_error($sSum));
...
$sSum2="select distinct substring(a.noakun, 1, 3) AS dnoakun,sum(setahun) as setahun,unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a 
    ...
    group by substr(a.noakun,1,3)  asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
//exit("Error".$sSum2);
$qSum2=mysql_query($sSum2) or die(mysql_error($sSum2));
$rupiahBulan=array();
while($rSum2=mysql_fetch_assoc($qSum2))
{
    $rupAkunStaon[$rSum2['dnoakun']][$rSum2['induk']]+=$rSum2['setahun'];
}
//noakun pengeluaraan
...
//echo $sSumPeng;
$qSumPeng=mysql_query($sSumPeng) or die(mysql_error($sSumPeng));
...
$sSum2="select distinct substring(a.noakun, 1, 3) AS dnoakun,sum(setahun) as setahun,unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a 
        left join ".$dbname.".organisasi b on a.unit=b.kodeorganisasi
        where tahunbudget='".$thnBudget."' and (substr(a.noakun,1,3)>'125' and substr(a.noakun,1,3)<'129') and a.noakun!=0 group by substr(a.noakun,1,3) order by substr(a.noakun,1,3) asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
//echo $sSum2;
//exit("Error".$sSum2);
$qSum2=mysql_query($sSum2) or die(mysql_error($sSum2));
$rupiahBulan=array();
...
$sSum2="select distinct substring(a.noakun, 1, 3) AS dnoakun,sum(setahun) as setahun, unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a  
        ...
        tahunbudget='".$thnBudget."' and substr(a.noakun,1,1)='6' and a.noakun!=0 group by substr(a.noakun,1,3) order by substr(a.noakun,1,3) asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
//echo $sSum2;
//exit("Error".$sSum2);
...
while($rSum2=mysql_fetch_assoc($qSum2))
{
    $rupAkunStaon3[$rSum2['dnoakun']][$rSum2['induk']]+=$rSum2['setahun'];
    $byLangsung+=$rSum2['setahun'];
}
...
$sSum2="select distinct substring(a.noakun, 1, 3) AS dnoakun,sum(setahun) as setahun, unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a  
        left join ".$dbname.".organisasi b on a.unit=b.kodeorganisasi where
        tahunbudget='".$thnBudget."'and substr(a.noakun,1,1)='7' or substr(a.noakun,1,1)='8' and a.noakun!=0 group by substr(a.noakun,1,3) order by substr(a.noakun,1,3) asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
//echo $sSum2;
//exit("Error".$sSum2);
$qSum2=mysql_query($sSum2) or die(mysql_error($sSum2));
$rupiahBulan2=array();
while($rSum2=mysql_fetch_assoc($qSum2))
{
    $rupAkunStaon5[$rSum2['dnoakun']][$rSum2['induk']]+=$rSum2['setahun'];
            ...
            foreach($dtNoakun as $barisNoakun)
            {
               ...
               // $tab.="<td   align=right>".number_format($totalDes[$barisNoakun],2)."</td>";
                foreach($listUnit as $listBln)
                {
                   ...
                   $totLangsung[$listBln]+=$rupAkunStaon2[$barisNoakun3][$listBln];
                }
