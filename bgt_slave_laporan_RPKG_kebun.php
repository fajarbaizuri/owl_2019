...
while($bar=mysql_fetch_object($res))
{
    ...
    if(substr($bar->noakun,0,3)=='122' or substr($bar->noakun,0,3)=='123'  or substr($bar->noakun,0,3)=='124'  or substr($bar->noakun,0,3)=='125'){
        if(substr($bar->noakun,0,5)=='12201')
        {
            $luasxtbm2=$luaslc;
            @$rpperha=$bar->rupiah/$luasxtbm2;
            @$rpperhatbm+=$bar->rupiah/$luasxtbm2;
        }
        else if(substr($bar->noakun,0,5)>='12201')
        {
            $luasxtbm2=$luasxtbm;
            @$rpperha=$bar->rupiah/$luasxtbm2;
            @$rpperhatbm+=$bar->rupiah/$luasxtbm2;
