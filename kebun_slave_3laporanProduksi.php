...
switch($proses)
{
	...
	if($tipeIntex==1)
	{
		//$sOrg="SELECT namaorganisasi,kodeorganisasi FROM ".$dbname.".organisasi WHERE tipe='KEBUN' and induk in(select induk from ".$dbname.".organisasi where tipe='PABRIK') order by namaorganisasi asc";
            $sOrg="SELECT namaorganisasi,kodeorganisasi FROM ".$dbname.".organisasi WHERE tipe='KEBUN' and induk ='FBB' order by namaorganisasi asc";
	}
	elseif($tipeIntex==0)
	{
		$sOrg="SELECT namasupplier,`supplierid`,`kodetimbangan` FROM ".$dbname.".log_5supplier WHERE substring(kodekelompok,1,1)='S' order by namasupplier asc";//echo "warning:".$sOrg;
		//$sOrg="SELECT namasupplier,`supplierid` FROM ".$dbname.".log_5supplier WHERE substring(kodekelompok,1,1)='S' order by namasupplier asc";//echo "warning:".$sOrg;
		//echo $sOrg;exit();
	}
	elseif($tipeIntex==2)
	{
	...
	while($rOrg=mysql_fetch_assoc($qOrg))
	{
		if($tipeIntex==0)
		{
		   if($rOrg['kodetimbangan']!=''){
			     $optorg.="<option value=".$rOrg['kodetimbangan'].">".$rOrg['namasupplier']."</option>";
			}
			else
			{
			   $optorg.="<option value=".$rOrg['supplierid'].">".$rOrg['namasupplier']."</option>";
			}			
		}
		else
		{
...
if($kodeUnit=='')
	...
	$sData="select kodeorg,sum(jumlahtandan1) as jjg,sum(beratbersih-kgpotsortasi) as netto,kodecustomer,tanggal,sum(kgpotsortasi) as potongan
	,substr(nospb,9,6) as afd 
	        from ".$dbname.".pabrik_timbangan where kodebarang='40000003' ".$where." group by substr(nospb,9,6),substr(tanggal,1,10)";
	...
	if($brs>0)
	{
		...
		{	
			...
			<td align=right>".number_format($rData['potongan'],2)."</td>
			</tr>";
                        //<tr><td colspan=5><div id=detail_".$no."></div></td></tr>";
			$subtota+=$rData['netto'];
			$poto+=$rData['potongan'];
		}
		echo"<tr class=rowcontent ><td colspan=5 align=right>Total (KG)</td>
		      <td align=right>".number_format($subtota,2)."</td>
			  <td align=right>".number_format($poto,2)."</td>
			  </tr>";
	}
	else
	{
	...
	$unit=$_GET['unit'];
	 ...
	 class PDF extends FPDF
       ...
       /*         $query = selectQuery($dbname,'organisasi','namaorganisasi,alamat,telepon',
				...
				$this->Cell(12/100*$width,$height,'AFD',1,0,'C',1);				
				$this->Cell(12/100*$width,$height,$_SESSION['lang']['tanggal'],1,0,'C',1);		
				$this->Cell(10/100*$width,$height,$_SESSION['lang']['janjang'],1,0,'C',1);			
				$this->Cell(15/100*$width,$height,"Normal (KG)",1,0,'C',1);	
				$this->Cell(15/100*$width,$height,"Potongan (KG)",1,1,'C',1);
            }
		...
		$sList="select kodeorg,sum(jumlahtandan1) as jjg,sum(beratbersih-kgpotsortasi) as netto,kodecustomer,tanggal,sum(kgpotsortasi) as potongan  
			    ,substr(nospb,9,6) as afd from ".$dbname.".pabrik_timbangan where kodebarang='40000003' ".$where." group by substr(nospb,9,6), substr(tanggal,1,10)";
		...
		{			
			...
			$pdf->Cell(12/100*$width,$height,$rData['afd'],1,0,'C',1);	
			...
			$pdf->Cell(15/100*$width,$height,number_format($rData['netto'],2),1,0,'R',1);
			$pdf->Cell(15/100*$width,$height,number_format($rData['potongan'],2),1,1,'R',1);
			$subtota+=$rData['netto'];
			$poto+=$rData['potongan'];
		}
		$pdf->Cell(55/100*$width,$height,"Total",1,0,'C',1);
		$pdf->Cell(15/100*$width,$height,number_format($subtota,2),1,0,'R',1);
		$pdf->Cell(15/100*$width,$height,number_format($poto,2),1,1,'R',1);
	...
	$tab.="<td bgcolor=#DEDEDE>Afd</td>";	
	$tab.="<td bgcolor=#DEDEDE>".$_SESSION['lang']['tanggal']."</td>
		<td bgcolor=#DEDEDE>".$_SESSION['lang']['janjang']."</td>
		<td bgcolor=#DEDEDE>Normal (KG)</td>
		<td bgcolor=#DEDEDE>Potongan (KG)</td>
	</tr>
	</thead>
	<tbody>";
	...
	if($brs>0)
	{
		...
		$tab.="<tr class=rowcontent ><td colspan=5 align=right>Total (KG)</td><td align=right>".number_format($subtota,2)."</td><td align=right>".number_format($poto,2)."</td></tr>";
	}
	else
	{
