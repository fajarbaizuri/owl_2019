...
if(($unit=='')||($tahun=='')){
    echo "Warning: silakan memilih gudang dan periode."; exit;
}
//namabarang
...
and periode = '".$tahunlalu."-12' and kodebarang like '".$kelompok."%' group by kodebarang";
$qData=mysql_query($sData);// or die(mysql_error());
$per=$tahun."-01";
$awal=$per."A"; 
while($rData=mysql_fetch_assoc($qData))
...
//harga januari
$sData="select kodebarang, sum(saldoakhirqty) as saldo, hargarata, periode from ".$dbname.".log_5saldobulanan where kodegudang = '".$unit."' ".$buattes."
and periode = '".$tahun."-01' and kodebarang like '".$kelompok."%' group by kodebarang order by periode";
$qData=mysql_query($sData);// or die(mysql_error());
while($rData=mysql_fetch_assoc($qData))
{
    $harga=$tahun."-01H";  
...
for ($i=1; $i<=$bulan; $i++)
{
    ...
    and tanggal like '".$per."-%' and notransaksi like '%GR%' and post = '1' and kodebarang like '".$kelompok."%' group by kodebarang";
    $qData=mysql_query($sData);// or die(mysql_error());
    while($rData=mysql_fetch_assoc($qData))
    {
        $resData[$rData['kodebarang']][kobar]=$rData['kodebarang'];
    ...
    //pengeluaran
    $sData="select kodebarang, sum(jumlah) as saldo from ".$dbname.".log_transaksi_vw where kodegudang = '".$unit."' ".$buattes."
    and tanggal like '".$per."-%' and notransaksi like '%GI%' and post = '1' and kodebarang like '".$kelompok."%' group by kodebarang";
    $qData=mysql_query($sData);// or die(mysql_error());
    while($rData=mysql_fetch_assoc($qData))
    {
        $resData[$rData['kodebarang']][kobar]=$rData['kodebarang'];  
    ...
    //saldoakhir sistem
    $sData="select kodebarang, sum(saldoakhirqty) as saldo, hargarata, periode from ".$dbname.".log_5saldobulanan where kodegudang = '".$unit."' ".$buattes."
    and periode = '".$per."' and kodebarang like '".$kelompok."%' group by kodebarang order by periode";
    $qData=mysql_query($sData);// or die(mysql_error());
    while($rData=mysql_fetch_assoc($qData))
    {
...
//saldo akhir	
$sData="select kodebarang, saldoqty as saldo from ".$dbname.".log_5masterbarangdt where kodegudang = '".$unit."' and kodebarang like '".$kelompok."%' ".$buattes."";
$qData=mysql_query($sData);// or die(mysql_error());
while($rData=mysql_fetch_assoc($qData))
{
...
//    $tab.="<td rowspan=4 bgcolor='AAAAFF' align=right>".number_format($ar[sallu])."</td>";
    ...
    for ($i=1; $i<=$bulan; $i++)
    {
        ...
        $tab.="<td bgcolor='AAFFAA' align=right>".number_format($tampilan)."</td>"; 
    ...
    $tab.="<td bgcolor='AAFFAA' align=right><b>".number_format($tampilan)."</b></td>"; 
    $tab.="</tr><tr class=rowcontent>";
    ...
    for ($i=1; $i<=$bulan; $i++)
    {
        ...
        $tab.="<td bgcolor='FFAAAA' align=right>".number_format($tampilan)."</td>"; 
    ...
    $tab.="<td bgcolor='FFAAAA' align=right><b>".number_format($tampilan)."</b></td>"; 
    $tab.="</tr><tr class=rowcontent>";
