    ...
    switch($_POST['proses']) {
		...
		//exit("Error:MASUK");
			...
			if($res>0)
			{
					...
					$query="insert into ".$dbname.".log_permintaanhargadt (`nomor`,`kodebarang`,`harga`,`spec`,`jumlah`, `kurs`,`matauang`,`tgldari`,`tglsmp`,`satuan`) 
                                            values ('".$data['kode']."','".$data['kdbrg']."','".$data['price']."','".$data['rspek']."','".$data['jmlh']."','".$data['jmlhKurs']."','".$data['kurs']."','".tanggalsystem($data['tglDari'])."','".tanggalsystem($data['tglSamp'])."','".$data['satuan']."')";
...
//                                                                    values('".$no_prmntan."','".$kdbrg."','".$spec."','".$jmlh."','".$jmlhKurs."','".$mtuang."')		
		...
		//exit("Error:ASD");
            ...
            # Make Query
			//echo $data['jmlh'];exit();
		   $query = "update ".$dbname.".`log_permintaanhargadt` set kodebarang='".$data['kdbrg']."',harga='".$data['price']."',spec='".$data['rspek']."',jumlah='".$data['jmlh']."',kurs='".$data['jmlhKurs']."',matauang='".$data['krs']."',`tgldari`='".tanggalsystem($data['tglDari'])."',`tglsmp`='".tanggalsystem($data['tglSamp'])."' ,satuan='".$data['satuan']."' 
    ...
    if($data!=array()) {
        foreach($data as $key=>$row) {
            ...
            $table .= "<td>".makeElement("nm_brg_".$key."",'txt',$res['namabarang'],
			 ...
			 $table.="<td><select id=sat_".$key." style='width:70px' disabled>".$optSatuan."</option></td>";
			
			/*$table .= "<td>".makeElement("sat_".$key."",'txt',$optSatuan,
                array('style'=>'width:70px','disabled'=>'disabled','class=myinputtext'));*/
            ...
            $table .= "<td>".makeElement("jumlah_".$key."",'textnumber',$row['jumlah'],
