...
$sKodeorg="select distinct kodebarang from ".$dbname.".kebun_pakai_material_vw 
          where substr(kodeorg,1,4)='".$kdUnit."' and substr(kodebarang,1,3) in ('311', '312')  order by kodebarang asc";
$qKodeorg=mysql_query($sKodeorg) or die(mysql_error());
while($rKodeorg=mysql_fetch_assoc($qKodeorg))
{
...
if($proses=='excel')
{
	...
	<tr>
        ...
        <td colspan=".$cols.">BI</td>
        <td colspan=".$cols.">SBI</td>
        </tr><tr>";
        ...
        case'pdf':
           ...
           function Header() {
            ...
            global $dtKodeorg;
                ...
                $this->Cell($width,$height,strtoupper("13.3 DISTRIBUSI PUPUK, AGROCHEMICAL DAN KACANGAN"),0,1,'L');
                ...
                $this->Cell($wit,$height,"BI",TLR,0,'C',1);
                $this->Cell($wit,$height,"SBI",TLR,1,'C',1);
            ...
            if(!empty($dtBarang))foreach($dtBarang as $lstBarang=>$lstDtBarang)
            {
              if($klmprBrg2!=substr($lstDtBarang,0,3))
              {
                ...
                $pdf->Cell($witda,$height," ",TBLR,1,'C',1);
              }
