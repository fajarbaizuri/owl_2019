...
//$where="tahunbudget='".$thnbudget."' and kodetraksi='".$kdorg."' and kodews='".$kdws."'";
//untuk update
//$where2="tahunbudget='".$thnbudget."' and kodetraksi='".$kdorg."' and kodews='".$kdtrak."'";
?>
...
switch($method)
{
	...
	case'cekHead':
        ...
        if($dtK!=$dtA)
        {
       		exit("Error:Data Tahun Tidak Benar");
        }           
    ...
    case'saveData':
        ...
        {
            exit("Error:Total Jam Perbulan Lebih Besar Dari Total Jam Setahun");
		}
	...
	case'loadData':
			...
			<button class=mybutton onclick=cariBast(".($page+1).");>".$_SESSION['lang']['lanjut']."</button>
				...
				/*case 'update':
					...
					{
						exit("Error:Total Jam/Tahun Tidak Boleh Kosong");
					}
					else if($totalSum>$totjamthn)
					{
						exit ("Error:AAA");	
					}
					$sGet="select jam01, jam02, jam03, jam04, jam05, jam06, jam07, jam08, jam09, jam10, jam11, jam12 from ".$dbname.".bgt_ws_jam where ".$where."";
					$qGet=mysql_query($sGet) or die(mysql_error($conn));
					while($rGet=mysql_fetch_assoc($qGet))
					{
						...
						$arrJam[$no]=$rGet['jam'.$b];
					}
					$sUpdate="update ".$dbname.".bgt_ws_jam set jampertahun='".$totjamthn."' where ".$where."";
					//exit("Error:$sUpdate");					
...
/*			case'getData':
			...
			if($thnbudget!=''||$kdorg!=''||$kdtrak!='')
			{
				$getdt="select * from ".$dbname.".bgt_ws_jam WHERE  ".$where." ";
				
				$qOpt=mysql_query($getdt) or die(mysql_error());
				while($rOpt=mysql_fetch_assoc($qOpt))
				{		
					...
					$isi.="<td><input type='text' class=myinputtextnumber id=jam_x".$bre." style='width:50px' onkeypress=\"return angka_doang(event);\" value='".$rOpt['jam'.$a]."' /></td>";
					}	
				}
			}
			...
			else
			{
				...
				{
					$isi.="<td><input type='text' class=myinputtextnumber id=jam_x".$brs2." value='".$bagi."' style='width:50px' onkeypress=\"return angka_doang(event);\" /></td>";
				}
			}
			$isi.="<td align=center style='cursor:pointer;'><img id='detail_add' title='Simpan' class=zImgBtn onclick=\"saveJam(".$totBrs.")\" src='images/save.png'/></td></tr>";
			echo $isi;
	break;*/
