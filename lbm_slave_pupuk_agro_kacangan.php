...
$thnLalu=$thn[0]-1;
$whre="and substr(kodebarang,1,3) in ('311', '312') and kodegudang='".$kdUnit."'
...
where substr(kodebarang,1,3) in ('311', '312') and periode='".$periode."' and kodegudang='".$kdUnit."'
 group by kodebarang order by kodebarang asc";
...
//bln lalu
$sPosAwalThn="SELECT sum( `saldoakhirqty` ) AS saldoAkhir, kodebarang from ".$dbname.".log_5saldobulanan 
where substr(kodebarang,1,3) in ('311', '312') and periode='".$thn[0]."-".$blnLalu."' and kodegudang='".$kdUnit."'
 group by kodebarang order by kodebarang asc";
...
//thn lalu
$sPosAwalThn="SELECT sum( `saldoakhirqty` ) AS saldoAkhir, kodebarang from ".$dbname.".log_5saldobulanan 
where substr(kodebarang,1,3) in ('311', '312') and periode='".$thnLalu."-12' and kodegudang='".$kdUnit."'
 group by kodebarang order by kodebarang asc";
$qPosAwalThn=mysql_query($sPosAwalThn) or die(mysql_error());
while($rPosAwalThn=mysql_fetch_assoc($qPosAwalThn))
{
...
if($proses=='excel')
{
    ...
    $tab.="
    <table>
    <tr><td colspan=5 align=left><b>13.2 STOCK PUPUK, AGROCHEMICAL DAN KACANGAN</b></td><td colspan=7 align=right><b>".$_SESSION['lang']['bulan']." : ".substr(tanggalnormal($periode),1,7)."</b></td></tr>
	...
	<tr>
        ...
        <td rowspan=2>POSISI AWAL TAHUN</td>
        <td rowspan=2>S/D BLN LALU</td>
        <td colspan=2>PENERIMAAN</td>
        <td colspan=2>PENGELUARAN</td>
        <td rowspan=2>SISA AKHIR BLN</td>
        </tr>
        <tr><td>BI</td><td>SBI</td><td>BI</td><td>SBI</td></tr>";
        $tab.="</thead>
	<tbody>";
//echo"<pre>";
//print_r($dtBarang);
...
switch($proses)
{
	...
	break;
        ...
        $dte=date("Hms");
        $nop_="stock_pupuk_dan_kacangan_".$dte;
        ...
        case'pdf':
           ...
           function Header() {
            ...
            global $dtBarang;
                ...
                $this->Cell($width,$height,strtoupper("13.2 STOCK PUPUK, AGROCHEMICAL DAN KACANGAN"),0,1,'L');
                ...
                $this->SetY($tinggiAkr+20);
                $this->SetX($ksamping);
                $this->Cell(120,$height,"NAMA",TLR,0,'C',1);
                $this->Cell(60,$height,"POSISI",TLR,0,'C',1);
                $this->Cell(50,$height,"S/D",TLR,0,'C',1);
                $this->Cell(100,$height,"PENERIMAAN",TLR,0,'C',1);
                $this->Cell(100,$height,"PENGELUARAN",TLR,0,'C',1);
                $this->Cell(50,$height,"SISA",TLR,1,'C',1);
                
                $this->Cell(120,$height,"BAHAN",BLR,0,'C',1);
                $this->Cell(60,$height,"AWAL TAHUN",BLR,0,'C',1);
                $this->Cell(50,$height,"BLN LALU",BLR,0,'C',1);
                $this->Cell(50,$height,"BI",TBLR,0,'C',1);
                $this->Cell(50,$height,"s/d BI",TBLR,0,'C',1);
                $this->Cell(50,$height,"BI",TBLR,0,'C',1);
                $this->Cell(50,$height,"s/d BI",TBLR,0,'C',1);
                $this->Cell(50,$height,"AKHIR BLN",BLR,1,'C',1);
            ...
            foreach($dtBarang as $lstBarang=>$lstBarangDt)
            {
                ...
                if($klmprBrg2!=substr($lstBarangDt,0,3))
                {
                    ...
                    $pdf->Cell(360,$height," ",TBLR,1,'C',1);
                }
                $pdf->Cell(120,$height,$optNmBrg[$lstBarangDt],TBLR,0,'L',1);
                $pdf->Cell(60,$height,number_format($dtAwalThn[$lstBarangDt],2),TBLR,0,'R',1);
                $pdf->Cell(50,$height,number_format($dtBlnLalu[$lstBarangDt],2),TBLR,0,'R',1);
