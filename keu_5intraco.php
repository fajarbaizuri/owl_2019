...
foreach($arrjenis as $kei=>$fal)
{
	$optpt.="<option value='".$kei."'>".$fal."</option>";
	 ...
	 echo"<select id=jenis style='width:300px;'>".$optpt."</select></td></tr>";
        //=================ambil akun;  
        $str="select noakun,namaakun from ".$dbname.".keu_5akun where (noakun like '116%') and char_length(noakun)=7 order by noakun";
        $res=mysql_query($str);
        $optpt="";
